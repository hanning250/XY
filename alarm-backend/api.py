...
from alarm_helpers import notify_alarm, serialize_alarm, to_alarm_record
...
def create_app(storage, config, alarm_service=None, notifier=None):
    ...
    app = FastAPI(
        title="海康 NVR 防护服告警后端",
        description="接收 NVR/AI 摄像头告警，提供查询、WebSocket 推送与 ISAPI HTTP 入口",
        version="1.1.0",
        lifespan=lifespan,
    )
