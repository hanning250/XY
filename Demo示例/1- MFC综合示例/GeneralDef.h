...
#define WM_PROC_CONSUME_ALARM	        WM_USER + 110   //consume alarm handle
#define WM_PROC_TRANS_RECORD_ALARM	    WM_USER + 111   //consume alarm handle

#define DEMO_FULL_WIDTH 1024	//default demo width
#define DEMO_FULL_HEIGHT 800	//default demo height
...
#define EVENT_ACK_TIMER	     		24		//event ack timer
...
#define MAX_OLD_RETRANS_ID 15
#define MAX_OLD_RETRANS_ID_LEN  (NET_SDK_UUID_LEN + 1)* MAX_OLD_RETRANS_ID
...
typedef struct STRU_DEVICE_INFO{
    ...
    BOOL    bEventAck;              //if the eventAck is enabled
    char	szRetransmissionID[NET_SDK_UUID_LEN + 1];		//retransmissionID,有结束符
    char	szOldRetransIDList[MAX_OLD_RETRANS_ID_LEN];   //历史重传ID，多个用逗号隔开
    int     iEventAckMode;               // ACK模式: 0-一对一默认应答, 1-间隔时间批量应答
    int     iEventAckInterval;      // 批量应答间隔时间(秒)
    int     iEventUploadRate;       //histrory event upload rate
    ...
    // bySupport4 & 0x01 同时支持RTSP和私有协议。为1时，表示所有码流类型同时支持RTSP协议和私有协议，此时byProtocol无效； 为0时，byProtocol有效。
    // bySupport4 & 0x02 表示是否支持NetSDK透传接口（NET_DVR_STDXMLConfig）透传表单格式
    // bySupport4 & 0x4表示是否支持拼控统一接口
    // bySupport4 & 0x20 是否支持DVR_GETWORKSTATE_V40和DVR_GET_HDGROUP_CFG_V40，值为0x20时支持，否则不支持
    // bySupport4 & 0x40 0表示设备不支持校验字段验证；1表示设备支持校验和字段验证；
...
typedef struct STRU_LOCAL_PARAM
{
    ...
    int			iEventAckInterval;	//event ack interval(s)
	STRU_LOCAL_PARAM()
	{
        bReconnect          = TRUE;
		bCyclePlay			= FALSE;
		...
		sprintf(chClientRecordPath ,"%s", "C:\\mpeg4record\\");
...
// 设备ACK信息结构
typedef struct DEVICE_ACK_INFO {
    int iDeviceIndex;                   // 设备ID(在g_struDeviceInfo中的下标)
    int* pPendingSequences;              // 待确认序号数组
    int iSequenceCount;                  // 当前序号数量
    int iArraySize;                      // 数组大小
    DWORD dwLastAckTime;                  // 上次发送ACK的时间
    struct DEVICE_ACK_INFO* pNext;        // 下一个设备节点
} DEVICE_ACK_INFO, *LPDEVICE_ACK_INFO;

//保存设备重传头文件的
typedef struct tagSAVE_RETRANS_EVENT_DATA
{
    char szRetransmissionID[64];
    NET_DVR_RETRANSMISSION_HEAD struHead;
    struct tagSAVE_RETRANS_EVENT_DATA* pNext;
} SAVE_RETRANS_EVENT_DATA, *LPSAVE_RETRANS_EVENT_DATA;

typedef struct tagSAVE_RETRANS_EVENT_QUEUE
{
    LPSAVE_RETRANS_EVENT_DATA pHead;
    LPSAVE_RETRANS_EVENT_DATA pTail;
    CRITICAL_SECTION csLock; // 保护队列
    HANDLE hEvent;           // 唤醒事件
    BOOL bExit;              // 退出标志
    // 注意：_beginthread 返回的是线程句柄，但通常在线程结束后会自动关闭
    // 如果需要等待线程结束，我们仍需保存句柄
    HANDLE hThread;
} SAVE_RETRANS_EVENT_QUEUE, *LPSAVE_RETRANS_EVENT_QUEUE;


// 全局管理结构
typedef struct {
    DEVICE_ACK_INFO* pDeviceList;          // 设备链表头
    CRITICAL_SECTION csDeviceList;      // 链表保护临界区
    UINT_PTR timerId;                   // 定时器ID
    //HWND hMainWnd;                      // 主窗口句柄
} EVENT_ACK_MANAGER, *LPEVENT_ACK_MANAGER;
