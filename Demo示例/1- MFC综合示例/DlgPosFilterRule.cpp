...
RECT g_rectSetArea;
void CALLBACK SetOSDRegionCallBack(LONG nPort,HDC hDc,void* pUser)
{
 	::DrawEdge(hDc,&g_rectSetArea,BDR_SUNKENOUTER,BF_RECT);
	SetBkMode(hDc,TRANSPARENT);
}

void CALLBACK  GetOSDCallBack(LONG nPort,HDC hDc,void* pUser)
{
	CRect rect;
	CPoint point;
...
void CDlgPosFilterRule::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_bDrawdetect)
	{
		if (MK_CONTROL&nFlags)
		{
			...
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, SetOSDRegionCallBack, NULL);
// 			point.x -= 1;
// 			point.y -= 1;
// 			if (point.x < 0)
// 			{
...
void CDlgPosFilterRule::OnChkShow() 
{
	...
	if (m_bShowOSD)
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
		Sleep(200);
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,GetOSDCallBack, 0);
	}
	else
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
