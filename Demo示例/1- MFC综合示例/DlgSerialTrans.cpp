...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CALLBACK g_fSerialDataCallBack(LONG lSerialHandle, char *pRecvDataBuffer, DWORD dwBufSize, void* pUser)
{
    CString tmp1;
    CString tmp2;
    CTime ct;
    int index = 0;
    char sString[MAX_CONTENT_LEN + 1] = { 0 };
    memcpy(sString, pRecvDataBuffer, (dwBufSize > MAX_CONTENT_LEN) ? MAX_CONTENT_LEN : dwBufSize);
...
void CCustListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    static _TCHAR szBuff[MAX_CONTENT_LEN];//这里是你要显示的字符串长度
    LV_ITEM lvi;
