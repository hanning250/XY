...
Description:		
Input:		lPlayHandle, playback handle;dwDataType,data type,data head or stream;pBuffer,data buffer;dwBufSize,data size;pUser,user data;
Output:	
Return:	
**********************************************************/
extern void CALLBACK PlayDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,void* pUser);
...
void CDlgPlayEvent::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_ZONE_NO, m_iZoneNo);
    DDX_Control(pDX, IDC_COMBO_DRIVER_BEHAVIOR, m_comboDriverBehaviortType);
    DDX_Control(pDX, IDC_COMBO_ADAS_TYPE, m_comboADASType);
    DDX_Control(pDX, IDC_COMBO_GSENSOR_TYPE, m_comboGSensorType);
    DDX_Control(pDX, IDC_COMBO_SENSOR_IN_TYPE, m_comboSensorInType);
}
...
BOOL CDlgPlayEvent::OnInitDialog()
{
	...
	index++;
	//minor event, now for Behavior detection
    ...
    m_comboDriverBehaviortType.ResetContent();
    int iIndex = 0;

    g_StringLanType(szLan, "保留", "reserve");
    m_comboDriverBehaviortType.AddString(szLan);
    m_comboDriverBehaviortType.SetItemData(iIndex, iIndex);
    iIndex++;
    ...
    m_comboDriverBehaviortType.SetCurSel(0);

    m_comboADASType.ResetContent();
    iIndex = 0;

    g_StringLanType(szLan, "保留", "reserve");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;
    ...
    g_StringLanType(szLan, "车道偏离", "drivingOnLaneLine");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "盲区检测", "vehicleBlindSpotDetection");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "车距检测", "vehicleDistanceDetectione");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;
    ...
    g_StringLanType(szLan, "急加速", "rapidAcceleration");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "急减速", "rapidDeceleration");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "急左转弯", "suddenLeftTurn");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "急右转弯", "suddenRightTurn");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "车辆翻车", "rollover");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "未礼让行人", "notYieldToPedestrian");
    m_comboADASType.AddString(szLan);
    m_comboADASType.SetItemData(iIndex, iIndex);
    iIndex++;

    m_comboADASType.SetCurSel(0);

    m_comboGSensorType.ResetContent();
    iIndex = 0;

    g_StringLanType(szLan, "保留", "reserve");
    m_comboGSensorType.AddString(szLan);
    m_comboGSensorType.SetItemData(iIndex, iIndex);
    iIndex++;
    ...
    g_StringLanType(szLan, "车辆碰撞", "collision");
    m_comboGSensorType.AddString(szLan);
    m_comboGSensorType.SetItemData(iIndex, iIndex);
    iIndex++;

    m_comboGSensorType.SetCurSel(0);

    m_comboSensorInType.ResetContent();
    iIndex = 0;

    g_StringLanType(szLan, "保留", "reserve");
    m_comboSensorInType.AddString(szLan);
    m_comboSensorInType.SetItemData(iIndex, iIndex);
    iIndex++;
    ...
    g_StringLanType(szLan, "左转弯", "leftTurn");
    m_comboSensorInType.AddString(szLan);
    m_comboSensorInType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "右转弯", "rightTurn");
    m_comboSensorInType.AddString(szLan);
    m_comboSensorInType.SetItemData(iIndex, iIndex);
    iIndex++;

    g_StringLanType(szLan, "倒车", "reverse");
    m_comboSensorInType.AddString(szLan);
    m_comboSensorInType.SetItemData(iIndex, iIndex);
    iIndex++;

    m_comboSensorInType.SetCurSel(0);
...
BOOL CDlgPlayEvent::CheckInitParam()
{
	...
	g_StringLanType(szLan, "异常行为检测", "Behavior detection");
	m_comboMajorType.AddString(szLan);
    ...
    g_StringLanType(szLan, "海康报警主机", "security control panel");
    m_comboDeviceType.InsertString(iIndex, szLan);
    m_comboDeviceType.SetItemData(iIndex, FILE_SECURITY_CONTROL_PANEL);
    iIndex++;
...
void CDlgPlayEvent::OnBnClickedBtnRemoteSearchList()
{
	...
	if (!m_bSearching)
	{
        //memset(&m_struEventParamV50, 0, sizeof(NET_DVR_SEARCH_EVENT_PARAM_V50));
        //获取通道信息
        //之前通道选中部分，然后取消选中的通道，代码中是直接删除最后一个,这样导致通道号不是界面上选中的通道号
        //因此直接设置前获取选中的通道号
        GetChannelInfoFromWnd();
		m_IsQuit = TRUE;
        ...
        case EVENT_IOT_CASE:
            ...
            m_struEventParamV50.uSeniorParam.struIOTAlarm.wZoneNo = m_iZoneNo;
            break;
		default:
			break;
		}
...
void CDlgPlayEvent::UpdateMinorEvent()
{
	...
	case EVENT_VCA_BEHAVIOR:
        ...
        g_StringLanType(szLan, "人员密度（人数预警）", "Person density");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_PERSON_DENSITY);
        index++;

        g_StringLanType(szLan, "手动报警", "Manual alarm");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_MANUAL_ALARM);
        index++;

        g_StringLanType(szLan, "超速", "Violation high speed");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_VIOLATION_HIGH_SPEED);
        index++;

        g_StringLanType(szLan, "G-Sensor事件", "G-Sensor event");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_GSENSOR);
        index++;

        g_StringLanType(szLan, "Sensor-In行为", "Sensor-In behavior");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_SENSOR_IN);
        index++;

        g_StringLanType(szLan, "司机驾驶行为", "DBD");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_VCA_DBD);
        index++;

        g_StringLanType(szLan, "高级辅助驾驶", "ABS");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_VCA_ABS);
        index++;

        g_StringLanType(szLan, "如厕超时", "Toilet tarry");
        m_comboMinorEvent.AddString(szLan);
        m_comboMinorEvent.SetItemData(index, EVENT_TOILET_TARRY);
        index++;
	...
	case EVENT_STREAM_INFO:
		...
		g_StringLanType(szLan, "目标抓拍", "Face snap");
		m_comboMinorEvent.AddString(szLan);
	...
	case EVENT_VCA_DETECTION:
		...
		index++;
		g_StringLanType(szLan, "人脸采集", "face detect");
		m_comboMinorEvent.AddString(szLan);
...
void CDlgPlayEvent::OnCbnSelchangeComboDeviceType()
{
    ...
    const char* szSecurityControlPanel[] = { "zone", "host" };
    ...
    case FILE_CAMERA_DETECTOR:
        ...
        m_comboFindSubType.SetItemData(iIndex, 0xffff);
        break;
    case FILE_SECURITY_CONTROL_PANEL:
        for (iIndex = 0; iIndex < sizeof(szSecurityControlPanel) / sizeof(char*); iIndex++)
        {
            m_comboFindSubType.InsertString(iIndex, szSecurityControlPanel[iIndex]);
            m_comboFindSubType.SetItemData(iIndex, iIndex + 1);
        }
        m_comboFindSubType.InsertString(iIndex, "all");
        m_comboFindSubType.SetItemData(iIndex, 0xffff);
...
//从界面按照表示通道获取通道信息
void CDlgPlayEvent::GetChannelInfoFromWnd()
{
    int iChannelNo=0;
    switch (m_iMajorType)
    {
    case EVENT_MOT_DET:
        //如果是全选，则所有通道都添加
        if (m_listChan.GetCheck(0))
        {
            for (int i = 1; i < m_listChan.GetItemCount(); i++)
            {
                m_struEventParamV50.uSeniorParam.struMotionParam.wMotDetChanNo[iChannelNo++] = m_listChan.GetItemData(i);
            }
        }
        else //否则只选择选中的
        {
    ...
    case EVENT_ALARM_IN:
        ...
        if (m_listChan.GetCheck(0))
        {
            for (int i = 1; i < m_listChan.GetItemCount(); i++)
            {
                m_struEventParamV50.uSeniorParam.struAlarmParam.wAlarmInNo[iChannelNo++] = m_listChan.GetItemData(i);
            }
        }
        else //否则只选择选中的
        {
    ...
    case EVENT_VCA_BEHAVIOR:
        ...
        if (m_listChan.GetCheck(0))
        {
            for (int i = 1; i < m_listChan.GetItemCount(); i++)
            {
                m_struEventParamV50.uSeniorParam.struVcaParam.wChanNo[iChannelNo++] = m_listChan.GetItemData(i);
            }
        }
        else //否则只选择选中的
        {
    ...
    case EVENT_VCA_DETECTION:
        ...
        else //否则只选择选中的
        {
            ...
            for (int i = 0; i < m_listChan.GetItemCount(); i++)
            {
                ...
                if (m_listChan.GetCheck(i))
                {
                    m_struEventParamV50.uSeniorParam.struVCADetect.wChanNo[iChannelNo++] = m_listChan.GetItemData(i);
                }
            }
        }
        break;
    case EVENT_POS:
        ...
        else //否则只选择选中的
        {
            for (int i = 1; i < m_listChan.GetItemCount(); i++)
            {
                ...
                if (m_listChan.GetCheck(i))
                {
                    m_struEventParamV50.uSeniorParam.struPosAlarm.wChannel[iChannelNo++] = m_listChan.GetItemData(i);
                }
            }
        }
        break;
    case EVENT_IOT_CASE:
        ...
        else //否则只选择选中的
        {
            for (int i = 1; i < m_listChan.GetItemCount(); i++)
            {
                ...
                if (m_listChan.GetCheck(i))
                {
                    m_struEventParamV50.uSeniorParam.struIOTAlarm.wChannel[iChannelNo++] = m_listChan.GetItemData(i);
                }
            }
        }
        break;
