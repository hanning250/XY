...
CDlgRecPassBack::CDlgRecPassBack(CWnd* pParent /*=NULL*/)
	...
	, m_iHourDiffWithUTCStart(0)
	, m_iMinDiffWithUTCStart(0)
	, m_iHourDiffWithUTCEnd(0)
	, m_iMinDiffWithUTCEnd(0)
	, m_bISO8601(FALSE)
{
...
// 	DDX_Text(pDX, IDC_EDIT_STREAM_ID2, m_csStreamID2);
	...
	DDX_Text(pDX, IDC_EDIT_UTC_DIFF_H_START, m_iHourDiffWithUTCStart);
	DDX_Text(pDX, IDC_EDIT_UTC_DIFF_M_START, m_iMinDiffWithUTCStart);
	DDX_Text(pDX, IDC_EDIT_UTC_DIFF_H_END, m_iHourDiffWithUTCEnd);
	DDX_Text(pDX, IDC_EDIT_UTC_DIFF_M_END, m_iMinDiffWithUTCEnd);
	DDX_Check(pDX, IDC_CHK_ISO8601, m_bISO8601); 
}
...
// 		}
		...
		pRecordPassBackManulCfg->byISO8601 = m_bISO8601;
		pRecordPassBackManulCfg->cStartTimeDifferenceH = m_iHourDiffWithUTCStart;
		pRecordPassBackManulCfg->cStartTimeDifferenceM = m_iMinDiffWithUTCStart;
		pRecordPassBackManulCfg->cStopTimeDifferenceH = m_iHourDiffWithUTCEnd;
		pRecordPassBackManulCfg->cStopTimeDifferenceM = m_iMinDiffWithUTCEnd;
...
void CDlgRecPassBack::AddTaskInfoToDlg( LPNET_DVR_RECORD_PASSBACK_MANUAL_TASK_RET lpInter )
{
	...
	else if (4 == lpInter->byTaskStatus)
	{
		g_StringLanType(szLan, "回传失败", "Failed");
	}
	else if (5 == lpInter->byTaskStatus)
	{
		g_StringLanType(szLan, "执行成功，但只回传了部分录像", "Success, but only part of the video was returned");
	}
	else if (6 == lpInter->byTaskStatus)
	{
		g_StringLanType(szLan, "执行成功，但前端无录像", "Success, but there is no video on the front end");
	}
	else
	{
		g_StringLanType(szLan, "未知状态", "Unknown status");
