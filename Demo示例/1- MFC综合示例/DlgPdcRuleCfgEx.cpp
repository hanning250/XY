...
void CALLBACK DrawPDCShowEx(LONG lRealHandle, HDC hDc, void* pUser)
{
    ...
    {
        g_pDlgPdcRuleCfg->F_DrawPdcRule(lRealHandle, hDc, pUser);
    }
...
BEGIN_MESSAGE_MAP(CDlgPdcRuleCfgEx, CDialog)
    ...
    ON_BN_CLICKED(IDC_CHK_RULE, &CDlgPdcRuleCfgEx::OnBnClickedChkRule)
END_MESSAGE_MAP()
...
void CDlgPdcRuleCfgEx::PDCDrawDirection(HDC hDc, void* pUser)
{
    CPen cpDrawPen;
...
void CDlgPdcRuleCfgEx::PDCDrawCalib(HDC hDc, void* pUser)
{
...
void CDlgPdcRuleCfgEx::PDCDrawCalibResult(HDC hDc, void* pUser)
{
    DRAW_POLYGON_PARAM_EX struMaxPolygonParam;
    DRAW_POLYGON_PARAM_EX struMinPolygonParam;
    memset(&struMaxPolygonParam, 0, sizeof(struMaxPolygonParam));
...
long CDlgPdcRuleCfgEx::StartRealPlay()
{
    ...
    {
        if (!NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawPDCShowEx, NULL)) 
        {
...
void CDlgPdcRuleCfgEx::F_DrawPdcRule(LONG lRealHandle, HDC hDc, void* pUser)
{
    F_DrawFrame(lRealHandle, hDc, pUser,  RGB(255, 255, 0));
    PDCDrawDirection(hDc, pUser);
    PDCDetectLine(hDc, m_struPdcRuleCfg.struLine);
    PDCDrawCalib(hDc, pUser);
    PDCDrawCalibResult(hDc, pUser);
    F_DrawPolyLine(lRealHandle, hDc, pUser, RGB(255, 255, 0));
}
...
void CDlgPdcRuleCfgEx::GetPdcRuleWnd()
{
    ...
    m_comboSignalType.SetCurSel(m_struPdcRuleCfg.bySignalType);
...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CDlgPdcRuleCfgEx::F_DrawFrame(long lRealHandle, HDC hDc, void* pUser,  COLORREF color)
{
    using namespace Gdiplus;
    Graphics graphics(hDc);
...
void CDlgPdcRuleCfgEx::F_DrawPolyLine(long lRealHandle, HDC hDc, void* pUser, COLORREF color)
{
    using namespace Gdiplus;
    Graphics graphics(hDc);
    SolidBrush  brush(Color(50, 0, 0, 0));
...
void CDlgPdcRuleCfgEx::OnBnClickedChkRule()
{
    // TODO:  在此添加控件通知处理程序代码
}
