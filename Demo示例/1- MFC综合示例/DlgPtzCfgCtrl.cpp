...
void CDlgPtzCfgCtrl::F_DrawPrivacyMasksRegion(LONG lRealHandle, HDC hDc, void* pUser)
{
...
void CDlgPtzCfgCtrl::F_DrawFrame(long lRealHandle, HDC hDc, void* pUser,  COLORREF color)
{
...
void CDlgPtzCfgCtrl::F_DrawFun(long lPlayHandle, HDC hDc, void* pUser)
{
    if (m_bChkPrivacyMasksRect)
    {
		F_DrawFrame(lPlayHandle, hDc, pUser,  RGB(0, 255, 0));
		//F_DrawPolygon(hDc);
    }
}

void CALLBACK DrawRectShow(LONG lRealHandle, HDC hDc,void* pUser)
{
    SetBkMode(hDc, TRANSPARENT);
    SetTextColor(hDc, RGB(255, 255, 255));
	
    lpPtzCfgCtrlDlg->F_DrawFun(lRealHandle, hDc, pUser);
}
...
BOOL CDlgPtzCfgCtrl::play()
{
	...
	{
		if (!NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawRectShow, NULL)) 
        {
            g_pMainDlg->AddLog(m_lLoginID, OPERATION_FAIL_T, "NET_DVR_RigisterDrawFunEx Fail");
