...
//最大1000个事件
#define MAX_EVENT_TYPE_NUM 1000

struct SubcirbeEventCap
{
    string strEventType;
    string strPictureUrlType;
    string strDefaultPictureUrlType;
};
...
public:
    ...
    //起一个线程获取能力集
    void CreateAbilityThread();
    void CloseAbilityThread();
    void SetBrokenStatus(BOOL bStatus);
    static DWORD WINAPI GetAbilityThread(LPVOID lpArg);
    ...
    void GenerateAndSetRetransmissionID();
    void AppendRetransID(char* szOldRetransIDList, int nMaxLen, const char* szNewRetransID);
    void GetRetransmissionParam(CString & strData);
    ...
    void GetSubcribeCapability();

    string m_strAllPictureUrlType;

    SubcirbeEventCap m_listSubcirbeEventCap[MAX_EVENT_TYPE_NUM];//暂定当前最大1000个事件
    afx_msg void OnCbnSelchangeComboEventType();
    BOOL m_bCheckSubcribeCap;
    afx_msg void OnBnClickedCheckSubcribeCap();
    BOOL m_bEnableXml;
    BOOL m_bChkHeatMapDurationPDC;
    BOOL m_bChkHeatMapIntersection;
    BOOL m_bRetAlarmTypeV40;
    BOOL m_bNVRAlarmIPC;
    BOOL m_bVMDPic;
    BOOL m_bAidBroken;
    BOOL m_bTfsBroken;
    BOOL m_bTPsStatBroken;
    BOOL m_bRuleBroken;

    HANDLE m_hAbilityThread;
    BOOL m_bAIOPByISAPI;
    BOOL m_bChildDevice;
    BOOL m_bVCAByISAPI;
    BOOL m_bSmokeByISAPI;
    BOOL m_bMoveByISAPI;
    BOOL m_bEventAck;
    CString m_szRetransmissionID;
    int m_iEventAckInterval;
    int m_iEventAckMode;
    CStringList m_retransmissionIDList;
    int m_iUploadRate;
    int m_iMaxUploadRate;
    int m_iMinUploadRate;
    void CheckRetransmissionAbility();
    CString GetSelectedRetransmissionID();
    BOOL IsIDInOldList(const CString& strID);
    void CAlarmAdvanceSetter::UpdateOldRetransIDList(const CStringArray& arrReturnedIDs);
    void SplitString(const CString& strSource, const CString& strDelim, CStringArray& arrResult);

    


    afx_msg void OnCbnSelchangeComboEventAckMode();
    afx_msg void OnBnClickedButtonGetRetransmissionId();
    afx_msg void OnBnClickedButtonCancaelRetransmissionId();

    afx_msg void OnEnKillfocusEditUploadrate();
};
