...
void CALLBACK g_DrawITCTriggerRgn(LONG lRealHandle, HDC hDc, void* pUser)
{  
 	CDlgVcaITCTriggerCfg* pDlg = (CDlgVcaITCTriggerCfg*)pUser;
...
CDlgVcaITCTriggerCfg::CDlgVcaITCTriggerCfg(CWnd* pParent /*=NULL*/)
    ...
    , m_bEnhancedMode(FALSE)
    , m_bPicRecognition(FALSE)
    , m_bEnableParkingDetect(FALSE)
    , m_iParkingDuration(0)
    , m_iParkingAlarmIntervalTime(0)
{
	...
	m_iTriggerMode = 0;
	m_lDownloadHandle = -1;
	m_szFilePath.Format("%s\\BlockAllowCfg.xls", g_struLocalParam.chRemoteCfgSavePath);
	m_timerHandle = -1;
	m_lUploadHandle = -1;
	m_bStop = FALSE;
    m_bCountryIndex = 0;
...
void CDlgVcaITCTriggerCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_ENHANCED_MODE, m_bEnhancedMode);
    DDX_Check(pDX, IDC_CHECK_PIC_RECOGNITION, m_bPicRecognition);
    DDX_Check(pDX, IDC_CHECK_ENABLE_PARKING_DETECT, m_bEnableParkingDetect);
    DDX_Text(pDX, IDC_EDIT_PARKING_DURATION, m_iParkingDuration);
    DDX_Text(pDX, IDC_EDIT_PARKING_INTERVAL_TIME, m_iParkingAlarmIntervalTime);
}
...
BEGIN_MESSAGE_MAP(CDlgVcaITCTriggerCfg, CDialog)
    ...
    ON_BN_CLICKED(IDC_BTN_BLOCKALLOWLIST_UPLOAD, OnBtnBlockallowlistUpload)
    ON_BN_CLICKED(IDC_BTN_BLOCKALLOWLIST_DOWNLOAD, OnBtnBlockallowlistDownload)
    ON_BN_CLICKED(IDC_BTN_BLOCKALLOWLIST_DOWNLOAD_STOP, OnBtnBlockallowlistDownloadStop)
    ON_BN_CLICKED(IDC_BTN_BLOCKALLOWLIST_UPLOAD_STOP, OnBtnBlockallowlistUploadStop)
    ON_BN_CLICKED(IDC_BTN_BLOCKALLOWLIST_BROWSE, OnBtnBlockallowlistBrowse)
...
// 		bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, g_DrawITCTriggerRgn, NULL);//DC Draw callback
// 	}
// 
...
{   
    ...
    g_StringLanType(szText, "治安布防", "security monitoring");
...
//		m_comboCarDriveDirect.SetCurSel(struPlateRecogParam.byCarDriveDirect);
        ...
        m_bEnhancedMode = struIpcHvtParam.byEnhancedMode;
        m_bPicRecognition = struIpcHvtParam.byPicRecognition;

        m_bEnableParkingDetect = struIpcHvtParam.struParkingDetection.byEnable;
        m_iParkingDuration = struIpcHvtParam.struParkingDetection.wDuration;
        m_iParkingAlarmIntervalTime = struIpcHvtParam.struParkingDetection.wAlarmIntervalTime;
...
//		struPlateRecogParam.byCarDriveDirect = m_comboCarDriveDirect.GetCurSel();
        ...
        struIpcHvtParam.byEnhancedMode = m_bEnhancedMode;
        struIpcHvtParam.byPicRecognition = m_bPicRecognition;
        struIpcHvtParam.struParkingDetection.byEnable = m_bEnableParkingDetect;
        struIpcHvtParam.struParkingDetection.wDuration = m_iParkingDuration;
        struIpcHvtParam.struParkingDetection.wAlarmIntervalTime = m_iParkingAlarmIntervalTime;
	}
	SaveViaLaneWndContent(m_nCurLaneIndex);
	UpdateData(FALSE);
}
...
BOOL CDlgVcaITCTriggerCfg::SetITCTriggerCfg()
{
    ...
    else
    {
        if (dwStatus != 0)
        {
            g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_SET_TRIGGEREX_CFG Chan[%d],dwStatus[%d]", m_lChannel, dwStatus);
            bRet = FALSE;
        }
        else
        {
...
void CDlgVcaITCTriggerCfg::OnBtnBlockallowlistUpload() 
{
	// TODO: Add your control notification handler code here
...
// 	}
	...
	m_lUploadHandle = NET_DVR_UploadFile_V40(m_lServerID, UPLOAD_VEHICLE_BLOCKALLOWLIST_FILE, NULL, 0, szFilePath, NULL, 0);
    //m_lUploadHandle = NET_DVR_UploadFile_V40(m_lUserID, UPLOAD_VEHICLE_BLOCKALLOWLIST_FILE, &struUploadDBIn, sizeof(struUploadDBIn), m_strFileID, &struUploadDBOut, sizeof(struUploadDBOut));
    ...
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "UPLOAD_VEHICLE_BLOCKALLOWLIST_FILE");
...
void CDlgVcaITCTriggerCfg::OnBtnBlockallowlistDownload() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	char szFilePath[256] = {0};
	...
	m_lDownloadHandle = NET_DVR_StartDownload(m_lServerID, NET_SDK_DOWNLOAD_VEHICLE_BLOCKALLOWLIST_FILE, NULL, 0, szFilePath);
...
void CDlgVcaITCTriggerCfg::OnBtnBlockallowlistDownloadStop() 
{
	// TODO: Add your control notification handler code here
...
void CDlgVcaITCTriggerCfg::OnBtnBlockallowlistUploadStop() 
{
	// TODO: Add your control notification handler code here
...
void CDlgVcaITCTriggerCfg::OnBtnBlockallowlistBrowse()
{
	UpdateData(TRUE);
