...
void DlgAcsCardPasswdCfg::AddToCardList(const NET_DVR_CARD_PASSWD_CFG& struCardInfo, const char * pCardNo)
{
    ...
    if ( iItemIndex == -1)
    {
        ...
        m_listCard.SetItemData(nItemCount, (LPARAM)pCardInfo);
    }
    else
    {
    ...
    lpArr[iItemIndex] = (LPARAM)pCardInfo;
...
PULONG_PTR DlgAcsCardPasswdCfg::GetCardCfgPoint()
{
    if ( !m_lpRecordCardCfg)
    {
...
void DlgAcsCardPasswdCfg::BatchAddCardToList()
{
    ...
    for (i=0; i<m_dwCardNum; i++)
    {
        ...
        m_listCard.SetItemData(iItemIndex, (LPARAM)pCardInfo);
