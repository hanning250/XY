...
RECT g_rectHideAreaShow[MAX_SHELTERNUM];//display mask area
RECT g_rectHideAreaMouse[MAX_SHELTERNUM];//set mask area
...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CALLBACK ShelterAreaGetCallBack(LONG nPort,HDC hDc,void* pUser)
{
	CRect rect;
	int i = 0;
	//CPoint point;
...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CALLBACK ShelterAreaSetCallBack(LONG nPort,HDC hDc,void* pUser)
{
...
BOOL CDlgRemoteHideArea::OnInitDialog() 
{
	...
	{
		char szLan[128] = {0};
		g_StringLanType(szLan, "ÍøÂçÁ¬½ÓÊ§°Ü", "Network connection failure");
		AfxMessageBox(szLan);
	}	
	m_bDrawdetect = FALSE;
...
/*********************************************************
  ...
  Desc:		ctrl+left click to draw mask area
  Input:	
  Output:	
  Return:	
**********************************************************/
void CDlgRemoteHideArea::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if (m_bDrawdetect)
	{
		...
		{
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, ShelterAreaSetCallBack, NULL);
...
/*********************************************************
  ...
  Desc:		mouse drag and click to bset mask area
...
/*********************************************************
  ...
  Desc:		complete mask area settings;
  Input:	
  Output:	
  Return:	
**********************************************************/
...
/*********************************************************
  ...
  Desc:		calcel mask area settings
  Input:	
  Output:	
  Return:	
**********************************************************/
...
/*********************************************************
  ...
  Desc:		display masik asrea settings
  Input:	
  Output:	
  Return:	
**********************************************************/
void CDlgRemoteHideArea::OnBnClickedChkHideAreaShow()
{
	...
	if (m_bCheckHideAreaShow)
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
		Sleep(200);
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,ShelterAreaGetCallBack, 0);
	}
	else
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
