...
void CALLBACK DrawSearchMotionArea(LONG lRealHandle, HDC hDc, void* pUser)
{
...
void CDlgPlayMotion::OnBtnPreview() 
{
    if (m_lRealPlayHandle >= 0)
    {
		NET_DVR_RigisterDrawFunEx(m_lRealPlayHandle, NULL, this);
        StopPlay();
        Invalidate();
    }
    else
    {
        ...
        if (m_lRealPlayHandle >= 0)
        {
            NET_DVR_RigisterDrawFunEx(m_lRealPlayHandle, DrawSearchMotionArea, this);
        }
    }
}
...
void CDlgPlayMotion::F_DrawFun( long lPlayHandle, HDC hDc, void* pUser)
{
    if (m_cmbSearchType.GetCurSel() == 0)
    {
        F_DrawMotion(hDc);
...
void CDlgPlayMotion::OnBtnGetPtz() 
{
    ...
    else
    {
        ...
        if (dwRetLen != 0)
        {
            pRecvBuf = new BYTE[dwRetLen + 1];
            if (pRecvBuf != NULL)
            {
                memset(pRecvBuf,0,dwRetLen + 1);
            }
    ...
    m_fPanPos = ntohl(m_struPTZInfo.dwP) / 10.0;
    m_fTitlePos = ((int)ntohl(m_struPTZInfo.dwT)) / 10.0;
    m_fZoomPos = ntohl(m_struPTZInfo.dwZ) / 10.0;
