...
// 		}
        ...
        char szUrl[256] = { 0 };
        _snprintf(szUrl, sizeof(szUrl), "GET /ISAPI/System/TwoWayAudio/channels/%d\r\n", m_lChannel);
        ...
        NET_LOCAL_AUDIOCAST_CFG struAudioCfg = { 0 };
        ...
        else
        {
            ...
            //解析
            if (NULL != xmlBase.Parse(pOutBuf))
            {
                if (xmlBase.FindElem("TwoWayAudioChannel") && xmlBase.IntoElem())
                {
                    if (xmlBase.FindElem("audioCompressionType") && ("PCM" == xmlBase.GetData()))
                    {
                        AfxMessageBox("SDK not support PCM");
                    }
                    else if ("AAC" == xmlBase.GetData()) //针对AAC的情况解析下参数
                    {
                        if (xmlBase.FindElem("audioBitRate"))
                        {
                            std::string csAudioBitRate = xmlBase.GetData();
                            if (csAudioBitRate.compare("8") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_8kps;
                            }
                            else if (csAudioBitRate.compare("16") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_16kps;
                            }
                            else if (csAudioBitRate.compare("32") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_32kps;
                            }
                            else if (csAudioBitRate.compare("64") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_64kps;
                            }
                            else if (csAudioBitRate.compare("128") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_128kps;
                            }
                            else if (csAudioBitRate.compare("192") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_192kps;
                            }
                            else if (csAudioBitRate.compare("40") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_40kps;
                            }
                            else if (csAudioBitRate.compare("48") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_48kps;
                            }
                            else if (csAudioBitRate.compare("56") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_56kps;
                            }
                            else if (csAudioBitRate.compare("80") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_80kps;
                            }
                            else if (csAudioBitRate.compare("96") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_96kps;
                            }
                            else if (csAudioBitRate.compare("112") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_112kps;
                            }
                            else if (csAudioBitRate.compare("144") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_144kps;
                            }
                            else if (csAudioBitRate.compare("160") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_160kps;
                            }
                            else if (csAudioBitRate.compare("224") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_224kps;
                            }
                            else if (csAudioBitRate.compare("256") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_256kps;
                            }
                            else if (csAudioBitRate.compare("320") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioBitRate = BITRATE_ENCODE_320kps;
                            }
                        }

                        if (xmlBase.FindElem("audioSamplingRate"))
                        {
                            std::string csSampleRate = xmlBase.GetData();
                            if (csSampleRate.compare("16") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioSamplingRate = 1;
                            }
                            else if (csSampleRate.compare("32") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioSamplingRate = 2;
                            }
                            else if (csSampleRate.compare("48") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioSamplingRate = 3;
                            }
                            else if (csSampleRate.compare("44.1") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioSamplingRate = 4;
                            }
                            else if (csSampleRate.compare("5") == 0)
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioSamplingRate = 5;
                            }
                            else
                            {
                                struAudioCfg.struAudioCastParam[2].byAudioSamplingRate = 0;
                            }
                        }
                    }
                    xmlBase.OutOfElem();
        ...
        struAudioCfg.struAudioCastParam[0].byAudioSamplingRate = 3;
        struAudioCfg.struAudioCastParam[0].byAudioBitRate = BITRATE_ENCODE_160kps;
        NET_DVR_SetSDKLocalCfg(NET_SDK_AUDIOCAST_CFG, &struAudioCfg);
