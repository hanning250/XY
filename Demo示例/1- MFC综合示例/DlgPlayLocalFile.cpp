...
{
    if (NULL == g_pDlgPlayLocalFile)
    {
        return;
    }

    g_pDlgPlayLocalFile->UpdateStepBackUI(TRUE);
}
...
void CDlgPlayLocalFile::OnBnClickedBtnSearchLocalList()
{
	...
	char szLan[1024] = {0};
	g_StringLanType(szLan, "�ļ����ҽ���", "Find the end of file");
	AfxMessageBox(szLan);
	UpdateData(FALSE);	
}
...
void CDlgPlayLocalFile::OnBnClickedBtnDeleteLocalFile()
{
	...
	g_StringLanType(szLan, "ȷ��ɾ�����ļ���?", "Sure to delete the file?");
	...
	{
		CString csError;
		g_StringLanType(szLan, "����ɾ���ļ�", "Can not delete file");
		csError.Format("����ɾ���ļ�:%s!", szLan, csFileName.GetBuffer(0));
		MessageBox(csError);
	}
}
...
void CDlgPlayLocalFile::OnBnClickedBtnDeleteAllLocalFile()
{
	...
	g_StringLanType(szLan, "ȷ��ɾ�������ļ���?", "Sure to delete all the files");
	if (IDOK!=MessageBox(szLan,"Warning!",IDOK))
	{
		return;
	}
...
int CDlgPlayLocalFile::SearchLocalFile(int iFileType,int iChanShowIndex, CTime tFrom, CTime tTo)
{
	...
	case 2:
		...
		break;
	case 3://��download record files from device
...
void CDlgPlayLocalFile::PlayBack()
{
	...
	{
		PlayM4_CloseFile(USED_PORT);
		g_StringLanType(szLan, "�ļ���СΪ0", "File size is 0");
		AfxMessageBox(szLan);
		return;
	}
