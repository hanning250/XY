...
void SetDeviceAddress(LPNET_DVR_IPDEVINFO_V31 lpDevInfo, char *lpAddress)
{
	...
	{
        BYTE byIPV6[128] = {0};
        memcpy(byIPV6, lpAddress, strlen(lpAddress));
		if (CheckIPStr(lpAddress))
		{
            if (!g_ValidIPv6(byIPV6))
            {
            ...
            {
                memcpy(lpDevInfo->struIP.byIPv6, byIPV6, 128);
            }
...
void CDlgRemoteIPAccess::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_GET_STREAM, m_bGetStream);
}
...
//  	m_listIPChan.InsertColumn(4, szLan,LVCFMT_LEFT,80,-1);
    ...
    g_StringLanType(szLan, "老师跟随", "Teacher Track");
    m_cmbCameraType.AddString(szLan);
    m_cmbCameraType.SetItemData(dwIndex, 1);
    dwIndex++;

    g_StringLanType(szLan, "学生跟随", "Student Track");
    m_cmbCameraType.AddString(szLan);
...
void CDlgRemoteIPAccess::OnBtnAddIPChan() 
{
	...
	if (m_comboStreamType.GetCurSel() == 0)
	{
		...
		if (!bFind)   //no find
		{
            ...
            if (m_bGetStream)//默认为取流
            {
                m_struIPParaCfgV40.struStreamMode[m_iCurChanIndex].uGetStream.struChanInfo.byGetStream = 0;
            }
            else
            {
                m_struIPParaCfgV40.struStreamMode[m_iCurChanIndex].uGetStream.struChanInfo.byGetStream = 1;
            }
		}
		else
		{ 
	...
	case 0: //direct get stream from the device
        if (m_iCurChanIndex >= 0)
		{
			...
			if (dwID > 0)
            {
                ...
                if (0 == m_struIPParaCfgV40.struStreamMode[m_iCurChanIndex].uGetStream.struChanInfo.byGetStream)
                {
                    m_bGetStream = TRUE;
                } 
                else
