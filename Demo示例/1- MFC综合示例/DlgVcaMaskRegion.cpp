...
void CALLBACK DrawIVMSMaskRegionShow(LONG lRealHandle, HDC hDc, void* pUser)
{
	SetBkMode(hDc, TRANSPARENT);
	SetTextColor(hDc, RGB(255, 255, 255));
	g_pIVMSMaskAreaSet->F_DrawFun(lRealHandle, hDc, pUser);
}
...
void CDlgVcaMaskRegion::EnableDrawPolygon(BOOL Enable)
{
	...
	// 			bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawATMMaskRegionShow, NULL);//set DC draw callback
	// 		}
	// 
	// 	}
}

void CDlgVcaMaskRegion::F_DrawFun(long lPlayHandle, HDC hDc, void* pUser)
{
	COLORREF color;
	color = RGB(255, 255, 0);//yellow
	F_DrawFrame(lPlayHandle, hDc, pUser,  color);
}


void CDlgVcaMaskRegion::F_DrawFrame(long lRealHandle, HDC hDc, void* pUser,  COLORREF color)
{
...
void CDlgVcaMaskRegion::OnRadioPolygon1() 
{
	...
	BOOL bRet;
	bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawIVMSMaskRegionShow, NULL);
...
LONG CDlgVcaMaskRegion::StartPlay()
{
    ...
    if (GetPuStreamCfg(m_struPuStream))
    {
        ...
        else
        {
            ...
            bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawIVMSMaskRegionShow, NULL);//DC Draw callback
        }
