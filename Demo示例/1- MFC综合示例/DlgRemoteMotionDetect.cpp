...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CALLBACK MotionDetectGetCallBack(LONG nPort,HDC hDc,void* pUser)
{
	CRect rect;
	int i = 0,j = 0;
	CPoint point;
...
void CDlgRemoteMotionDetect::F_DrawFun(/*long lPlayHandle,*/ HDC hDc, void* pUser)
{
...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CALLBACK MotionDetectSetCallBack(LONG nPort,HDC hDc,void* pUser)
{
	DWORD i = 0;
...
void CALLBACK MultiMotionDetectSetCallBack(LONG nPort,HDC hDc,void* pUser)
{
	lpRemoteMotionDet->F_DrawFun(/*lRealHandle,*/ hDc, pUser);
}
...
BOOL CDlgRemoteMotionDetect::PreTranslateMessage(MSG* pMsg)
{
	...
	ScreenToClient(&pt);
	//NET_DVR_RigisterDrawFunEx(m_lPlayHandle, MultiMotionDetectSetCallBack, NULL);	
	switch (pMsg->message)
	{
	case WM_LBUTTONDOWN:
...
void CDlgRemoteMotionDetect::OnBnClickedChkMotionDetectShow()
{
	...
	else
	{
		...
		if (m_bChkMotionShow)
		{
			...
			Sleep(200);
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle,MotionDetectGetCallBack, 0);
		}
		else
		{
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
...
void CDlgRemoteMotionDetect::OnBnClickedChkMotionDetectSet()
{
	...
	if (m_bChkMotionSet)
	{
		...
		if (m_bCurConfigurationMode == 1)
		{
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, NULL, NULL);
			Sleep(200);
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, MultiMotionDetectSetCallBack, NULL);
		}
	}
...
void CDlgRemoteMotionDetect::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_bDrawArea)
	{
		...
		{
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, MotionDetectSetCallBack, NULL);
