...
void CALLBACK g_DrawItsCalibrate(LONG lRealHandle, HDC hDc, void* pUser)
{
    if (g_pDlgItsCalibrate != NULL)
    {
        g_pDlgItsCalibrate->F_DrawFun(lRealHandle, hDc, pUser);
    }
...
LONG CDlgVcaITSCalibrate::RealPlay()
{
    ...
    {
        bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, g_DrawItsCalibrate, NULL);//DC Draw callback
    }
...
void CDlgVcaITSCalibrate::F_DrawFun(long lRealHandle, HDC hDc, void* pUser)
{
    if (m_bVerify)
    {
