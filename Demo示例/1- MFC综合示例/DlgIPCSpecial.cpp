...
CDlgIPCSpecial *g_CDlgIPCSpecial = NULL;
...
void CALLBACK g_DrawManualtRace(LONG lRealHandle, HDC hDc, void* pUser)
{
    //CDlgIPCSpecial* pDlg = (CDlgIPCSpecial*)pUser;

    if (g_CDlgIPCSpecial != NULL)
    {
        g_CDlgIPCSpecial->DrawVcaRect(hDc);
    }
...
void CDlgIPCSpecial::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Check(pDX, IDC_CHECK2, m_bPTZPosExEnable);
    DDX_Text(pDX, IDC_EDIT3, m_Panex);
    DDX_Text(pDX, IDC_EDIT12, m_Tiltex);
    DDX_Text(pDX, IDC_EDIT10, m_VisibleZoom);
    DDX_Text(pDX, IDC_EDIT13, m_VisibleFocus);
    DDX_Text(pDX, IDC_EDIT14, m_ThermalFocus);
    DDX_Text(pDX, IDC_EDIT11, m_ThermalZoom);
}
...
// 	{
	   ...
	   else
	   {
		   ...
		   bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, g_DrawManualtRace, NULL);
	   }
 //   }	
...
//	if (!NET_DVR_RemoteControl(m_lServerID, NET_DVR_GET_MOTION_TRACK_CFG, &strMotionTtackCfg, sizeof(strMotionTtackCfg)))
    {
        ...
        g_StringLanType(szLan, "获取跟随模式状态失败", "Fail to get motion track");
        AfxMessageBox(szLan);
    }
    else
    {
...
void CDlgIPCSpecial::OnEditchangeComboPtzpresetno() 
{
    ...
    m_bPTZPosExEnable = m_struPresetName[i].byPTZPosExEnable;
    m_Panex = m_struPresetName[i].struPtzPosEx.fPan;
    m_Tiltex = m_struPresetName[i].struPtzPosEx.fTilt;
    m_VisibleZoom = m_struPresetName[i].struPtzPosEx.fVisibleZoom;
    m_VisibleFocus = m_struPresetName[i].struPtzPosEx.dwVisibleFocus;
    m_ThermalZoom = m_struPresetName[i].struPtzPosEx.fThermalZoom;
    m_ThermalFocus = m_struPresetName[i].struPtzPosEx.dwThermalFocus;
...
void CDlgIPCSpecial::OnBtnSetpresetname() 
{
    ...
    m_struPresetName[i].byPTZPosExEnable = m_bPTZPosExEnable;
    m_struPresetName[i].struPtzPosEx.fPan = m_Panex;
    m_struPresetName[i].struPtzPosEx.fTilt = m_Tiltex;
    m_struPresetName[i].struPtzPosEx.fVisibleZoom = m_VisibleZoom;
    m_struPresetName[i].struPtzPosEx.dwVisibleFocus = m_VisibleFocus;
    m_struPresetName[i].struPtzPosEx.fThermalZoom = m_ThermalZoom;
    m_struPresetName[i].struPtzPosEx.dwThermalFocus = m_ThermalFocus;
...
void CDlgIPCSpecial::OnBtnGettracktype() 
{
    ...
    struTrack.dwChannel = g_struDeviceInfo[m_iDevIndex].pStruChanInfo[iChanIndex].iChannelNO;
	DWORD iRet = 0;
