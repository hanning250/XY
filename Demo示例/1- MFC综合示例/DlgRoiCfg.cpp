...
void CALLBACK DrawRoiDetectShow(LONG lRealHandle, HDC hDc, void* pUser)
{
//      SetBkMode(hDc, TRANSPARENT);
//      SetTextColor(hDc, RGB(255, 255, 255));
 	
     lpRoiCfgDlg->F_DrawFun(lRealHandle, hDc, pUser);     
}
...
BOOL CDlgRoiCfg::Play()
{
	...
	bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawRoiDetectShow, NULL);
	
	return TRUE;
}

void CDlgRoiCfg::F_DrawFun(long lPlayHandle, HDC hDc, void* pUser)
{
	if (m_bFixRoi)
	{
		F_DrawRect(hDc);
...
void CDlgRoiCfg::OnBtnTrack() 
{
	...
	if (bRet)
	{
		g_StringLanType(szLan,"设置Roi跟随模式成功","Success to Set Roi detect Info");
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "Set Roi detect Info Success");
		AfxMessageBox(szLan);
	}
	else
	{        
		g_StringLanType(szLan,"设置Roi跟随模式失败","Fail to Set Roi detect Info");
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "Set Roi detect Info Fail");
		AfxMessageBox(szLan);
	}
...
void CDlgRoiCfg::OnBtnTrackGet() 
{
	...
	if (!bRet || (*pStatus != 0))
	{
		g_StringLanType(szLan,"获取跟随ROI检测区域信息失败","Fail to Get Roi Track Detect Info");
