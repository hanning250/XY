...
void CALLBACK DataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, void* pUser)
{
	int nUserIndex = (int)(LONG_PTR)pUser;
	CString csStr;
	csStr.Format("data length = %d, PassiveHandlle = %d*************///**********\n", dwBufSize, g_struDeviceInfo[g_pMainDlg->GetCurDeviceIndex()].struPassiveDecode[nUserIndex].lPassiveHandle);
	OutputDebugString(csStr);
//	g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), OPERATION_SUCC_T, csStr);
	if (!NET_DVR_MatrixSendData(g_struDeviceInfo[g_pMainDlg->GetCurDeviceIndex()].struPassiveDecode[nUserIndex].lPassiveHandle, (char*)pBuffer, dwBufSize))
	{
...
#endif
	{
		m_PassiveMode.byStreamType = m_comboStreamType.GetCurSel() + 1;//�������ͣ�1-ʵʱ����2-�ļ���
	}
...
void CMatPassive::OnStoppassivedecode() 
{
    ...
    {
		NET_DVR_SetRealDataCallBackEx(g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[dwSelIndex].lRealHandle, NULL, (void*)(LONG_PTR)m_DecChanCombo.GetCurSel());
...
//			NET_DVR_SetStandardDataCallBack(g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[dwSelIndex].lRealHandle, DataCallBack, sel);
			NET_DVR_SetRealDataCallBackEx(g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[dwSelIndex].lRealHandle, DataCallBack, (void*)(LONG_PTR)sel);
        }
    }
#ifdef SUPPORT_SCE
	else if (m_bDesk)
...
#ifdef SUPPORT_WRITEFILE
        m_hRecordHandle = CreateFile("D:\\deskrecord_���ͳɹ�.mp4", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        m_hRecordHandle1 = CreateFile("D:\\deskrecord_ԭʼ��������.mp4", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
...
void CMatPassive::OnStopsenddata() 
{
    ...
    {
        NET_DVR_SetRealDataCallBackEx(g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[dwSelIndex].lRealHandle, NULL, (void*)(LONG_PTR)0);
...
void CMatPassive::OnLogin() 
{
    ...
    if (g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[dwSelIndex].lUserID < 0)
    {
        ...
        {
            char szLan[1024] = {0};
            g_StringLanType(szLan, "�������豸��ַ!", "please input the device ip!");
            AfxMessageBox(szLan);
            return;
        }
        ...
        {
            char szLan[1024] = {0};
            g_StringLanType(szLan, "�Ƿ��Ķ˿ں�!", "illeage port!");
            AfxMessageBox(szLan);
            return;
        }
        ...
        {
            char szLan[1024] = {0};
            g_StringLanType(szLan, "�û��������벻�ܳ���16λ!", "user name and pwd should be shorter than 16 BYTES!");
            AfxMessageBox(szLan);
            return;
        }
