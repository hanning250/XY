...
void CALLBACK DrawHeatMapShow(LONG lRealHandle, HDC hDc, void* pUser)
{
    ...
    {
        g_pDlgHeatMapCfg->F_DrawHeatMapRegion(lRealHandle, hDc, pUser);
    }
...
void CDlgHeatMapCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHK_DAY_REPORT, m_bDayReport);
    DDX_Check(pDX, IDC_CHK_WEEK_REPORT, m_bWeekReport);
    DDX_Check(pDX, IDC_CHK_MONTH_REPORT, m_bMonthReport);
    DDX_Check(pDX, IDC_CHK_YEAR_REPORT, m_bYearReport);
}
...
void CDlgHeatMapCfg::OnBtnPdcRulecfg() 
{
    ...
    m_struHeatMapCfg.struHeatMap.byDayReport = m_bDayReport;
    m_struHeatMapCfg.struHeatMap.byWeekReport =  m_bWeekReport;
    m_struHeatMapCfg.struHeatMap.byMonthReport = m_bMonthReport;
    m_struHeatMapCfg.struHeatMap.byYearReport = m_bYearReport;
...
long CDlgHeatMapCfg::StartRealPlay()
{
    ...
    {
        if (!NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawHeatMapShow, NULL)) 
        {
            g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_RigisterDrawFunEx Fail");
...
void CDlgHeatMapCfg::F_DrawHeatMapRegion(LONG lRealHandle, HDC hDc, void* pUser)
{
    F_DrawFrame(lRealHandle, hDc, pUser,  RGB(255, 255, 0));
}
...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CDlgHeatMapCfg::F_DrawFrame(long lRealHandle, HDC hDc, void* pUser,  COLORREF color)
{
...
void CDlgHeatMapCfg::GetHeatMapRuleWnd()
{
    ...
    m_bDayReport = m_struHeatMapCfg.struHeatMap.byDayReport;
    m_bWeekReport = m_struHeatMapCfg.struHeatMap.byWeekReport;
    m_bMonthReport = m_struHeatMapCfg.struHeatMap.byMonthReport;
    m_bYearReport = m_struHeatMapCfg.struHeatMap.byYearReport;
