...
Input:		lPlayHandle,play handle;dwDataType,data type or file stream;pBuffer,data;dwBufSize,data length;pUser,user address;
Output:	
Return:	
**********************************************************/
...
UINT CDlgPlayRemoteFileMedical::GetMedicalFileThread(LPVOID pParam)
{
	...
	while (g_pDlgRemoteFile->m_bQuit)
	{
		...
		{			
            ...
            if (struFileInfo.byBigFileType == 0) //普通文件搜索
            {
            ...
            else if (struFileInfo.dwTotalLenH == 0)  //1为大文件搜索 
            {
                if (struFileInfo.dwTotalLenL / 1024 == 0)
                {
                    csTmp.Format("%d", struFileInfo.dwTotalLenL);
                }
                else if (struFileInfo.dwTotalLenL / (1024 * 1024) == 0)
                {
                    csTmp.Format("%dK", struFileInfo.dwTotalLenL / 1024);
                }
                else
                {
                    csTmp.Format("%dM", struFileInfo.dwTotalLenL / 1024 / 1024);
                }
            }
            else
            {
                DWORD dwUnit = 0x100000000 / 1024 / 1024 / 1024;
                DWORD dwTotalLen = struFileInfo.dwTotalLenH * dwUnit;   // 把高32位长度转换成GB
                if (dwTotalLen / 1024 == 0)
                {
                    csTmp.Format("%dG", dwTotalLen);
                }
                else if (dwTotalLen / (1024 * 1024) == 0)
                {
                    csTmp.Format("%dT", dwTotalLen / 1024);
                }
                else
                {
                    csTmp.Format("%dP", dwTotalLen / 1024 / 1024);
                }
            }
...
void CDlgPlayRemoteFileMedical::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHK_BIGFILE_SEARCH, m_bBigFileSearch);
}
...
///#endif //NET_DVR_GET_TRAVERSE_PLANE_DETECTION
        ...
        index++;
        g_StringLanType(szLan, "目标抓拍", "FaceSnap");
        m_comboFileType.InsertString(index, szLan);
...
void CDlgPlayRemoteFileMedical::OnBnClickedBtnRemoteSearchList()
{
	...
	if (!m_bSearching)
	{
        ...
        m_strFileCondMedical.dwBigFileType = m_bBigFileSearch;
