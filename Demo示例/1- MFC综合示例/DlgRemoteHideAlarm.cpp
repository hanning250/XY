...
/*********************************************************
  ...
  Return:	
**********************************************************/
void CALLBACK HideAlarmGetCallBack(LONG nPort,HDC hDc,void* pUser)
{
	CRect rect;
	CPoint point;
...
void CALLBACK HideAlarmSetCallBack(LONG nPort,HDC hDc,void* pUser)
{
...
void CDlgRemoteHideAlarm::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if (m_bDrawdetect)
	{
		...
		{
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, HideAlarmSetCallBack, NULL);
...
void CDlgRemoteHideAlarm::OnBnClickedChkHideAlarmShow()
{
	...
	if (m_bChkHideAlarmShow)
	{
		...
		Sleep(200);
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,HideAlarmGetCallBack, 0);
	}
	else
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
