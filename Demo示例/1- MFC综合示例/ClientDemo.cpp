...
EVENT_ACK_MANAGER g_struAckManager = {0};
SAVE_RETRANS_EVENT_QUEUE g_struSaveRetransQueue = { 0 };
...
WORD g_nVideoSecurityTotalNum = 0;	//video security points for preview(camera number)
int g_iNextVideoSecurityIndex = 0;	//last video security point's next serial NO.
...
NET_DVR_ALARM_ISAPI_INFO g_struConsumeISAPIAlarm = { 0 }; //消费机ISAPI报警数据全局存储变量
CDlgConsumeSimulate* g_pDlgConsumeSimulate = NULL;        //消费机-消费模拟窗口全局对象，用于接收报警消息
...
//     _CrtSetBreakAlloc(21094);
    ...
    //SetDllDirectory("..\\Win64_SDK\\");

    //设置HCNetSDK库所在目录

    //     NET_DVR_LOCAL_SDK_PATH struSDKPath = { 0 };
    //     strcpy(struSDKPath.sPath, "E:\\Task2020\\PJ14PD20200615209_2020年HCNetSDK基线项目V6.1.6\\工程路径\\code\\common\\HCNetSDK\\VS2013\\lib\\Win64_SDK\\");
    //     if (!NET_DVR_SetSDKInitCfg(NET_SDK_INIT_CFG_SDK_PATH, &struSDKPath))
    //     {
    //         DWORD dwErrCode = NET_DVR_GetLastError();
    //         char szErrLan[128] = { 0 };
    //         sprintf_s(szErrLan, "NET_SDK_INIT_CFG_SDK_PATH, ErrCode[%d]", dwErrCode);
    ...
    /*//设置openssl路径
    if (!NET_DVR_SetSDKInitCfg(NET_SDK_INIT_CFG_LIBEAY_PATH, "D:\\ComGroup\\code\\Base\\HCNetSDK\\V6.0\\code\\common\\HCNetSDK\\VS2013\\lib\\win32\\ClientDemoDll\\libcrypto-1_1.dll"))
    {
        DWORD dwErrCode = NET_DVR_GetLastError();
        char szErrLan[128] = { 0 };
        sprintf_s(szErrLan, "ErrCode:%d", dwErrCode);
    ...
    if (!NET_DVR_SetSDKInitCfg(NET_SDK_INIT_CFG_SSLEAY_PATH, "D:\\ComGroup\\code\\Base\\HCNetSDK\\V6.0\\code\\common\\HCNetSDK\\VS2013\\lib\\win32\\ClientDemoDll\\libssl-1_1.dll"))
    {
        DWORD dwErrCode = NET_DVR_GetLastError();
        char szErrLan[128] = { 0 };
        sprintf_s(szErrLan, "ErrCode:%d", dwErrCode);
...
//     }
    ...
    NET_DVR_LOCAL_CHECK_DEV struLoaclCheckDev = { 0 };
    struLoaclCheckDev.dwCheckOnlineTimeout = 15 * 1000; //心跳15秒钟发送一次
    ...
    NET_DVR_SetSDKLocalCfg(NET_SDK_LOCAL_CFG_TYPE_CHECK_DEV, &struLoaclCheckDev);

    NET_DVR_LOCAL_GENERAL_CFG struGeneralCfg = { 0 };
    NET_DVR_GetSDKLocalCfg(NET_DVR_LOCAL_CFG_TYPE_GENERAL, &struGeneralCfg);
    struGeneralCfg.byAlarmReconnectMode = 1;
    NET_DVR_SetSDKLocalCfg(NET_DVR_LOCAL_CFG_TYPE_GENERAL, &struGeneralCfg);

    NET_DVR_MESSAGE_CALLBACK_PARAM_V51 struMsgCallbackCfg = { 0 };
    struMsgCallbackCfg.byVcaAlarmJsonType = 1;//默认走老格式回调
    if (!NET_DVR_SetSDKLocalCfg(NET_DVR_LOCAL_CFG_MESSAGE_CALLBACK_V51, &struMsgCallbackCfg))
    {
        OutputDebugString("Set NET_DVR_LOCAL_CFG_MESSAGE_CALLBACK_V51 failed\n");
    }
    else
    {
        OutputDebugString("Set NET_DVR_LOCAL_CFG_MESSAGE_CALLBACK_V51 successful\n");
    }
...
float g_GetFloatValue(float fValue)
{
    ...
    DWORD i = (DWORD)(fValue*1000000); //解决浮点的精度问题 
    DWORD j = (DWORD)(fValue*100000);
