...
void CALLBACK g_RealDataCallBack_V30(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, void *pUser)
{
	...
	case NET_DVR_SYSHEAD://coming the stream header, open stream
		...
		{   
            ...
            if (!PlayM4_ResetBuffer(lPort, BUF_VIDEO_SRC))
            {
                g_dlgOutput[dwWinIndex].AddLog(PLAY_FAIL_T, "PlayM4_ResetBuffer err[%d]",  PlayM4_GetLastError(lPort));
            }
...
//		}
	...
	case NET_DVR_AUDIOSTREAMDATA:
		...
		break;
    case NET_DVR_METADATA_DATA:
    {
        byte *pRtpPacket = new byte[500 * 1024];
        char *pTempBuf = new char[500 * 1024];
        memset(pRtpPacket, 0, 500 * 1024);
        memset(pTempBuf, 0, 500 * 1024);
        memcpy(pRtpPacket, pBuffer, dwBufSize);

        int iPaddingCount = 0;
        if ((pRtpPacket[0] & 0x20) == 0x20)//为1，padding填充
        {
            //如果有padding，RTP包最后一个字节表示padding字节的个数(包括这个数字本身)
            iPaddingCount = pRtpPacket[dwBufSize - 1];
        }

        memcpy(pTempBuf, pBuffer + 12, dwBufSize - 12 - iPaddingCount);

        if (NULL != pTempBuf)
        {
            FILE *fp = fopen("C:\\MetadataData\\Metadata.txt", "a+");
            fseek(fp, 0, SEEK_END);//定位到文件末尾
            fwrite(pTempBuf, strlen(pTempBuf), 1, fp);
            fclose(fp);
        }
...
void CALLBACK g_ESDataCallBack(LONG lRealHandle, NET_DVR_PACKET_INFO_EX *pstruPackInfo, void *pUser)
{
    DWORD dwWinIndex = *((unsigned long *)pUser);
    if (pstruPackInfo->dwPacketSize > 0 && g_dlgOutput[dwWinIndex].m_fpES != NULL)
    {
        fwrite(pstruPackInfo->pPacketBuffer, 1, pstruPackInfo->dwPacketSize, g_dlgOutput[dwWinIndex].m_fpES);
    }
}
...
void CALLBACK g_StdDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize, DWORD dwUser)
{
    ...
    sprintf(szOutput, "gStdDataCallBack, dwDataType[%d], dwBufSize[%d], dwUser[%d]\n", dwDataType, dwBufSize, dwUser);
    OutputDebugString(szOutput);
}

void CALLBACK g_StdDataCallBackEx(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, void* pUser)
{
    char szOutput[1024] = { 0 };
    sprintf(szOutput, "gStdDataCallBackEx, dwDataType[%d], dwBufSize[%d], dwUser[0x%X]\n", dwDataType, dwBufSize, pUser);
    OutputDebugString(szOutput);
}

void CALLBACK g_TpDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, void* pUser)
{
    char szOutput[1024] = { 0 };
    sprintf(szOutput, "gTpDataCallBack, dwDataType[%d], dwBufSize[%d], dwUser[0x%X]\n", dwDataType, dwBufSize, pUser);
    OutputDebugString(szOutput);
}
...
//			struPlayInfo.sMultiCastIP = g_struDeviceInfo[iDeviceIndex].chDeviceMultiIP;
            ...
            if (!g_struDeviceInfo[iDeviceIndex].byURL)
            {
                ...
                //DEBUG
                //NET_DVR_SetStandardDataCallBack(m_lPlayHandle, g_StdDataCallBack, 1234);
                //NET_DVR_SetStandardDataCallBackEx(m_lPlayHandle, g_StdDataCallBackEx, this);
            }
            else
            {
	...
	if (m_lPlayHandle >= 0)
	{
        ...
        if (g_bESCB)
        {
            ...
            sprintf(cFilename, "%s%s_%02d_%d_%4d%02d%02d_%02d%02d%02d_ES.mp4", g_struLocalParam.chClientRecordPath, g_struDeviceInfo[iDeviceIndex].chDeviceIPInFileName, \
                g_struDeviceInfo[iDeviceIndex].iStartChan + pChanInfo->iChanIndex, (pChanInfo->dwProtocol & 0x80000000) >> 31, \
                time.GetYear(), time.GetMonth(), time.GetDay(), time.GetHour(), time.GetMinute(), time.GetSecond());
            if (m_fpES == NULL)
            {
                m_fpES = fopen(cFilename, "wb");
            }
            if (NET_DVR_SetESRealPlayCallBack(m_lPlayHandle, g_ESDataCallBack, &m_iSubWndIndex))
            {
                g_pMainDlg->AddLog(iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SetESRealPlayCallBack");
            }
            else
            {
                g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SetESRealPlayCallBack");
            }
        }
        ...
        int isize = sizeof(struParam);
        if (NET_DVR_GetLinkAddr(m_lPlayHandle, ENUM_LINK_PREVIEW, &struParam))
        {
...
//     char szBuf[128] = { 0 };
//     //非订阅url
//     sprintf_s(szBuf, "rtsp://%s:554/ISAPI/Streaming/channels/101", g_struDeviceInfo[iDeviceIndex].chDeviceIP);
//     //订阅url
//     sprintf_s(szBuf, "rtsp://%s:554/ISAPI/Streaming/nonVideoChannels/1?type=radarDetection", g_struDeviceInfo[iDeviceIndex].chDeviceIP);
//     NET_DVR_PREVIEWINFO_SPECIAL struPlayInfoSp = { 0 };
//     struPlayInfoSp.hPlayWnd = NULL;
//     struPlayInfoSp.bBlocked = g_struLocalParam.bPreviewBlock;
//     struPlayInfoSp.dwDisplayBufNum = m_iNetDelayBufNum;
//     strncpy(struPlayInfoSp.sURL, szBuf, 1024);
//     m_lPlayHandle = NET_DVR_RealPlaySpecial(g_struDeviceInfo[iDeviceIndex].lLoginID, &struPlayInfoSp, g_RealDataCallBack_V30, &m_iSubWndIndex);
	...
	//	struPlayInfo.sMultiCastIP = g_struDeviceInfo[iDeviceIndex].chDeviceMultiIP;
		...
		if (g_bStandardCB) //call back standard stream or not
		{
			...
			if (m_lPlayHandle >= 0)
			{
                ...
                //if (!NET_DVR_SetTransparentDataCallBack(m_lPlayHandle, g_TpDataCallBack, this))
                //{
                //    g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SetTransparentDataCallBack %s", g_struDeviceInfo[iDeviceIndex].pStruChanInfo[iChanIndex].chChanName);
                //}
			}
		}
		else
...
void CDlgOutput::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_lPlayHandle >= 0)
    {
        if (m_bShowOSD)
        {
            g_StringLanType(szLan,"关闭私有信息", "Close OSD");
        }
        else
        {            
            g_StringLanType(szLan,"显示私有信息", "Show OSD");            
        }
...
void CDlgOutput::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if ((MK_CONTROL&nFlags) && (m_lPlayHandle != -1))
	{
		if (g_bPtzMaualTrace)
		{
            ...
            struPtzTrace.dwChannel = g_struDeviceInfo[m_struChanInfo.iDeviceIndex].pStruChanInfo[iChanIndex].iChannelNO;
...
void CDlgOutput::OnMenuShowOSD()
{
    ...
    DWORD dwPRIDATA_RENDER = RENDER_ANA_INTEL_DATA | RENDER_MD | RENDER_ADD_POS | RENDER_ADD_PIC | RENDER_FIRE_DETCET | RENDER_TEM | RENDER_TRACK_TEM | RENDER_THERMAL;
    if (!m_bShowOSD)
    {
        if (m_lPlayHandle > -1)
        {
           ...
           // bRet = RenderPrivateData(lPlayerPort, dwPRIDATA_RENDER,TRUE );
            bRet = NET_DVR_RenderPrivateData(m_lPlayHandle, dwPRIDATA_RENDER, TRUE);
            if (!bRet)
            {
            ...
            NET_DVR_RenderPrivateDataEx(m_lPlayHandle, RENDER_FIRE_DETCET, dwPRIDATA_RENDER, TRUE);
            dwPRIDATA_RENDER = TEM_REGION_BOX | TEM_REGION_LINE | TEM_REGION_POINT;
            NET_DVR_RenderPrivateDataEx(m_lPlayHandle, RENDER_TEM, dwPRIDATA_RENDER, TRUE);

            dwPRIDATA_RENDER = THERMAL_FIREMASK | THERMAL_RULEGAS | THERMAL_TARGETGAS;
            NET_DVR_RenderPrivateDataEx(m_lPlayHandle, RENDER_THERMAL, dwPRIDATA_RENDER, TRUE);
            m_bShowOSD = TRUE;
        }
        else
        {
    ...
    else
    {
        if (m_lPlayHandle > -1)
        {
            bRet = NET_DVR_RenderPrivateData(m_lPlayHandle, dwPRIDATA_RENDER, FALSE);
            if (!bRet)
            {
                m_bShowOSD = TRUE;
            ...
            dwPRIDATA_RENDER = FIRE_FRAME_DIS | FIRE_MAX_TEMP | FIRE_MAX_TEMP_POSITION | FIRE_DISTANCE;
            NET_DVR_RenderPrivateDataEx(m_lPlayHandle, RENDER_FIRE_DETCET, dwPRIDATA_RENDER, FALSE);
            dwPRIDATA_RENDER = TEM_REGION_BOX | TEM_REGION_LINE | TEM_REGION_POINT;
            NET_DVR_RenderPrivateDataEx(m_lPlayHandle, RENDER_TEM, dwPRIDATA_RENDER, FALSE);

            dwPRIDATA_RENDER = THERMAL_FIREMASK | THERMAL_RULEGAS | THERMAL_TARGETGAS;
            NET_DVR_RenderPrivateDataEx(m_lPlayHandle, RENDER_THERMAL, dwPRIDATA_RENDER, FALSE);
