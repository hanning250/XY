...
void CALLBACK g_DrawAidRule(LONG lRealHandle, HDC hDc, void* pUser)
{
    g_pDlgVcaAidRule->F_DrawFun(lRealHandle, hDc, pUser);
}
...
void CDlgVcaAidRuleCfg::InitWnd()
{
    ...
    {
        g_StringLanType(szLan, "非准入名单", "Vehicle Ctrl List");
        m_listAIDType.InsertItem(index, szLan);
        m_listAIDType.SetItemData(index, SMOKE);
        index++;
    }
...
LONG CDlgVcaAidRuleCfg::StartPlay()
{
    if (GetPuStreamCfg(m_struPuStream))
    {
        ...
        else
        {
            ...
            bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, g_DrawAidRule, NULL);//DC Draw callback
        }
...
void CDlgVcaAidRuleCfg::F_DrawFun(long lPlayHandle, HDC hDc, void* pUser)
{
