...
extern EVENT_ACK_MANAGER g_struAckManager;
extern SAVE_RETRANS_EVENT_QUEUE g_struSaveRetransQueue;
...
extern WORD g_nVideoSecurityTotalNum;	//VideoSecurity points for preview(camera number)
extern int g_iNextVideoSecurityIndex;//the  serial NO. of last VideoSecurity point in all play or circulative play
...
extern NET_DVR_ALARM_ISAPI_INFO g_struConsumeISAPIAlarm; //消费机ISAPI报警数据全局存储变量
extern CDlgConsumeSimulate* g_pDlgConsumeSimulate;       //消费机-消费模拟窗口全局对象，用于接收报警消息
...
typedef enum _REMOTE_FILE_TYPE_
{
    ...
    FILE_VCA_EVENT_FACE_SNAP = 52,//目标抓拍
    ...
    FILE_TRAFFIC_VIOLATION = 74, //交通违章
    FILE_THERMOMETRY_DREDGERDETECTION = 75, //挖沙船检测报警
    FILE_THERMOMETRY_WASTE = 76, //废气排放监测上报
    FILE_THERMOMETRY_GREY_SCALE_ALARM = 77, //灰度报警
    FILE_VIBRATION_DETECTION = 78, //振动侦测
    FILE_SMOKE_DETECTION = 79, //烟雾检测
    FILE_CHANNEL_TIMING_RECORDING = 80, //通道定时录像事件
    FILE_AI_OPEN_PLATFORM = 81, //AI开放平台事件
    FILE_SENSOR_ALARM = 82, //传感器事件
    FILE_UVPHOTONEN_DECTION = 83, //紫光电子检测
    FILE_DEV_POWER_STATUS_REPORT = 84, //设备电源状态上报
    FILE_ALL = 0xff //全部
}REMOTE_FILE_TYPE;
