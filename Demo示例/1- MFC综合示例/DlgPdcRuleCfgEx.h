...
public:
	...
	void PDCDrawDirection(HDC hDc, void* pUser);
    void PDCDetectLine(HDC hDc, NET_VCA_LINE struLine);
	void GetPdcRuleWnd();
    void PDCDrawCalibResult(HDC hDc, void* pUser);
    void PDCDrawCalib(HDC hDc, void* pUser);
	long StartRealPlay();  // 开始预览 
    BOOL StopRealPlay();  // 停止预览
	void F_DrawFrame(long lRealHandle, HDC hDc, void* pUser,  COLORREF color);
	void F_DrawPdcRule(LONG lRealHandle, HDC hDc, void* pUser);
    ...
    void F_DrawPolyLine(long lRealHandle, HDC hDc, void* pUser, COLORREF color);
