...
BOOL CALLBACK CertificationCallback(DWORD uiResult, LPNET_DVR_CETTIFICATE_INFO lpCertificateInfo, char *pUserData)
{
    if (uiResult)
    {
        return  TRUE;
...
, m_bPortMultiplex(FALSE)
, m_ullMaxFileLen(0)
, m_bAsyncEnabled(FALSE)
, m_csCertificationPath(_T(""))
, m_bFastUpgrade(FALSE)
, m_csLibcryptoPath(_T(""))
, m_csLibsslPath(_T(""))
{
...
void CDlgLocalConfig::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_PORT_MULTIPLEX, m_bPortMultiplex);
    DDX_Check(pDX, IDC_CHECK_MULTIPLEXING, m_bUsingMultiplexing);
    //此窗口在英文demo中没有，放开会导致提示demo不支持或崩溃，先用宏圈起来，若需要用时，再添加
#if !defined DEMO_LAN_EN
    DDX_Text(pDX, IDC_EDIT2, m_ullMaxFileLen);
#endif
    DDX_Check(pDX, IDC_CHECK_ASYNC_ENABLED, m_bAsyncEnabled);
    DDX_Text(pDX, IDC_EDIT_CERTIFICATION_PATH, m_csCertificationPath);
    DDX_Check(pDX, IDC_CHECK_FASTUPGRADE, m_bFastUpgrade);
    DDX_Text(pDX, IDC_EDIT_LOG_LIBCRYPTO, m_csLibcryptoPath);
    DDX_Text(pDX, IDC_EDIT_LOG_LIBSSL, m_csLibsslPath);
}
...
BEGIN_MESSAGE_MAP(CDlgLocalConfig, CDialog)
    ...
    ON_BN_CLICKED(IDC_BTN_SELECT_CERTIFICATION_PATH, &CDlgLocalConfig::OnBnClickedBtnSelectCertificationPath)
    ON_BN_CLICKED(IDC_BTN_P2P_CFG, &CDlgLocalConfig::OnBnClickedBtnP2pCfg)
    ON_BN_CLICKED(IDC_BTN_SELECT_LOG_LIBCRYPTO, &CDlgLocalConfig::OnBnClickedBtnSelectLogLibcrypto)
    ON_BN_CLICKED(IDC_BTN_SELECT_LOG_LIBSSL, &CDlgLocalConfig::OnBnClickedBtnSelectLogLibssl)
END_MESSAGE_MAP()
...
void CDlgLocalConfig::OnBnClickedLocalOk()
{
    ...
    if (m_csCertificationPath !="")
    {
        NET_DVR_LOCAL_CERTIFICATION struCerTemp = { 0 };
        memcpy(struCerTemp.szLoadPath, m_csCertificationPath, strlen(m_csCertificationPath));
        struCerTemp.fnCB = CertificationCallback;

        NET_DVR_SetSDKLocalCfg(NET_SDK_LOCAL_CFG_CERTIFICATION, &struCerTemp);
    }
    ...
    NET_DVR_SetConnectTime(m_iConnectTimeout, 1);
    ...
    struGeneralCfg.i64FileSize = m_ullMaxFileLen;   // [add] by yangzheng 2019/10/16 test shall delete
    struGeneralCfg.byMultiplexing = m_bUsingMultiplexing;
    struGeneralCfg.byFastUpgrade = m_bFastUpgrade;
    struGeneralCfg.byAlarmPrealloc = 10;
	struGeneralCfg.bChangeEventIPToDomain = 1; //域名登录,布防事件回调转换成域名
    ...
    if (m_csLibcryptoPath.GetLength() > 0)
    {
        //设置openssl路径
        if (!NET_DVR_SetSDKInitCfg(NET_SDK_INIT_CFG_LIBEAY_PATH, (void*)m_csLibcryptoPath.GetBuffer(0)))
        {
            g_pMainDlg->AddLog(0, OPERATION_FAIL_T, "SET NET_SDK_INIT_CFG_LIBEAY_PATH failed");
        }
        else
        {
            g_pMainDlg->AddLog(0, OPERATION_SUCC_T, "SET NET_SDK_INIT_CFG_LIBEAY_PATH successful");
        }
    }
    
    if (m_csLibsslPath.GetLength() > 0)
    {
        //设置openssl路径
        if (!NET_DVR_SetSDKInitCfg(NET_SDK_INIT_CFG_SSLEAY_PATH, (void*)m_csLibsslPath.GetBuffer(0)))
        {
            g_pMainDlg->AddLog(0, OPERATION_FAIL_T, "SET NET_SDK_INIT_CFG_SSLEAY_PATH failed");
        }
        else
        {
            g_pMainDlg->AddLog(0, OPERATION_SUCC_T, "SET NET_SDK_INIT_CFG_SSLEAY_PATH successful");
        }
    }
...
void CDlgLocalConfig::OnButtonSetSdkCfg()
{
    ...
    NET_DVR_LOCAL_PORT_MULTI_CFG struPortMultiParams = { 0 };
    struPortMultiParams.bEnable = m_bPortMultiplex;
    if (!NET_DVR_SetSDKLocalCfg(NET_SDK_LOCAL_CFG_PORT_MULTIPLEX, &struPortMultiParams))
    {
        g_pMainDlg->AddLog(-1, OPERATION_FAIL_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_PORT_MULTIPLEX");
    }
    else
    {
        g_pMainDlg->AddLog(-1, OPERATION_SUCC_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_PORT_MULTIPLEX");
    }

    NET_DVR_LOCAL_ASYNC_CFG struAsyncParams = { 0 };
    struAsyncParams.bEnable = m_bAsyncEnabled;
    if (!NET_DVR_SetSDKLocalCfg(NET_SDK_LOCAL_CFG_ASYNC, &struAsyncParams))
    {
        g_pMainDlg->AddLog(-1, OPERATION_FAIL_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_ASYNC");
    }
    else
    {
        g_pMainDlg->AddLog(-1, OPERATION_SUCC_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_ASYNC");
    }
}
...
void CDlgLocalConfig::OnButtonGetSdkCfg()
{
    ...
    NET_DVR_LOCAL_PORT_MULTI_CFG struPortMultiParams = { 0 };
    if (!NET_DVR_GetSDKLocalCfg(NET_SDK_LOCAL_CFG_PORT_MULTIPLEX, &struPortMultiParams))
    {
        g_pMainDlg->AddLog(-1, OPERATION_FAIL_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_PORT_MULTIPLEX");
    }
    else
    {
        m_bPortMultiplex = struPortMultiParams.bEnable;
        g_pMainDlg->AddLog(-1, OPERATION_SUCC_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_PORT_MULTIPLEX");
    }

    NET_DVR_LOCAL_ASYNC_CFG struAsyncParams = { 0 };
    if (!NET_DVR_GetSDKLocalCfg(NET_SDK_LOCAL_CFG_ASYNC, &struAsyncParams))
    {
        g_pMainDlg->AddLog(-1, OPERATION_FAIL_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_ASYNC");
    }
    else
    {
        m_bAsyncEnabled = struAsyncParams.bEnable;
        g_pMainDlg->AddLog(-1, OPERATION_SUCC_T, "NET_DVR_SetSDKLocalCfg NET_SDK_LOCAL_CFG_ASYNC");
    }
...
void CDlgLocalConfig::OnBnClickedBtnCallback()
{
    ...
    struStreamCallBackCfg.byPlayBackNewPos = m_comboPlayBackEndFlag.GetCurSel();//用同一个控制
...
void CDlgLocalConfig::OnBnClickedBtnSelectCertificationPath()
{
    // TODO:  在此添加控件通知处理程序代码
    CString csPath = GetDirectoryPath();
    if (csPath != "")
    {
        m_csCertificationPath = csPath;
        UpdateData(FALSE);
    }
}
...
void CDlgLocalConfig::OnBnClickedBtnP2pCfg()
{
    // TODO:  在此添加控件通知处理程序代码
    P2PProxyCfgDlg	dlgP2PProxyCfg;
    dlgP2PProxyCfg.DoModal();
}


void CDlgLocalConfig::OnBnClickedBtnSelectLogLibcrypto()
{
    // TODO:  在此添加控件通知处理程序代码
    CString csPath = GetDirectoryPath();
    if (csPath != "")
    {
        m_csLibcryptoPath = csPath;
        UpdateData(FALSE);
    }
}


void CDlgLocalConfig::OnBnClickedBtnSelectLogLibssl()
{
    // TODO:  在此添加控件通知处理程序代码
    CString csPath = GetDirectoryPath();
    if (csPath != "")
    {
        m_csLibsslPath = csPath;
        UpdateData(FALSE);
    }
}
