...
"IP地址冲突", "非法访问", "目标抓拍", "人脸比对", "允许名单人脸比对（陌生人比对）", "目标抓拍",
"车牌识别", "人员密度", "交通事件检测", "目标抓拍建模", "名单车辆报警上传", "热度图报警", "车辆图片二次识别",
...
CAlarmAdvanceSetter::CAlarmAdvanceSetter(CWnd* pParent /*=NULL*/)
    ...
    , m_bAIOPByISAPI(FALSE)
    , m_bVCAByISAPI(FALSE)
    , m_bSmokeByISAPI(FALSE)
    , m_bMoveByISAPI(FALSE)
    , m_bEventAck(FALSE)
    , m_szRetransmissionID(_T(""))
    , m_iEventAckInterval(0)
    , m_iEventAckMode(0)
    , m_iUploadRate(0)
    , m_iMaxUploadRate(0)
    , m_iMinUploadRate(0)
...
void CAlarmAdvanceSetter::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_AIOP_ISAPI, m_bAIOPByISAPI);
    DDX_Check(pDX, IDC_CHECK_CHILD_DEVICE, m_bChildDevice);
    DDX_Check(pDX, IDC_CHECK_VCA_ISAPI, m_bVCAByISAPI);
    DDX_Check(pDX, IDC_CHECK_SMOKE_ISAPI, m_bSmokeByISAPI);
    DDX_Check(pDX, IDC_CHECK_MOVE_ISAPI, m_bMoveByISAPI);
    DDX_Check(pDX, IDC_CHECK_EVENT_ACK, m_bEventAck);
    DDX_Text(pDX, IDC_EDIT_RETRANSMISSION_ID, m_szRetransmissionID);
    DDX_Text(pDX, IDC_EDIT_EVENT_ACK_INTERVAL, m_iEventAckInterval);
    DDX_CBIndex(pDX, IDC_COMBO_EVENT_ACK_MODE, m_iEventAckMode);
    DDX_Text(pDX, IDC_EDIT_UPLOADRATE, m_iUploadRate);
}
...
BEGIN_MESSAGE_MAP(CAlarmAdvanceSetter, CDialogEx)
    ...
    ON_WM_CLOSE()
    ON_CBN_SELCHANGE(IDC_COMBO_EVENT_ACK_MODE, &CAlarmAdvanceSetter::OnCbnSelchangeComboEventAckMode)
    ON_BN_CLICKED(IDC_BUTTON_GET_RETRANSMISSION_ID, &CAlarmAdvanceSetter::OnBnClickedButtonGetRetransmissionId)
    ON_BN_CLICKED(IDC_BUTTON_CANCAEL_RETRANSMISSION_ID, &CAlarmAdvanceSetter::OnBnClickedButtonCancaelRetransmissionId)
    ON_EN_KILLFOCUS(IDC_EDIT_UPLOADRATE, &CAlarmAdvanceSetter::OnEnKillfocusEditUploadrate)
END_MESSAGE_MAP()
...
BOOL CAlarmAdvanceSetter::OnInitDialog()
{
    ...
    //断网重传相关的按钮（有能力判断，所以默认置灰）
    GetDlgItem(IDC_CHECK_EVENT_ACK)->EnableWindow(FALSE);
    GetDlgItem(IDC_EDIT_RETRANSMISSION_ID)->EnableWindow(FALSE);
    GetDlgItem(IDC_BUTTON_GET_RETRANSMISSION_ID)->EnableWindow(FALSE);
    GetDlgItem(IDC_BUTTON_CANCAEL_RETRANSMISSION_ID)->EnableWindow(FALSE);

    /*   GetDlgItem(IDC_COMBO_EVENT_ACK_MODE)->EnableWindow(FALSE);
       GetDlgItem(IDC_EDIT_EVENT_ACK_INTERVAL)->EnableWindow(FALSE);*/
    ...
    if (m_iDeviceIndex != -1)
    {
        ...
        //需要选择对应的设备，否则无法加载界面
        m_iEventAckMode = g_struDeviceInfo[m_iDeviceIndex].iEventAckMode;
        m_iEventAckInterval = g_struDeviceInfo[m_iDeviceIndex].iEventAckInterval;
        CheckRetransmissionAbility();
        OnCbnSelchangeComboEventAckMode();
    }
    else
    {
...
void CAlarmAdvanceSetter::CheckITDeviceAbility()
{
    ...
    for (int iChannelindex = 0; iChannelindex <= g_struDeviceInfo[m_iDeviceIndex].iDeviceChanNum; iChannelindex++) //存在设备0通道
    {
        if (g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[iChannelindex].bEnable == FALSE && 0 != g_struDeviceInfo[m_iDeviceIndex].iDeviceChanNum)
        {
            continue;
        }
...
void CAlarmAdvanceSetter::GenerateAndSetRetransmissionID()
{
    if (m_szRetransmissionID.GetLength() == 0) {
        char szGUID[MAX_XML_ELEM_LEN] = { 0 };

        // 生成GUID
        if (!GenerateGUID(szGUID, MAX_XML_ELEM_LEN)) {
            memcpy(szGUID, "123e4567-e89b-12d3-a456-426614174000", NET_SDK_UUID_LEN);
            szGUID[NET_SDK_UUID_LEN] = '\0';
        }

        // 将前7位替换为"sdkDemo"
        memcpy(szGUID, "SDKDEMO", 7);

        // 赋值给成员变量
        m_szRetransmissionID = szGUID;
    }
}
...
/////////////////////////////////////////////////////////////////////////////
/*********************************************************
Function:  CheckRetransmissionAbility
Desc:      Get the RetransmissionAbility ability 
Input:     none
Output:    none
Return:    none
**********************************************************/
void CAlarmAdvanceSetter::CheckRetransmissionAbility()
{
    //1.查询是否支持防断网链路重传
    ...
    if (NET_DVR_STDXMLConfig(g_struDeviceInfo[m_iDeviceIndex].lLoginID, &xmlConfigInput, &xmlCongfigOutput))
    {
        ...
        {
            if (xmlBase.FindElem("eventAck"))
            {
                GetDlgItem(IDC_CHECK_EVENT_ACK)->EnableWindow(TRUE);
                GetDlgItem(IDC_EDIT_RETRANSMISSION_ID)->EnableWindow(TRUE);
                m_szRetransmissionID = g_struDeviceInfo[m_iDeviceIndex].szRetransmissionID;
                GenerateAndSetRetransmissionID(); //如果还未布防，没有重传ID，先生成一个重传ID，回填到界面上，降低用户的操作复杂度
            ...
            xmlBase.OutOfElem();
            xmlBase.IntoElem();
            if (xmlBase.FindElem("isSupportRetransmissionStatus"))
            {
                CString strValue = xmlBase.GetData().c_str();
                if ("true" == strValue)
                {
                    GetDlgItem(IDC_BUTTON_GET_RETRANSMISSION_ID)->EnableWindow(TRUE);
                    OnBnClickedButtonGetRetransmissionId();
                }
                else
                {
                    GetDlgItem(IDC_BUTTON_GET_RETRANSMISSION_ID)->EnableWindow(FALSE);
            ...
            xmlBase.OutOfElem();
            xmlBase.IntoElem();
            if (xmlBase.FindElem("isSupportCancelRetransmission"))
            {
                CString strValue = xmlBase.GetData().c_str();
                if ("true" == strValue)
                {
                    GetDlgItem(IDC_BUTTON_CANCAEL_RETRANSMISSION_ID)->EnableWindow(TRUE);
                }
                else
                {
                    GetDlgItem(IDC_BUTTON_CANCAEL_RETRANSMISSION_ID)->EnableWindow(FALSE);
            ...
            if (xmlBase.FindElem("uploadRateLimit"))
            {
                GetDlgItem(IDC_EDIT_UPLOADRATE)->EnableWindow(TRUE);
                m_iMaxUploadRate = atoi(xmlBase.GetAttributeValue("max").c_str());
                m_iMinUploadRate = atoi(xmlBase.GetAttributeValue("min").c_str());
                m_iUploadRate = g_struDeviceInfo[m_iDeviceIndex].iEventUploadRate;
                if (m_iUploadRate == 0)
                {
                    m_iUploadRate = atoi(xmlBase.GetAttributeValue("def").c_str());
                }
            }
            else
            {
                GetDlgItem(IDC_EDIT_UPLOADRATE)->EnableWindow(FALSE);
            }
            //TODO(HQH):需要删除
            GetDlgItem(IDC_EDIT_UPLOADRATE)->EnableWindow(TRUE);
        }
...
{
    //查询VcaChanAbility 
    /*人脸报警信息类型-目标抓拍: 1.接口调用成功；2.判断<VcaChanAbility>中的<Face>中的<FaceSnap>，如果节点存在，表示设备支持“目标抓拍”
      目标抓拍续传：1.接口调用成功；2.判断<VcaChanAbility>中的<Face>中的<FaceSnap>中的<brokenNetHttp>，如果节点存在，表示设备支持“目标抓拍断网续传” */
    DWORD dwAbilityType = DEVICE_ABILITY_INFO;
    char pInBuf[512] = "";
    DWORD dwOutputLen = 1024 * 1024;
    char *pOutBuf = new char[dwOutputLen];
...
void CAlarmAdvanceSetter::OnBnClickedButtonConfirm()
{
    ...
    m_pStruSetupAlarmParamV50->byCustomCtrl = 0;
    m_pStruSetupAlarmParamV50->byCustomCtrl |= (m_bCopilotFace << 0);
    m_pStruSetupAlarmParamV50->byCustomCtrl |= (m_bAIOPByISAPI << 1);
    m_pStruSetupAlarmParamV50->byCustomCtrl |= (m_bVCAByISAPI << 2);
    m_pStruSetupAlarmParamV50->byCustomCtrl |= (m_bSmokeByISAPI << 3);
    m_pStruSetupAlarmParamV50->byCustomCtrl |= (m_bMoveByISAPI << 4);
    ...
    m_pStruSetupAlarmParamV50->byChildDeviceFlag = m_bChildDevice;
...
/** @fn void CAlarmAdvanceSetter::GetRetransmissionParam(CString & strData)
*  @brief 获取布防断网链路重传参数
*  @param (in)	CString & strData
*  @return void
*/

void CAlarmAdvanceSetter::GetRetransmissionParam(CString & strData)
{
    strData = _T("");

    if (m_bEventAck)
    {
        GenerateAndSetRetransmissionID(); //如果界面上删除了重传ID，但是ack使能了，则自动生成一个sdkdemo的UUID
        strData.Format("%s%s%s", "<eventAck>true</eventAck>\r\n<retransmissionID>", m_szRetransmissionID, "</retransmissionID>");

        if (m_iUploadRate !=0 )
        {
            strData.AppendFormat(_T("<uploadRateLimit>%d</uploadRateLimit>"), m_iUploadRate);
        }

        g_struDeviceInfo[m_iDeviceIndex].bEventAck = TRUE;
        memcpy(g_struDeviceInfo[m_iDeviceIndex].szRetransmissionID, m_szRetransmissionID, NET_SDK_UUID_LEN + 1);
        AppendRetransID(g_struDeviceInfo[m_iDeviceIndex].szOldRetransIDList, sizeof(g_struDeviceInfo[m_iDeviceIndex].szOldRetransIDList), m_szRetransmissionID);

        g_struDeviceInfo[m_iDeviceIndex].iEventAckMode = m_iEventAckMode;
        g_struDeviceInfo[m_iDeviceIndex].iEventAckInterval = m_iEventAckInterval;
        g_struDeviceInfo[m_iDeviceIndex].iEventUploadRate = m_iUploadRate;
...
*  @return void
*/
void CAlarmAdvanceSetter::AppendRetransID(char* szOldRetransIDList, int nMaxLen, const char* szNewRetransID) {
    if (!szOldRetransIDList || !szNewRetransID || nMaxLen <= 0) {
        return;
    }

    int nCurrentLen = strlen(szOldRetransIDList);
    int nNewIDLen = strlen(szNewRetransID);

    if (nCurrentLen == 0) {
        // 首次赋值
        strncpy(szOldRetransIDList, szNewRetransID, nMaxLen - 1);
    }
    else {
        // 追加新ID（逗号分隔）
        if (nCurrentLen + nNewIDLen + 2 <= nMaxLen) { // +2 for comma and null terminator
            strncat(szOldRetransIDList, ",", nMaxLen - nCurrentLen - 1);
            strncat(szOldRetransIDList, szNewRetransID, nMaxLen - nCurrentLen - 2);
        }
        else {
            // 缓冲区不足，清空后重新赋值
            strncpy(szOldRetransIDList, szNewRetransID, nMaxLen - 1);
    ...
    szOldRetransIDList[nMaxLen - 1] = '\0'; // 确保字符串终止
}
...
void CAlarmAdvanceSetter::GenerateSubcribeParam()
{
    ...
    if (m_cmbAlarmSubscribeInfo.GetCurSel() == 0) //ALL
    {
        ...
        m_strSubscribe += strTemp;
        //重传相关参数
        GetRetransmissionParam(strTemp);
        m_strSubscribe += strTemp;
...
void CAlarmAdvanceSetter::GenerateSubcribeListData()
{
    ...
    if (xmlBase.FindElem("SubscribeEvent") && xmlBase.IntoElem())
    {
        ...
        //eventack和eventMode模式没有关系
        if (xmlBase.FindElem("eventAck") && "true" == xmlBase.GetData())
        {
            m_bEventAck = TRUE;
        }
        else
        ...
        if (xmlBase.FindElem("uploadRateLimit"))
        {
            m_iUploadRate = atoi(xmlBase.GetData().c_str());
        }
...
void CAlarmAdvanceSetter::OnCbnSelchangeComboEventAckMode()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
   
    if (m_iEventAckMode == 1)  // 批量应答模式
    {
        GetDlgItem(IDC_EDIT_EVENT_ACK_INTERVAL)->EnableWindow(TRUE);
    }
    else  // 一对一默认应答模式
    {
        GetDlgItem(IDC_EDIT_EVENT_ACK_INTERVAL)->EnableWindow(FALSE);
...
void CAlarmAdvanceSetter::OnBnClickedButtonGetRetransmissionId()
{
    // 清空之前的数据
    m_retransmissionIDList.RemoveAll();

    // 发送GET请求
    char* strUrl = "GET /ISAPI/Event/notification/RetransmissionStatus?format=json\r\n";
    ...
    if (bResult)
    {
        ...
        {
            // 解析重传状态列表
            cJSON* pRetransmissionStatus = cJSON_GetObjectItem(pRootResp, "retransmissionStatus");
            if (pRetransmissionStatus != NULL)
            {
                int iArraySize = cJSON_GetArraySize(pRetransmissionStatus);

                // 获取下拉框控件
                CComboBox* pCombo = (CComboBox*)GetDlgItem(IDC_COMBO_RETRANSMISSION_ID);
                if (pCombo != NULL)
                {
                    pCombo->ResetContent();

                    // 创建临时集合用于存储接口返回的ID（用于后续比较）
                    CStringArray arrReturnedIDs;

                    // 第一步：收集所有接口返回的ID
                    for (int i = 0; i < iArraySize; i++)
                    {
                        cJSON* pItem = cJSON_GetArrayItem(pRetransmissionStatus, i);
                        if (pItem != NULL)
                        {
                            cJSON* pRetransmissionID = cJSON_GetObjectItem(pItem, "retransmissionID");
                            if (pRetransmissionID != NULL && pRetransmissionID->valuestring != NULL)
                            {
                                arrReturnedIDs.Add(CString(pRetransmissionID->valuestring));
                            }
                        }
                    }

                    // 第二步：处理old列表，移除设备未返回的ID
                    UpdateOldRetransIDList(arrReturnedIDs);
                    ...
                    for (int i = 0; i < iArraySize; i++)
                    {
                        ...
                        {
                            cJSON* pRetransmissionID = cJSON_GetObjectItem(pItem, "retransmissionID");
                            cJSON* pStatusValue = cJSON_GetObjectItem(pItem, "status");

                            if (pRetransmissionID != NULL && pRetransmissionID->valuestring != NULL)
                            {
                                CString strID = pRetransmissionID->valuestring;
                                CString strDisplay;

                                // 检查是否在old列表中存在
                                BOOL bExistsInOld = IsIDInOldList(strID);

                                // 构建显示字符串
                                if (pStatusValue != NULL && pStatusValue->valuestring != NULL)
                                {
                                    if (bExistsInOld)
                                    {
                                        strDisplay.Format(_T("%s (%s) [local]"), strID, pStatusValue->valuestring);
                                    }
                                    else
                                    {
                                        strDisplay.Format(_T("%s (%s)"), strID, pStatusValue->valuestring);
                                    }
                                }
                                else
                                {
                                    if (bExistsInOld)
                                    {
                                        strDisplay.Format(_T("%s [local]"), strID);
                                    }
                                    else
                                    {
                                        strDisplay = strID;
                                ...
                                // 添加到下拉框
                                int nIndex = pCombo->AddString(strDisplay);

                                // 保存到CStringList（自动管理内存）
                                m_retransmissionIDList.AddTail(strID);

                                // 设置项数据为索引值
                                pCombo->SetItemData(nIndex, (DWORD_PTR)(m_retransmissionIDList.GetCount() - 1));
                            }
                        }
                    }
...
void CAlarmAdvanceSetter::OnBnClickedButtonCancaelRetransmissionId()
{
    // TODO:  在此添加控件通知处理程序代码
    // 获取选中的重传ID
    CString strSelectedID = GetSelectedRetransmissionID();
    if (strSelectedID.IsEmpty())
    {
        AfxMessageBox(_T("请先选择一个重传ID"));
        return;
    }
    ...
    // cancelMode - 必需
    cJSON_AddStringToObject(pRoot, "cancelMode", "list");

    // retransmissionIDList - 必需
    cJSON* pIDList = cJSON_CreateArray();
    cJSON_AddItemToArray(pIDList, cJSON_CreateString(CT2A(strSelectedID)));
    cJSON_AddItemToObject(pRoot, "retransmissionIDList", pIDList);
    ...
    char* strUrl = "PUT /ISAPI/Event/notification/CancelRetransmission?format=json\r\n";
    xmlInput.lpRequestUrl = strUrl;
    xmlInput.dwRequestUrlLen = strlen(strUrl);
    ...
    if (bResult)
    {
        ...
        if (pRootResp != NULL)
        {
            ...
            {
                cJSON* pErrorMsg = cJSON_GetObjectItem(pRootResp, "errorMsg");
                CString strError;
                if (pErrorMsg != NULL && pErrorMsg->valuestring != NULL)
                {
                    strError.Format(_T("Error: %s"), pErrorMsg->valuestring);
                }
                else
                {
                    strError = _T("Error: Unknown Error");
                }
        ...
        {
            AfxMessageBox(_T("Error: Failed to parse"));
        }
    }
    else
    {
        ...
        CString strError;
        strError.Format(_T("NET_DVR_STDXMLConfig(%s) Failed"), strUrl);
        AfxMessageBox(strError);
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, strUrl);
    }
...
// 新增辅助函数：检查ID是否在old列表中
BOOL CAlarmAdvanceSetter::IsIDInOldList(const CString& strID)
{
    CString strOldList = g_struDeviceInfo[m_iDeviceIndex].szOldRetransIDList;

    if (strOldList.IsEmpty())
    {
        return FALSE;
    }

    // 将逗号分隔的字符串拆分为数组
    CStringArray arrOldIDs;
    SplitString(strOldList, _T(","), arrOldIDs);

    for (int i = 0; i < arrOldIDs.GetSize(); i++)
    {
        if (arrOldIDs[i].Trim().Compare(strID) == 0)
        {
            return TRUE;
        }
    }
...
// 新增辅助函数：更新old列表，移除设备未返回的ID
void CAlarmAdvanceSetter::UpdateOldRetransIDList(const CStringArray& arrReturnedIDs)
{
    CString strOldList = g_struDeviceInfo[m_iDeviceIndex].szOldRetransIDList;
    ...
    // 构建新的old列表（只保留设备返回的ID）
    CString strNewOldList;
    for (int i = 0; i < arrOldIDs.GetSize(); i++)
    {
        CString strOldID = arrOldIDs[i].Trim();

        // 检查该ID是否在设备返回的列表中
        BOOL bExistsInReturned = FALSE;
        for (int j = 0; j < arrReturnedIDs.GetSize(); j++)
        {
            if (strOldID.Compare(arrReturnedIDs[j]) == 0)
            {
                bExistsInReturned = TRUE;
                break;
            }
        }

        if (bExistsInReturned)
        {
            if (!strNewOldList.IsEmpty())
            {
                strNewOldList += _T(",");
            }
            strNewOldList += strOldID;
        }
    }

    // 更新全局变量
    _tcsncpy(g_struDeviceInfo[m_iDeviceIndex].szOldRetransIDList,
        strNewOldList,
        MAX_OLD_RETRANS_ID_LEN - 1);
    g_struDeviceInfo[m_iDeviceIndex].szOldRetransIDList[MAX_OLD_RETRANS_ID_LEN - 1] = _T('\0');
}

// 字符串分割函数（如果类中不存在需要添加）
void CAlarmAdvanceSetter::SplitString(const CString& strSource, const CString& strDelim, CStringArray& arrResult)
{
    arrResult.RemoveAll();

    int iStart = 0;
    int iEnd = strSource.Find(strDelim);

    while (iEnd != -1)
    {
        CString strToken = strSource.Mid(iStart, iEnd - iStart);
        if (!strToken.IsEmpty())
        {
            arrResult.Add(strToken.Trim());
        }
        iStart = iEnd + strDelim.GetLength();
        iEnd = strSource.Find(strDelim, iStart);
    }

    // 添加最后一个token
    if (iStart < strSource.GetLength())
    {
        CString strToken = strSource.Mid(iStart);
        if (!strToken.IsEmpty())
        {
            arrResult.Add(strToken.Trim());
        }
...
// 辅助函数：获取当前选中的重传ID
CString CAlarmAdvanceSetter::GetSelectedRetransmissionID()
{
    CComboBox* pCombo = (CComboBox*)GetDlgItem(IDC_COMBO_RETRANSMISSION_ID);
    if (pCombo != NULL)
    {
        ...
        if (nCurSel != CB_ERR)
        {
            DWORD_PTR dwIndex = pCombo->GetItemData(nCurSel);
            if (dwIndex < (DWORD_PTR)m_retransmissionIDList.GetCount())
            {
                POSITION pos = m_retransmissionIDList.FindIndex((INT_PTR)dwIndex);
                if (pos != NULL)
                {
                    return m_retransmissionIDList.GetAt(pos);
                }
            }
        }
    }
...
void CAlarmAdvanceSetter::OnEnKillfocusEditUploadrate()
{
    CString strText;
    int nNewVal = 0;
    ...
    // 1. 直接从界面获取最新输入的文本
    GetDlgItemText(IDC_EDIT_UPLOADRATE, strText);

    // 2. 处理空字符串情况
    if (strText.IsEmpty())
    {
        // 策略：如果允许为空，可以设为 0 或者保留空（视业务而定）
        // 这里假设空值视为 0
        nNewVal = 0;
    ...
    {
        // 3. 尝试将字符串转换为整数
        // 使用 try-catch 防止非数字字符导致崩溃（虽然 OnChange 通常已拦截，但双重保险更好）
        try
        {
            // _tstoi 在遇到非数字字符时会停止转换，如果第一个字符就不是数字会抛异常或返回 0
            // 更严谨的做法是检查整个字符串是否都是数字，这里简化处理
            TCHAR* endPtr = nullptr;
            long lVal = _tcstol(strText, &endPtr, 10);

            // 检查是否完全转换成功（endPtr 应该指向字符串结尾）
            if (*endPtr == _T('\0') && strText.GetLength() > 0)
            {
                nNewVal = (int)lVal;
                bConvertSuccess = true;
            }
    ...
    // 4. 范围校验
    if (nNewVal < m_iMinUploadRate || nNewVal > m_iMaxUploadRate)
    {
        char szLanTemplate[256] = { 0 };
        g_StringLanType(szLanTemplate, "输入值 %d 超出范围 (%d-%d)，已自动修正为边界值", 
            "The input value %d is out of range (%d-%d) and has been automatically corrected to the boundary limit.");

        CString strMsg;
        strMsg.Format(szLanTemplate, nNewVal, m_iMinUploadRate, m_iMaxUploadRate);

        // 6. 弹出提示
        AfxMessageBox(strMsg, MB_ICONEXCLAMATION | MB_OK);

        // 7. 自动修正为边界值
        if (nNewVal < m_iMinUploadRate)
        {
            nNewVal = m_iMinUploadRate;
        }
        else if (nNewVal > m_iMaxUploadRate)
        {
            nNewVal = m_iMaxUploadRate;
        }

        // 8. 更新成员变量
        m_iUploadRate = nNewVal;

        // 9. 将修正后的值回写到界面控件
        CString strCorrected;
        strCorrected.Format(_T("%d"), m_iUploadRate);
        SetDlgItemText(IDC_EDIT_UPLOADRATE, strCorrected);
    }
    else
    {
        // 10. 校验通过，直接更新成员变量
        // 此时界面显示的值就是用户输入的值，无需 SetDlgItemText
        m_iUploadRate = nNewVal;
    }
}
