...
void CALLBACK g_DrawLane(LONG lRealHandle, HDC hDc, void* pUser)
{
//     SetBkMode(hDc, TRANSPARENT);
    //    if(g_pDlgVcaRuleCfg->m_bRuleActive)
    {
        g_pDlgVcaLancCfg->F_DrawFun(lRealHandle, hDc, pUser); 
    }
...
void CDlgVcaLaneCfg::F_DrawFun(long lPlayHandle, HDC hDc, void* pUser)
{
    F_DrawFrame(lPlayHandle, hDc, pUser,  RGB(255, 255, 0)); 
    
}

void CDlgVcaLaneCfg::F_DrawFrame(long lRealHandle, HDC hDc, void* pUser,  COLORREF color)
{
...
LONG CDlgVcaLaneCfg::StartPlay()
{
    if (GetPuStreamCfg(m_struPUStream))
    {
        ...
        else
        {
            ...
            bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, g_DrawLane, NULL);//DC Draw callback
        }
