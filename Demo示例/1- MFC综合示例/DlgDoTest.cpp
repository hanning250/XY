...
BEGIN_MESSAGE_MAP(CDlgDoTest, CDialog)
    ...
    ON_WM_DESTROY()
    ON_BN_CLICKED(IDC_BTN_STARTVIDEOAUDIO, &CDlgDoTest::OnBnClickedBtnStartvideoaudio)
END_MESSAGE_MAP()
...
BOOL CDlgDoTest::OnInitDialog()
{
    ...
    m_comboFileType.InsertString(2, "ES");
	if (g_bInSide)
	{
...
//	int i=0;
	...
	DWORD dwReturned;
    //char szTmp[64] = { 0 };
    m_lChannel = g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[g_pMainDlg->GetCurChanIndex()].iChannelNO;
    bRet = NET_DVR_GetDVRConfig(g_struDeviceInfo[m_iDeviceIndex].lLoginID, NET_DVR_GET_PTZPOS, m_lChannel, &m_ptzPos, sizeof(NET_DVR_PTZPOS), &dwReturned);
    if (!bRet)
    {
    ...
    UpdateData(FALSE);
    //bRet = NET_DVR_TransPTZ_Other(g_struDeviceInfo[m_iDeviceIndex].lLoginID, 1, "B5001400000000C9", 8);
    /*bRet = NET_DVR_TransPTZ_Other(g_struDeviceInfo[m_iDeviceIndex].lLoginID, 1, "B5001400000000C9", strlen("B5001400000000C9"));
    if (!bRet)
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_TransPTZ_Other");
        return;
    }
    g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_TransPTZ_Other");*/
...
void CDlgDoTest::OnBtn3() 
{
    ...
    m_lChannel = g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[g_pMainDlg->GetCurChanIndex()].iChannelNO;
    bRet = NET_DVR_SetDVRConfig(g_struDeviceInfo[m_iDeviceIndex].lLoginID, NET_DVR_SET_PTZPOS, m_lChannel, &m_ptzPos, sizeof(NET_DVR_PTZPOS));
	if (!bRet)
	{
...
int CALLBACK Demo_CharEncodeConvert(char *pInput, DWORD dwInputLen, DWORD dwInEncodeType, char *pOutput, DWORD dwOutputLen, DWORD dwOutEncodeType)
{
    ...
    g_pMainDlg->AddLog(-1, OPERATION_SUCC_T, "Demo_CharEncodeConvert recv len[%d] dwInEncodeType[%d] dwOutEncodeType[%d]", dwInputLen, dwInEncodeType, dwOutEncodeType);
...
void CDlgDoTest::OnBnClickedBtnFiletypeOk()
{
    ...
    else if (m_comboFileType.GetCurSel() == 2)
    {
        g_bESCB = TRUE;
    }
...
void CDlgDoTest::OnBnClickedBtnStartvideoaudio()
{
    // TODO:  在此添加控件通知处理程序代码
    CDlgNVRAudioVideoCfg dlg;
