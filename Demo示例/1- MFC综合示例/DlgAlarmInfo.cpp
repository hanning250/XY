...
void CDlgAlarmInfo::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMBO_SUB_EVENT_TYPE, m_comboSubEventType);
}
...
BOOL CDlgAlarmInfo::OnInitDialog()
{
    ...
    m_comboSubEventType.SetCurSel(0);
    char szLan[128] = { 0 };
    if (m_lHandle >= 0)
    {
        g_StringLanType(szLan, "停止", "STOP");
        GetDlgItem(IDC_BUTTON_ALARM_INFO_GET)->SetWindowText(szLan);
    }
    else
    {
...
void CDlgAlarmInfo::OnBnClickedButtonAlarmInfoGet()
{
    ...
    m_struAlarmSearchCond.wEventType = m_comboEventType.GetCurSel();
    m_struAlarmSearchCond.wSubEventType = m_comboSubEventType.GetCurSel();
    m_struAlarmSearchCond.bySupport |= 0x1;//默认写死支持
    m_struAlarmSearchCond.byNoBoundary = 1; //去除boundary字段
    ...
    else
    {
        ...
        if (m_lHandle >= 0)
        {
            ...
            AfxMessageBox(szLan);
            g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_GET_ALARM_INFO");
...
void CDlgAlarmInfo::AddResolution()
{
    ...
    //m_cmbCommand.InsertString(index, "人脸采集结果上传");
...
void CDlgAlarmInfo::OnDestroy()
{
    g_pMainDlg->m_lAlarmInfoHandle = m_lHandle;
    CDialog::OnDestroy();
