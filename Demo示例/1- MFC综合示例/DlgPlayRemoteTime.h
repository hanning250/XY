...
public:
	...
	//}}AFX_DATA
	BOOL	m_PlayConvert;
	BOOL    m_bCallBack;     // 按时间回放回调方式开关
    BOOL    m_bNewPos;       // 是否等待新码流标记开关
    BOOL    m_bWaitNewPos;   // 是否需要等待新码流标记（用来在码流标记前丢数据）
    BOOL    m_bNewCmd;       // 是否使用新命令
    NET_DVR_LOCAL_STREAM_CALLBACK_CFG struStreamCallBackCfg;
    BOOL    m_bChangingSeq;  // 是否在正倒放切换
    VOD_SEQUENCE    m_emDstSequence; // 正倒放切换方向  1：正放 2：倒放
    BOOL    m_bDrawFrameStatus; // 是否抽帧
    BOOL    m_bPosChange;       // 是否在时间定位
    BOOL    m_bPlayStop;        // 是否停止回放
...
public:
    ...
    CString m_strDoubleVerificationUsername;
    CString m_strDoubleVerificationPassword;
    BOOL m_bRemoteFile;
    // 是否下载保存为MP4格式文件
    BOOL m_bSaveMp4;
    BOOL m_bHLS;
