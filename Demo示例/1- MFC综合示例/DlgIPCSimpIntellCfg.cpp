...
void CALLBACK DrawVideoEpoliceAlertShow(LONG lRealHandle, HDC hDc, void* pUser)
{
    SetBkMode(hDc, TRANSPARENT);
    SetTextColor(hDc, RGB(255, 255, 255));

	CDlgIPCSimpIntellCfg *lp = (CDlgIPCSimpIntellCfg *)pUser;
    lp->F_DrawFun(lRealHandle, hDc, pUser);     
}
...
CDlgIPCSimpIntellCfg::CDlgIPCSimpIntellCfg(CWnd* pParent /*=NULL*/)
    ...
    , m_iLinkageTime(0)
    , m_iFilterTime(0)
    , m_bChkWhiteLight(FALSE)
    , m_bTraverseDetTargetOthers(FALSE)
    , m_bIntrusionDetTargetOthers(FALSE)
    , m_bEnterDetTargetAll(FALSE)
    , m_bEnterDetTargetHuman(FALSE)
    , m_bEnterDetTargetVehicle(FALSE)
    , m_bEnterDetTargetOthers(FALSE)
    , m_bExitDetTargetAll(FALSE)
    , m_bExitDetTargetHuman(FALSE)
    , m_bExitDetTargetVehicle(FALSE)
    , m_bExitDetTargetOthers(FALSE)
    , m_iAudioTypeLineDetection(0)
    , m_iTimeLineDetection(0)
    , m_iBrightnessFieldDetection(0)
    , m_iAudioTypeFieldDetection(0)
    , m_iTimeFieldDetection(0)
    , m_bAudioAlarm(FALSE)
    , m_enterAlaConfidence(0)
    , m_enterRecConfidence(0)
    , m_iAlarmConfidence(0)
    , m_iRecordConfidence(0)
    , m_iExitAlaConfidence(0)
    , m_iExitRecConfidence(0)
    , m_iIntrAlaConfidence(0)
    , m_iIntrRecConfidence(0)
    , m_bChkIndicatorLightAlarm(FALSE)
    {
...
void CDlgIPCSimpIntellCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHK_WHITE_LIGHT, m_bChkWhiteLight);
    DDX_Check(pDX, IDC_CHECK_TRAVERSE_DETTARGET_OTHERS, m_bTraverseDetTargetOthers);
    DDX_Check(pDX, IDC_CHECK_INTRUSION_DETTARGET_OTHERS, m_bIntrusionDetTargetOthers);
    DDX_Check(pDX, IDC_CHECK_ENTER_DETTARGET_ALL, m_bEnterDetTargetAll);
    DDX_Check(pDX, IDC_CHECK_ENTER_DETTARGET_HUMAN, m_bEnterDetTargetHuman);
    DDX_Check(pDX, IDC_CHECK_ENTER_DETTARGET_VEHICLE, m_bEnterDetTargetVehicle);
    DDX_Check(pDX, IDC_CHECK_ENTER_DETTARGET_OTHERS, m_bEnterDetTargetOthers);
    DDX_Check(pDX, IDC_CHECK_EXIT_DETTARGET_ALL, m_bExitDetTargetAll);
    DDX_Check(pDX, IDC_CHECK_EXIT_DETTARGET_HUMAN, m_bExitDetTargetHuman);
    DDX_Check(pDX, IDC_CHECK_EXIT_DETTARGET_VEHICLE, m_bExitDetTargetVehicle);
    DDX_Check(pDX, IDC_CHECK_EXIT_DETTARGET_OTHERS, m_bExitDetTargetOthers);
    DDX_Check(pDX, IDC_EDIT_BRIGHTNESS_LINE_DETECTION, m_iBrightnessLineDetection);
    DDX_Text(pDX, IDC_EDIT_AUDIO_TYPE_LINE_DETECTION, m_iAudioTypeLineDetection);
    DDX_Text(pDX, IDC_EDIT_TIMES_LINE_DETECTION, m_iTimeLineDetection);
    DDX_Text(pDX, IDC_EDIT_BRIGHTNESS_FIELD_DETECTION, m_iBrightnessFieldDetection);
    DDX_Text(pDX, IDC_EDIT_AUDIO_TYPE_FIELD_DETECTION, m_iAudioTypeFieldDetection);
    DDX_Text(pDX, IDC_EDIT_TIMES_FIELD_DETECTION, m_iTimeFieldDetection);
    DDX_Check(pDX, IDC_CHK_AUDIO, m_bAudioAlarm);
    DDX_CBIndex(pDX, IDC_COMBO_ENTERALARM_CONFIDENCE, m_enterAlaConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_ENTERECORD_CONFIDENCE, m_enterRecConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_ALARM_CONFIDENCE, m_iAlarmConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_RECORD_CONFIDENCE, m_iRecordConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_EXITALARM_CONFIDENCE, m_iExitAlaConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_EXITRECORD_CONFIDENCE, m_iExitRecConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_INTRALARM_CONFDENCE, m_iIntrAlaConfidence);
    DDX_CBIndex(pDX, IDC_COMBO_INTRRECORD_CONFDENCE, m_iIntrRecConfidence);
    DDX_Check(pDX, IDC_CHK_INDICATOR_LIGHT, m_bChkIndicatorLightAlarm);
}
...
DWORD  fRegisterDrawFun(LPVOID pParam)
{
    ...
    while (1)
    {
        if (!NET_DVR_RigisterDrawFunEx(pIPCSimpInterllCfg->m_lPlayHandle,  DrawVideoEpoliceAlertShow , pParam)) 
        {
            Sleep(1000);            
            continue;       
        }
...
// 	}
	...
	if (m_lPlayHandle >= 0)
	{
        NET_DVR_RigisterDrawFunEx(m_lPlayHandle,  NULL ,0);        
...
//     while (1)
//     {
//         if (!NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawVideoEpoliceAlertShow, this)) 
//         {
//             continue;
//         }
//         else
...
// 	NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawRoiDetectShow, NULL);
...
void CDlgIPCSimpIntellCfg::ShowData()
{
    ...
    m_bChkWhiteLight = (m_pStruAlarmHandleType->dwHandleType >> 14) & 0x01;
    m_bAudioAlarm = (m_pStruAlarmHandleType->dwHandleType >> 15) & 0x01;
    m_bChkIndicatorLightAlarm = (m_pStruAlarmHandleType->dwHandleType >> 17) & 0x01;
...
//	m_iCross = m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].dwCrossDirection;
    ...
    m_iAlarmConfidence = (int)m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].byAlarmConfidence;
    m_iRecordConfidence = (int)m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].byRecordConfidence;
	//区域入侵
...
//	m_iFieldDuration = m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].wDuration;
    ...
    m_iIntrAlaConfidence = (int)m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].byAlarmConfidence;
    m_iIntrRecConfidence = (int)m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].byRecordConfidence;
    m_bIntrusionDetTargetAll = FALSE;
    m_bIntrusionDetTargetHuman = FALSE;
    m_bIntrusionDetTargetVerhicle = FALSE;
    m_bIntrusionDetTargetOthers= FALSE;
    ...
    else
    {
        ...
        m_bIntrusionDetTargetOthers = (m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].byDetectionTarget >> 2) & 1;
    }
    m_bTraverseDetTargetAll = FALSE;
    m_bTraverseDetTargetHuman = FALSE;
    m_bTraverseDetTargetVerhicle = FALSE;
    ...
    else
    {
        ...
        m_bTraverseDetTargetOthers = (m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].byDetectionTarget >> 2) & 1;
    }
...
// 		}
        ...
        m_bIntrusionWhiteLightDurationTime = m_struLineDetection.struHandleException.byWhiteLightDurationTime;
        m_iBrightnessLineDetection = m_struLineDetection.struHandleException.byBrightness;
        m_iAudioTypeLineDetection = m_struLineDetection.struHandleException.byAudioType;
        m_iTimeLineDetection = m_struLineDetection.struHandleException.byTimes;
	}
	else if(i == 1) //当前配置区域入侵侦测
	{
        ...
        m_bTraverseWhiteLightDurationTime = m_struFieldDetection.struHandleException.byWhiteLightDurationTime;
        m_iBrightnessFieldDetection = m_struFieldDetection.struHandleException.byBrightness;
        m_iAudioTypeFieldDetection = m_struFieldDetection.struHandleException.byAudioType;
        m_iTimeFieldDetection = m_struFieldDetection.struHandleException.byTimes;
	}
    ...
    else if (i == 5)
    {
        ...
        m_enterAlaConfidence = m_struRegionEntrDetection.struRegion[j].byAlarmConfidence;
        m_enterRecConfidence = m_struRegionEntrDetection.struRegion[j].byRecordConfidence;
        UpdateData(FALSE);
    ...
    else if (i == 6)
    {
        ...
        m_iExitAlaConfidence = m_struRegionExitDetection.struRegion[j].byAlarmConfidence;
        m_iExitRecConfidence = m_struRegionExitDetection.struRegion[j].byRecordConfidence;
        UpdateData(FALSE);
    ...
    {
        if(GetEventTrigger(NET_DVR_GET_VEHICLE_BLOCKLIST_EVENT_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_BLOCKLST_EVENT_TRIGGER");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_BLOCKLST_EVENT_TRIGGER");
        }
        if(GetEventSchedule(NET_DVR_GET_VEHICLE_BLOCKLIST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_BLOCKLST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_BLOCKLST_SCHEDULE");
        }
    }    
    else if (i == 17)
    {
        if(GetEventTrigger(NET_DVR_GET_VEHICLE_ALLOWLIST_EVENT_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_ALLOWLIST_EVENT_TRIGGER");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_ALLOWLIST_EVENT_TRIGGER");
        }
        if(GetEventSchedule(NET_DVR_GET_VEHICLE_ALLOWLIST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_ALLOWLIST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_ALLOWLIST_SCHEDULE");
        }
    }
    else if (i == 18)
    {
...
//         }
    ...
    {
        //禁止名单人脸比对
        if (GetEventTrigger(NET_DVR_GET_BLOCKLIST_FACECONTRAST_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_BLOCKLIST_FACECONTRAST_TRIGGER");
        }
        else
        {

            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_BLOCKLIST_FACECONTRAST_TRIGGER");
        }
        if (GetEventSchedule(NET_DVR_GET_BLOCKLIST_FACECONTRAST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_BLOCKLIST_FACECONTRAST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_BLOCKLIST_FACECONTRAST_SCHEDULE");
        }
    }
    else if (i == 29)
    {
        //允许名单人脸比对
        if (GetEventTrigger(NET_DVR_GET_ALLOWLIST_FACECONTRAST_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_ALLOWLIST_FACECONTRAST_TRIGGER");
        }
        else
        {

            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_ALLOWLIST_FACECONTRAST_TRIGGER");
        }
        if (GetEventSchedule(NET_DVR_GET_ALLOWLIST_FACECONTRAST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_ALLOWLIST_FACECONTRAST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_ALLOWLIST_FACECONTRAST_SCHEDULE");
        }
    }
    else if (i == 30)
    {
...
void CDlgIPCSimpIntellCfg::OnBtnSavecfg() 
{
	...
	if (i == 0)
	{
        ...
        m_struLineDetection.struHandleException.byWhiteLightDurationTime = m_bIntrusionWhiteLightDurationTime;
        m_struLineDetection.struHandleException.byBrightness = m_iBrightnessLineDetection;
        m_struLineDetection.struHandleException.byAudioType = m_iAudioTypeLineDetection;
        m_struLineDetection.struHandleException.byTimes = m_iTimeLineDetection;
		SaveLineDetection();
	}
	else if (i == 1)
	{
        ...
        m_struFieldDetection.struHandleException.byWhiteLightDurationTime = m_bTraverseWhiteLightDurationTime;
        m_struFieldDetection.struHandleException.byBrightness = m_iBrightnessFieldDetection;
        m_struFieldDetection.struHandleException.byAudioType = m_iAudioTypeFieldDetection;
        m_struFieldDetection.struHandleException.byTimes = m_iTimeFieldDetection;
		SaveFieldDetection();
	}
	else if (i == 2)
	{
    ...
    {
        if(SetEventTrigger(NET_DVR_SET_VEHICLE_BLOCKLIST_EVENT_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_BLOCKLIST_EVENT_TRIGGER");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_BLOCKLIST_EVENT_TRIGGER");
        }
        if(SetEventSchedule(NET_DVR_SET_VEHICLE_BLOCKLIST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_BLOCKLIST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_BLOCKLIST_SCHEDULE");
        }
    }
    else if (i == 17)
    {
        if(SetEventTrigger(NET_DVR_SET_VEHICLE_ALLOWLIST_EVENT_TRIGGER))
        {
        ...
        if(SetEventSchedule(NET_DVR_SET_VEHICLE_ALLOWLIST_SCHEDULE))
        {
...
//         }
    ...
    {
        //禁止名单人脸比对
        if (SetEventTrigger(NET_DVR_SET_BLOCKLIST_FACECONTRAST_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_BLOCKLIST_FACECONTRAST_TRIGGER");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_BLOCKLIST_FACECONTRAST_TRIGGER");
        }
        if (SetEventSchedule(NET_DVR_SET_BLOCKLIST_FACECONTRAST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_BLOCKLIST_FACECONTRAST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_BLOCKLIST_FACECONTRAST_SCHEDULE");
        }
    }
    else if (i == 29)
    {
        //允许名单人脸比对
        if (SetEventTrigger(NET_DVR_SET_ALLOWLIST_FACECONTRAST_TRIGGER))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_ALLOWLIST_FACECONTRAST_TRIGGER");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_ALLOWLIST_FACECONTRAST_TRIGGER");
        }
        if (SetEventSchedule(NET_DVR_SET_ALLOWLIST_FACECONTRAST_SCHEDULE))
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_ALLOWLIST_FACECONTRAST_SCHEDULE");
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_ALLOWLIST_FACECONTRAST_SCHEDULE");
        }
    }
    else if (i == 30)
    {
...
// 	}
    ...
    m_pStruAlarmHandleType->dwHandleType |= (m_bChkWhiteLight << 14);
    m_pStruAlarmHandleType->dwHandleType |= (m_bAudioAlarm << 15);
    m_pStruAlarmHandleType->dwHandleType |= (m_bChkIndicatorLightAlarm << 17);
...
// 	}
    ...
    if (m_bTraverseDetTargetOthers)
    {
        m_bTraverseDetTarget |= (1 << 2);
    }
    ...
    if (m_bIntrusionDetTargetOthers)
    {
        m_bIntrusionDetTarget |= (1 << 2);
    }
...
//	if (m_Combo_CfgType.GetCurSel() == 0)
	{
        ...
        m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].byDetectionTarget = m_bTraverseDetTarget;
        m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].byAlarmConfidence = m_iAlarmConfidence;
        m_struLineDetection.struAlertParam[m_Combo_LineNo.GetCurSel()].byRecordConfidence = m_iRecordConfidence;
...
void CDlgIPCSimpIntellCfg::OnFieldNoChanged()
{
	//if (m_Combo_CfgType.GetCurSel() == 1)
	{
        ...
        m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].byDetectionTarget = m_bIntrusionDetTarget;
        m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].byAlarmConfidence = m_iIntrAlaConfidence;
        m_struFieldDetection.struIntrusion[m_Comnbo_FieldNo.GetCurSel()].byRecordConfidence = m_iIntrRecConfidence;
...
void CDlgIPCSimpIntellCfg::F_DrawFun( long lPlayHandle, HDC hDc, void* pUser )
{
	if (m_Combo_CfgType.GetCurSel() == 0)
	{
		F_DrawLine(hDc);
...
void CDlgIPCSimpIntellCfg::OnBtnRegionEntrRefresh() 
{
    ...
    m_bEnterDetTargetOthers = false;
    m_bEnterDetTargetAll = false;
    if (!m_struRegionEntrDetection.struRegion[i].byDetectionTarget)
    {
        m_bEnterDetTargetAll = true;
    }
    else
    {
        m_bEnterDetTargetHuman = m_struRegionEntrDetection.struRegion[i].byDetectionTarget & 1;
        m_bEnterDetTargetVehicle = (m_struRegionEntrDetection.struRegion[i].byDetectionTarget >> 1) & 1;
        m_bEnterDetTargetOthers = (m_struRegionEntrDetection.struRegion[i].byDetectionTarget >> 2) & 1;
    }
    m_enterAlaConfidence = m_struRegionEntrDetection.struRegion[i].byAlarmConfidence;
    m_enterRecConfidence = m_struRegionEntrDetection.struRegion[i].byRecordConfidence;
...
void CDlgIPCSimpIntellCfg::OnBtnRegionEntrSave() 
{
    ...
    m_struRegionEntrDetection.struRegion[i].byAlarmConfidence = m_enterAlaConfidence;
    m_struRegionEntrDetection.struRegion[i].byRecordConfidence = m_enterRecConfidence;
    //if (!m_struRegionEntrDetection.struRegion[i].byDetectionTarget)
    //{
    //    m_bEnterDetTargetAll = true;
    //}
    ...
    //}
    m_bEnterDetTarget = 1;
    if (!m_bEnterDetTargetHuman)
    {
        m_bEnterDetTarget &= 0;
    }
    if (m_bEnterDetTargetVehicle)
    {
        m_bEnterDetTarget |= (1 << 1);
    }
    if (m_bEnterDetTargetOthers)
    {
        m_bEnterDetTarget |= (1 << 2);
    }
    if (m_bEnterDetTargetAll)
    {
        m_bEnterDetTarget = 0;
    }
    m_struRegionEntrDetection.struRegion[i].byDetectionTarget = m_bEnterDetTarget;
...
void CDlgIPCSimpIntellCfg::OnBtnRegionExitRefresh() 
{
   ...
   // m_lChannel = m_cmbChannel.GetItemData(m_cmbChannel.GetCurSel());
    ...
    m_iExitAlaConfidence = m_struRegionExitDetection.struRegion[i].byAlarmConfidence;
    m_iExitRecConfidence = m_struRegionExitDetection.struRegion[i].byRecordConfidence;
    m_bExitDetTargetAll = false;
    m_bExitDetTargetHuman = false;
    m_bExitDetTargetVehicle = false;
    m_bExitDetTargetOthers = false;
    if (!m_struRegionExitDetection.struRegion[i].byDetectionTarget)
    {
        m_bExitDetTargetAll = true;
    }
    else
    {
        m_bExitDetTargetHuman = m_struRegionExitDetection.struRegion[i].byDetectionTarget & 1;
        m_bExitDetTargetVehicle = (m_struRegionExitDetection.struRegion[i].byDetectionTarget >> 1) & 1;
        m_bExitDetTargetOthers = (m_struRegionExitDetection.struRegion[i].byDetectionTarget >> 2) & 1;
    }
...
void CDlgIPCSimpIntellCfg::OnBtnRegionExitSave() 
{
   ...
   // m_lChannel = m_cmbChannel.GetItemData(m_cmbChannel.GetCurSel());
    ...
    m_struRegionExitDetection.struRegion[i].byAlarmConfidence = m_iExitAlaConfidence;
    m_struRegionExitDetection.struRegion[i].byRecordConfidence = m_iExitRecConfidence;
    m_bExitDetTarget = 1;
    if (!m_bExitDetTargetHuman)
    {
        m_bExitDetTarget &= 0;
    }
    if (m_bExitDetTargetVehicle)
    {
        m_bExitDetTarget |= (1 << 1);
    }
    if (m_bExitDetTargetOthers)
    {
        m_bExitDetTarget |= (1 << 2);
    }
    if (m_bExitDetTargetAll)
    {
        m_bExitDetTarget = 0;
    }
    m_struRegionExitDetection.struRegion[i].byDetectionTarget = m_bExitDetTarget;
