...
void CALLBACK AreaMaskGetCallBack(LONG nPort,HDC hDc,void* pUser)
{
	CRect rect;
	CPoint point;
...
void CALLBACK AreaMaskSetCallBack(LONG nPort,HDC hDc,void* pUser)
{
...
void CDlgAreaMaskCfg::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if (m_bDrawdetect)
	{
		...
		{			
			NET_DVR_RigisterDrawFunEx(m_lPlayHandle, AreaMaskSetCallBack, NULL);
...
void CDlgAreaMaskCfg::OnBnClickedChkMaskShow()
{
	...
	if (m_bChkMaskShow)
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
		Sleep(200);
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,AreaMaskGetCallBack, NULL);
	}
	else
	{
		NET_DVR_RigisterDrawFunEx(m_lPlayHandle,NULL, 0);
