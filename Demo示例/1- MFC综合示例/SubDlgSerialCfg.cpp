...
/*********************************************************
  ...
  Input:	lSerialHandle， serial port handle
			...
			pUser，relative function set by the baclback；
  Output:	none
  Return:	none
**********************************************************/
void CALLBACK fSerialDataCallBack(LONG lSerialHandle,char *pRecvDataBuffer,DWORD dwBufSize,void* pUser)
{
	char pRecv[2048] = {0}; 
	CString csTemp;
	DWORD i = 0;
    ...
    else
    {
        for (i=0; i <g_struDeviceInfo[m_iDeviceIndex].iDeviceChanNum ; i++)//PTZ parameter settings only valid for analog channels
        {
            ...
            if (g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].bEnable)
            {
                ...
                if (!NET_DVR_GetDVRConfig(m_lLoginID, NET_DVR_GET_DECODERCFG_V40, iChanShow, &m_pStru485DecoderCfg[i], sizeof(NET_DVR_DECODERCFG_V40), &dwReturned))
                {		
