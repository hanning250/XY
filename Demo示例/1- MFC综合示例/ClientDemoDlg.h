...
typedef struct tagLOCAL_ALARM_HEAD
{
    DWORD       dwSize;
    BYTE        byRetransAlarm; //是否是重传报警
    BYTE        byRes[3];
    NET_DVR_RETRANSMISSION_HEAD struRetransHead;
    BYTE        byChildDevAlarm; //是否是子设备报警
    BYTE        byRes1[3];
    NET_DVR_CHILD_DEVICE_HEAD struChildDevHead;
} LOCAL_ALARM_HEAD;
...
void AddAlarmHeadLog(LONG lCommand, NET_DVR_ALARMER *pAlarmer, char *pAlarmInfo, UINT iDeviceIndex);
...
public:
    ...
    afx_msg void OnBnClickedBtnEventReconciliation();
...
// 	3.configuration buttons
    ...
    CDlgEventReconciliation* m_dlgEventReconciliation;
    ...
    //组装报警日志
    void MergeAlarmMsg(char *pMergeBuf, int iMergeBufSize, const char *iNewBuf);
    //组装报警文件存储文件夹目录
    void FormatAlarmFileDir(LPLOCAL_ALARM_INFO pAlarmDev, int iChannelID, char* outFilePath, DWORD dwPathLen);
    //组装报警文件存储路径存储路径
    void FormatAlarmFilePath(LOCAL_ALARM_HEAD* struAlarmHead, const char* pBaseDir, char* pCustomName, char* fileSuffix, char* pOutFilePath, DWORD dwPathLen);
    //将重传报警内容写入报警文件中
    BOOL SaveAlarmInfoWithRetransHead(char* szFilename, LOCAL_ALARM_HEAD* struAlarmHead, char* pAlarmInfo, DWORD dwBufLen);
	...
	void ProcFacecaptureStatistics(WPARAM wParam, LPARAM lParam);//目标抓拍统计上传
...
//压线报警暂时不对外开放。
    ...
    void shipsDetectionToTxt(LPNET_DVR_SHIPSDETECTION_ALARM pShipsDetection, int iDeviceIndex, LOCAL_ALARM_HEAD* struAlarmHead);
    ...
    void ProcessVcaAttendAlarm(WPARAM wParam, LPARAM lParam);
    ...
    void dealFileName(char* szFile, char* szFilename, string strType, bool bNonMotor, string strTriggerType);
	void ProcAIOPHistoryVideo(WPARAM wParam, LPARAM lParam);
    void ProcAlarmException(WPARAM wParam, LPARAM lParam);
	//4.playback
    ...
    //5.event ACK
    void InitAckManager();
    void CleanupAckManager();
    BOOL AddEventToAckManager(NET_DVR_RETRANSMISSION_HEAD struHead, LOCAL_ALARM_INFO* pAlarmer);
    void ProcessEventAckTimer();


    void InitSaveRetransEventQueue();       // 初始化
    void UninitSaveRetransEventQueue();     // 清理
    void PushSaveRetransEventTask(const char* szID, const NET_DVR_RETRANSMISSION_HEAD* pHead); // 入队

    //void __cdecl ThreadSaveWorkerProc(void* pParam);
    void SendEventAcksToDevices(DEVICE_ACK_INFO* devicesToAck);




    //void SaveEventSeqToFile(const char* szRetransmissionID, DWORD dwEventSequenceID);
    void SaveEventSeqToFileJson(const char* szRetransmissionID, const NET_DVR_RETRANSMISSION_HEAD* struHead);
    ...
    LONG m_lAlarmInfoHandle;
    int  m_iAlarmNum;
    enum ROW_STATE {
        ROW_NORMAL = 0,
        ROW_RETRANSMISSION_ALARM = 1,  // 重传告警高亮
        ROW_SEND_EVENT_ACK = 2,  // ACK确认高亮
    };
