...
const char* DeviceCfgFile::c_strNode_RetransID = _T("RetransID");
const char* DeviceCfgFile::c_strNode_Old_RetransID_List = _T("OldRetransIDList");
...
cJSON* DeviceCfgFile::WriteDeviceToNode(const STRU_DEVICE_INFO& device, int nIndex)
{
    ...
    do
    {
        ...
        cJSON_AddStringToObject(pDevice, c_strNode_RetransID, device.szRetransmissionID);
        cJSON_AddStringToObject(pDevice, c_strNode_Old_RetransID_List, device.szOldRetransIDList);
...
BOOL DeviceCfgFile::ReadDeviceFromNode(const cJSON* pNode, PLOCAL_DEVICE_INFO pDevice, int* pIndex)
{
    ...
    do
    {
        ...
        *pIndex = nTmp;
        ...
        GetNodeVal_JSON(pNode, c_strNode_RetransID, pDevice->szRetransmissionID, NET_SDK_UUID_LEN);

        GetNodeVal_JSON(pNode, c_strNode_Old_RetransID_List, pDevice->szOldRetransIDList, MAX_OLD_RETRANS_ID_LEN);
        ...
        int i = 0;
        for (; i < sizeof(pDevice->chDeviceIP); i++)
        {
            if (pDevice->chDeviceIP[i] == '\0')
            {
                break;
            }
            else if (pDevice->chDeviceIP[i] == ':')
            {
                sIPV6Addr[i] = '.';
            }
            else
            {
                sIPV6Addr[i] = pDevice->chDeviceIP[i];
            }
        }
        strncpy(pDevice->chDeviceIPInFileName, sIPV6Addr, sizeof(sIPV6Addr));
