...
//     }
    ...
    NET_DVR_BLOCKLIST_ALARM_RECORD struAlarmRecord = {0};
...
//     if (NET_DVR_STDControl(m_lUserID,NET_DVR_DEL_BLOCKLIST_ALARM_RECORD,&struSTDCtrl))
//     {
//         g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_DEL_BLOCKLIST_ALARM_RECORD");
//     }
//     else
//     {
//         g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_DEL_BLOCKLIST_ALARM_RECORD");
//     }


    
        NET_DVR_BLOCKLIST_GROUP_COND struGetCond = {0};
       struGetCond.dwSize = sizeof(NET_DVR_BLOCKLIST_GROUP_COND);
       struGetCond.dwGroupID = 12;
       struCfg.lpInBuffer = &struGetCond;
       struCfg.dwInSize = sizeof(NET_DVR_BLOCKLIST_GROUP_COND);

       
       if (!NET_DVR_GetSTDConfig(m_lUserID,NET_DVR_GET_BLOCKLIST_GROUP_INFO,&struCfg))
       {
           
           g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_GET_BLOCKLIST_GROUP_INFO");
       }
       else
       {
           g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_GET_BLOCKLIST_GROUP_INFO");
       }
	...
	case NET_DVR_DEL_AREA_MONITOR_CFG:
        {
           ...
           //     std::string strConfigXML = "<AreaMonitorDelCond version=\"2.0\" xmlns=\"http://www.isapi.org/ver20/XMLSchema\">\<deleteType>delAll</deleteType><areaID>1</areaID></AreaMonitorDelCond>";
            ...
            int iSdkCmd = NET_DVR_DEL_AREA_MONITOR_CFG;//删除区域布防点
   //         std::string strConfigXML = "<AreaMonitorDelCond xmlns=\"http://www.isapi.org/ver20/XMLSchema\" version=\"2.0\"><deleteType>delSingle</deleteType><areaID>1</areaID><monitorID>31</monitorID></AreaMonitorDelCond>";
            //std::string strConfigXML = "\n<AreaMonitorDelCond xmlns=\"http://www.isapi.org/ver20/XMLSchema\" version=\"2.0\"><deleteType>delSingle</deleteType><areaID>1</areaID><monitorID>31</monitorID></AreaMonitorDelCond>\n";
        ...
        break;
	case NET_DVR_GET_BLOCKLIST_ALARM_INFO:
		{
	...
	case NET_DVR_GET_ALARMLIST:
		...
		break;
	case NET_DVR_DEL_BLOCKLIST_ALARM_RECORD:
		m_dwInterfaceType = INTERFACE_STD_CTRL;
		struSTDCtrl.byDataType = 1;
		...
		break;
	case NET_DVR_GET_BLOCKLIST_GROUP_INFO:
		{
			NET_DVR_BLOCKLIST_GROUP_COND *pCondTemp = (NET_DVR_BLOCKLIST_GROUP_COND *)pCondBuf;
			ParseGetBlockListGroupInfoCond(pCondTemp);
			struSTDCfg.lpCondBuffer = pCondBuf;
			struSTDCfg.dwCondSize = sizeof(NET_DVR_BLOCKLIST_GROUP_COND);
			struSTDCfg.byDataType = 1;
			m_dwInterfaceType = INTERFACE_STD_GET;
		}
		break;
	case NET_DVR_SET_BLOCKLIST_GROUP_INFO:
		{	
			m_dwInterfaceType = INTERFACE_STD_SET; 
			DWORD *pCondTemp = (DWORD *)pCondBuf;
			struSTDCfg.byDataType = 1;
		...
		break;
	case NET_DVR_DEL_BLOCKLIST_GROUP_INFO:
		m_dwInterfaceType = INTERFACE_STD_CTRL;
		struSTDCtrl.byDataType = 1;
		struSTDCtrl.dwXmlSize =m_csConfigParam.GetLength();
		pSetXmlBuf = new char[struSTDCtrl.dwXmlSize];
		...
		memcpy(pSetXmlBuf, m_csConfigParam.GetBuffer(0),struSTDCtrl.dwXmlSize);
		break;
	case NET_DVR_GET_BLOCKLIST_GROUP_RECORD_CFG:
		{
			struSTDCfg.dwInSize =m_csInputCond.GetLength();
		...
		break;
	case NET_DVR_SET_BLOCKLIST_GROUP_RECORD_CFG:
		{
			m_dwInterfaceType = INTERFACE_STD_SET; 
			DWORD *pCondTemp = (DWORD *)pCondBuf;
			struSTDCfg.byDataType = 1;
		...
		break;
	case NET_DVR_DEL_BLOCKLIST_GROUP_RECORD_CFG:
		m_dwInterfaceType = INTERFACE_STD_CTRL;
		struSTDCtrl.byDataType = 1;
		struSTDCtrl.dwXmlSize =m_csConfigParam.GetLength();
		pSetXmlBuf = new char[struSTDCtrl.dwXmlSize];
	...
	case NET_DVR_RETRIEVAL_FACE_DATABASE:
		...
		m_dwInterfaceType = INTERFACE_STD_GET;
		break;
	case NET_DVR_SET_BLOCKLIST_REL_DEV_CFG:
		{
			m_dwInterfaceType = INTERFACE_STD_SET; 
			DWORD *pCondTemp = (DWORD *)pCondBuf;
			struSTDCfg.byDataType = 1;
		...
		break;
	case NET_DVR_DEL_BLOCKLIST_REL_DEV:
		m_dwInterfaceType = INTERFACE_STD_CTRL;
		struSTDCtrl.byDataType = 1;
		struSTDCtrl.dwXmlSize =m_csConfigParam.GetLength();
		pSetXmlBuf = new char[struSTDCtrl.dwXmlSize];
...
void CDlgAreaMonitorCfg::OnSelchangeCmbCfgType() 
{
	...
	case 4://删除区域的单个布防点
		...
		break;
	case 5:  //获取禁止名单轨迹
		m_dwCfgType = NET_DVR_GET_BLOCKLIST_ALARM_INFO;	
	...
	case 9:  //由分组信息获取名单报警列表
		...
		break;
	case 10://删除名单报警记录
		m_dwCfgType = NET_DVR_DEL_BLOCKLIST_ALARM_RECORD;	
		...
		break;
	case 11://获取名单分组列表
		m_dwCfgType = NET_DVR_GET_BLOCKLIST_GROUP_INFO;
		break;
	case 12:  //添加名单分组参数
		m_dwCfgType = NET_DVR_SET_BLOCKLIST_GROUP_INFO;	
		break;

	case 14: //删除名单分组
		m_dwCfgType = NET_DVR_DEL_BLOCKLIST_GROUP_INFO;	
	...
	case 15://获取名单分组记录
		m_dwCfgType = NET_DVR_GET_BLOCKLIST_GROUP_RECORD_CFG;
		...
		break;
	case 16: //添加名单分组记录
		m_dwCfgType = NET_DVR_SET_BLOCKLIST_GROUP_RECORD_CFG;
	...
	case 17: //修改名单分组记录
		...
		break;
	case 18: //删除名单分组记录
		m_dwCfgType = NET_DVR_DEL_BLOCKLIST_GROUP_RECORD_CFG;
		xmlCfgParam.Parse("<RecordInfo>\
										 <groupID>1</groupID>\
										 <faceDBID>1</faceDBID>\
										 <faceRecordID>1</faceRecordID>\
	...
	case 31: //人脸库检索
		...
		break;		
	case 32: //添加分组名单关联设备
		m_dwCfgType = NET_DVR_SET_BLOCKLIST_REL_DEV_CFG;
	...
	case 33: //修改分组名单关联设备
		...
		break;
	case 34: //删除分组名单关联设备
		m_dwCfgType = NET_DVR_DEL_BLOCKLIST_REL_DEV;	
		xmlCfgParam.Parse("<GroupRelDevInfo>\
											  <RelateInfoList>\
											  <RelateInfo>\
											  <devID>1</devID>\
...
BOOL    CDlgAreaMonitorCfg::ParseGetBlockListGroupInfoCond(NET_DVR_BLOCKLIST_GROUP_COND* pData)
{
	CXmlBase    struXml;
	char szLan[128] = {0};
    if (struXml.Parse(m_csInputCond.GetBuffer(0)) == NULL)
	...
	pData->dwSize = sizeof(NET_DVR_BLOCKLIST_GROUP_COND);
    string csDetectionTarget = "";
