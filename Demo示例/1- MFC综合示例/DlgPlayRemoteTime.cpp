...
void CALLBACK DrawPlaybackNPQState(long lRealHandle, HDC hDc, DWORD nUser)
{
    ...
    CDlgPlayRemoteTime* pThis = (CDlgPlayRemoteTime*)g_pDlgRemoteTime;
    char szBuffer[256] = { 0 };
    int iPos = 0;
...
void CALLBACK PlayRemoteTimeDataCallBack_V40(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, void *pUser)
{
    CDlgPlayRemoteTime *pThis = (CDlgPlayRemoteTime*)pUser;
    BOOL bRet = FALSE;
    char szUUID[65] = { 0 };
    int i = 0;
    switch (dwDataType)
    {
    case NET_DVR_PLAYBACK_NEW_STREAM: //码流标记
        memcpy(szUUID, pBuffer, 64);
        g_pMainDlg->AddLog(pThis->m_iDeviceIndex, OPERATION_SUCC_T, "PlayRemoteTimeDataCallBack_V40 new pos 1204, uuid %s", szUUID);
        if (pThis->m_bCallBack && pThis->m_bWaitNewPos)
        {
            PlayM4_ResetSourceBuffer(lPort);
            pThis->m_bWaitNewPos = FALSE;
            if (!g_hRemoteTimeHandle)
            {
                g_hRemoteTimeHandle = CreateFile("./RemoteTime.mp4", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                if (g_hRemoteTimeHandle == INVALID_HANDLE_VALUE)
                {
            ...
            if (lPort >= 0)
            {
                if (pThis->m_emDstSequence == PLAY_FORWARD)
                {
                    bRet = PlayM4_Play(lPort, g_hPlayBackWnd);
                }
                else  if (pThis->m_emDstSequence == PLAY_REVERSE)
                {
                    bRet = PlayM4_ReversePlay(lPort);
                }
                if (pThis->m_emDstSequence!= PLAY_NONE && !bRet)
                {
                    g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_FAIL_T, "PlayM4_Play err [%d]!", PlayM4_GetLastError(lPort));
                }
            }
            pThis->m_emDstSequence = PLAY_NONE;
        }
        break;
    case NET_DVR_SYSHEAD:
        ...
        if (lPort < 0)  
        {
            ...
            if (strcmp(g_sSecretKey, "StreamNotEncrypt") != 0)
            {
                ...
                {
                    g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_FAIL_T, "PlayM4_SetSecretKey err [%d]!", PlayM4_GetLastError(lPort));
                }
            }
            ...
            if (dwBufSize > 0)
            {
                ...
                if (bRet)
                {
                    if (!PlayM4_Play(lPort, g_hPlayBackWnd))
                    {
                        g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_FAIL_T, "PlayM4_Play err [%d]!",PlayM4_GetLastError(lPort));
                    }
                    else
                    {
                        if (pThis->m_emDstSequence == PLAY_REVERSE)
                        {
                            if (!PlayM4_ReversePlay(lPort))
                            {
                                g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_FAIL_T, "PlayM4_ReversePlay err [%d]!", PlayM4_GetLastError(lPort));
                            }
                        }
                    }
                }
                ...
                {
                    g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_FAIL_T, "PlayM4_OpenStream err[%d]!", PlayM4_GetLastError(lPort));
                }
                pThis->m_emDstSequence = PLAY_NONE;
            }
        }
        break;
    case NET_DVR_VOD_DRAW_FRAME:
    case NET_DVR_VOD_DRAW_DATA:
    case NET_DVR_STREAMDATA:
        if (dwDataType == NET_DVR_VOD_DRAW_FRAME && pThis->m_bCallBack && pThis->m_bWaitNewPos)
        {
            pThis->m_bWaitNewPos = FALSE;
        }
        if (pThis->m_bWaitNewPos || pThis->m_bPlayStop || pThis->m_bChangingSeq || pThis->m_bPosChange || (pThis->m_bDrawFrameStatus && dwDataType == NET_DVR_STREAMDATA)) // 数据丢弃
        {
            g_pMainDlg->AddLog(pThis->m_iDeviceIndex, OPERATION_SUCC_T, "PlayRemoteTimeDataCallBack_V40 drop");
            break;
        }
        if (dwBufSize > 0 && lPort >= 0)
        {
            for (i = 0; i < 1000; i++)
            {
                if (pThis->m_bPlayStop || pThis->m_bPosChange || pThis->m_bChangingSeq || pThis->m_emDstSequence != PLAY_NONE || (pThis->m_bDrawFrameStatus && dwDataType == NET_DVR_STREAMDATA))
                {
                    break;
                }
                bRet = PlayM4_InputData(lPort, pBuffer, dwBufSize);
                if (!bRet)
                {
                    if (i >= 999)
                    {
                        g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_FAIL_T, "PlayM4_InputData err[%d]!", PlayM4_GetLastError(lPort));
                    }
                ...
                {
                    DWORD  dwReturn = 0;
                    WriteFile(g_hRemoteTimeHandle, pBuffer, dwBufSize, &dwReturn, NULL);
                    break;
                }
...
void CALLBACK cbFunction(long nPort, char * pBuf, long nSize, FRAME_INFO * pFrameInfo, long nReserved1, long nReserved2)
{
	...
	case NET_DVR_STREAMDATA:
		...
		break;
    case NET_DVR_HLS_INDEX_DATA:
    {
        g_pMainDlg->AddLog(g_pMainDlg->GetCurDeviceIndex(), PLAY_SUCC_T, "Playback recv HLS index data!");
    }
    break;
	default:
...
CDlgPlayRemoteTime::CDlgPlayRemoteTime(CWnd* pParent /*=NULL*/)
    ...
    , m_strDoubleVerificationUsername(_T(""))
    , m_strDoubleVerificationPassword(_T(""))
    , m_bRemoteFile(FALSE)
    , m_bSaveMp4(FALSE)
    , m_bHLS(FALSE)
    , m_bCallBack(FALSE)
    ...
    , m_bNewPos(FALSE)
    , m_bWaitNewPos(FALSE)
    , m_bChangingSeq(FALSE)
    , m_emDstSequence(PLAY_NONE)
    , m_bDrawFrameStatus(FALSE)
    , m_bPosChange(FALSE)
    , m_bPlayStop(FALSE)
{
...
void CDlgPlayRemoteTime::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_EXTERNALCALLBACK, m_bCallBack);
    DDX_Check(pDX, IDC_CHECK_NEWCMD, m_bNewCmd);
    //}}AFX_DATA_MAP
    ...
    DDX_Text(pDX, IDC_EDIT_DOUBLE_VERIFICATION_USERNAME, m_strDoubleVerificationUsername);
    DDX_Text(pDX, IDC_EDIT_DOUBLE_VERIFICATION_PASSWORD, m_strDoubleVerificationPassword);
    DDX_Check(pDX, IDC_CHECK_REMOTE_FILE, m_bRemoteFile);
    DDX_Check(pDX, IDC_CHECK_MP4, m_bSaveMp4);
...
void CDlgPlayRemoteTime::OnBnClickedBtnTimePlay()
{
	...
	if (m_lPlayHandle == -1)
	{
        ...
        m_bNewPos = struStreamCallBackCfg.byPlayBackNewPos;
        ...
        sprintf((char *)struVodPara.sUserName, "%s", m_strDoubleVerificationUsername);
        sprintf((char *)struVodPara.sPassword, "%s", m_strDoubleVerificationPassword);
        struVodPara.byRemoteFile = m_bRemoteFile;
        struVodPara.byHls = m_bHLS;
...
//         struPlayCon.byOptimalStreamType = m_bOptimalStreamType;
        ...
        struVodPara.byUseAsyn = 0;
        m_lPlayHandle = NET_DVR_PlayBackByTime_V50(m_lLoginID, &struVodPara);
		if (m_lPlayHandle == -1)
		{
			g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_PlayBackByTime_V50 ChanNum[%d]", m_iChanShowNum);
			g_StringLanType(szLan, "远程回放失败", "Fail to play back remote");
			AfxMessageBox(szLan);
			return;
		}

        if (m_bCallBack) // 使能码流标记，使用回调方式
        {
            m_emDstSequence = struVodPara.byPlayMode == 0 ? PLAY_FORWARD : PLAY_REVERSE;
            m_bWaitNewPos = FALSE;
            m_bPlayStop = FALSE;
            m_bDrawFrameStatus = FALSE;
            m_bPosChange = FALSE;
            m_bChangingSeq = FALSE;
            NET_DVR_SetPlayDataCallBack_V40(m_lPlayHandle, PlayRemoteTimeDataCallBack_V40, this);
        }

 		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_PlayBackByTime_V50 ChanNum[%d]", m_iChanShowNum);

        NET_DVR_LINK_ADDR struParam = { 0 };
        if (NET_DVR_GetLinkAddr(m_lPlayHandle, ENUM_LINK_PLAYBACK, &struParam))
        {
        ...
        if (1 == m_dwLinkMode)
        {
            ...
            //NET_SDK_NPQ_NOTIFY_PARAM struParam = { 0 };
            //struParam.bHaveBitrate = 1;
            //struParam.dwBitRate = 10000;

            //if (NET_DVR_PlaybackSetNPQNotifyParam(m_lPlayHandle, &struParam))
            //{
            //    g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_PlaybackSetNPQNotifyParam, Succ!");
            //}
            //else
            //{
            //    g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_PlaybackSetNPQNotifyParam, Fail!");
            //}
        }
...
void CDlgPlayRemoteTime::StopPlay()
{
    ...
    if (g_hRemoteTimeHandle != NULL)
    {
        CloseHandle(g_hRemoteTimeHandle);
        g_hRemoteTimeHandle = NULL;
    }
}
...
void CDlgPlayRemoteTime::OnBnClickedBtnRtDownload()
{
	...
	if (!m_bDown)
	{
        ...
        // [add] by yangzheng 2019/10/15
        if (m_bSaveMp4==TRUE)
        {
            struDownloadCond.byVODFileType = 1;
        }
...
void CDlgPlayRemoteTime::OnBtnSetTime() 
{
    ...
    if (m_bCallBack)
    {
        m_bWaitNewPos = m_bNewPos;
        m_bPosChange = TRUE;
        PlayM4_SetPlayedTimeEx(lPort, 0);
        //PlayM4_ResetSourceBuffer(lPort);
    }

    BOOL bPlayCtrl = FALSE;
    if (m_bNewCmd)
    {
        NET_DVR_PLAYBACKCTRL_V60 struPlaybackCtrl = { 0 };
        struPlaybackCtrl.dwSize = sizeof(NET_DVR_PLAYBACKCTRL_V60);
        memcpy(&struPlaybackCtrl.struTime, &struTime, sizeof(NET_DVR_TIME_V50));
        //memcpy(struPlaybackCtrl.byUUID, "1234567812345678123456781234567812345678123456781234567812345678", strlen("1234567812345678123456781234567812345678123456781234567812345678"));
        dwUUIDCnt++;
        char buffer[9] = {0}; 
        sprintf(buffer, "%08x", dwUUIDCnt);
        memcpy(struPlaybackCtrl.byUUID, "12345678123456781234567812345678123456781234567812345678", strlen("12345678123456781234567812345678123456781234567812345678"));
        memcpy(&struPlaybackCtrl.byUUID[56], buffer, 8);
        bPlayCtrl = NET_DVR_PlayBackControl_V40(m_lPlayHandle, NET_DVR_PLAYSETTIME_V60, &struPlaybackCtrl, sizeof(struPlaybackCtrl), NULL, 0);
    }
    else
    {
        m_bWaitNewPos = FALSE;
        bPlayCtrl = NET_DVR_PlayBackControl_V40(m_lPlayHandle, NET_DVR_PLAYSETTIME_V50, &struTime, sizeof(struTime), NULL, 0);
    }

    if (bPlayCtrl)
    {
        if (m_bCallBack && !m_bWaitNewPos)
        {
            if (!g_hRemoteTimeHandle)
            {
                g_hRemoteTimeHandle = CreateFile("./RemoteTime.mp4", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        ...
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_PLAYSETTIME_Vxx, uuid %d", dwUUIDCnt);
    }
    else
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_PLAYSETTIME_Vxx");
    }
...
void CDlgPlayRemoteTime::OnRadioForward() 
{
	...
	{
        DWORD timep = PlayM4_GetSpecialData(lPort);
        NET_DVR_TIME  struTime = {};
        struTime.dwYear = ((timep >> 26) + 2000);
        struTime.dwMonth = ((timep >> 22) & 15);
        struTime.dwDay = ((timep >> 17) & 31);
        struTime.dwHour = ((timep >> 12) & 31);
        struTime.dwMinute = ((timep >> 6) & 63);
        struTime.dwSecond = ((timep >> 0) & 63);
        if (m_bCallBack)
        {
            m_bWaitNewPos = m_bNewPos;
            m_emDstSequence = PLAY_FORWARD;
            m_bChangingSeq = TRUE;
            PlayM4_Pause(lPort, 1);
        }
        ...
        if (m_bNewCmd)
        {
            NET_DVR_PLAYBACKCTRL_V60 struPlaybackCtrl = { 0 };
            struPlaybackCtrl.dwSize = sizeof(NET_DVR_PLAYBACKCTRL_V60);
            struPlaybackCtrl.struTime.wYear = (WORD)struTime.dwYear;
            struPlaybackCtrl.struTime.byMonth = (BYTE)struTime.dwMonth;
            struPlaybackCtrl.struTime.byDay = (BYTE)struTime.dwDay;
            struPlaybackCtrl.struTime.byHour = (BYTE)struTime.dwHour;
            struPlaybackCtrl.struTime.byMinute = (BYTE)struTime.dwMinute;
            struPlaybackCtrl.struTime.bySecond = (BYTE)struTime.dwSecond;
            dwUUIDCnt++;
            char buffer[9] = { 0 };
            sprintf(buffer, "%08x", dwUUIDCnt);
            memcpy(struPlaybackCtrl.byUUID, "12345678123456781234567812345678123456781234567812345678", strlen("12345678123456781234567812345678123456781234567812345678"));
            memcpy(&struPlaybackCtrl.byUUID[56], buffer, 8);
            bPlayCtrl = NET_DVR_PlayBackControl_V40(m_lPlayHandle, NET_DVR_PLAY_FORWARD_V60, &struPlaybackCtrl, sizeof(struTime), NULL, NULL);
        }
        else
        {
            m_bWaitNewPos = FALSE;
            bPlayCtrl = NET_DVR_PlayBackControl_V40(m_lPlayHandle, NET_DVR_PLAY_FORWARD, &struTime, sizeof(struTime), NULL, NULL);
        }
		...
		if(bPlayCtrl)
		{
            if (m_bCallBack && !m_bWaitNewPos)
            {
                PlayM4_Play(lPort, g_hPlayBackWnd);
                m_bChangingSeq = FALSE;
                m_emDstSequence = PLAY_NONE;
                if (!g_hRemoteTimeHandle)
                {
			...
			g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_PLAY_FORWARDxx");
			((CButton *)GetDlgItem(IDC_RADIO_FORWARD_TIME))->SetCheck(TRUE);
			((CButton *)GetDlgItem(IDC_RADIO_REVERSE_TIME))->SetCheck(FALSE);
			UpdateData(FALSE);
		}
		...
		{
			g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_PLAY_FORWARDxx");
...
void CDlgPlayRemoteTime::OnRadioReverse() 
{
	...
	else
	{
        ...
        if (m_bNewCmd)
        {
            ...
            memcpy(struPlaybackCtrl.byUUID, "12345678123456781234567812345678123456781234567812345678", strlen("12345678123456781234567812345678123456781234567812345678"));
            memcpy(&struPlaybackCtrl.byUUID[56], buffer, 8);
            bPlayCtrl = NET_DVR_PlayBackControl_V40(m_lPlayHandle, NET_DVR_PLAY_REVERSE_V60, &struPlaybackCtrl, sizeof(struTime), NULL, NULL);
        }
        else
        {
            m_bWaitNewPos = FALSE;
            bPlayCtrl = NET_DVR_PlayBackControl_V40(m_lPlayHandle, NET_DVR_PLAY_REVERSE, &struTime, sizeof(struTime), NULL, NULL);
        }
		if(bPlayCtrl)
		{
            if (m_bCallBack && !m_bWaitNewPos)
            {
                PlayM4_ReversePlay(lPort);
                m_bChangingSeq = FALSE;
                m_emDstSequence = PLAY_NONE;
                if (!g_hRemoteTimeHandle)
                {
