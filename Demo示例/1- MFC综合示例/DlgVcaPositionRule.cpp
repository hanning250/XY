...
Return:	none
**********************************************************/
void CALLBACK DrawPositionAlertShow(LONG lRealHandle, HDC hDc, void* pUser)
{
	SetBkMode(hDc, TRANSPARENT);
	SetTextColor(hDc, RGB(255, 255, 255));
	if(g_pDlgVcaPositionRule->m_bRuleActive)
	{
		g_pDlgVcaPositionRule->F_DrawFun(lRealHandle, hDc, pUser); 
	}
}
...
void CDlgVcaPositionRule::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMBO1, m_BackgroundSuppression);
    DDX_Text(pDX, IDC_EDIT_TOILETTARRY_DELAY, m_iToiletTarryDelay);
}
...
BEGIN_MESSAGE_MAP(CDlgVcaPositionRule, CDialog)
    ...
    ON_BN_CLICKED(IDC_BTN_DEL_ONE_RULE, &CDlgVcaPositionRule::OnBnClickedBtnDelOneRule)
END_MESSAGE_MAP()
...
BOOL CDlgVcaPositionRule::OnInitDialog() 
{
    ...
    if (!NET_DVR_GetDVRConfig(m_lServerID, NET_DVR_GET_VCA_CTRLCFG, 0, &m_struVcaCtrlCfg, sizeof(NET_VCA_CTRLCFG), &dwReturned))
    {
        ...
        AfxMessageBox(szLan);
        //	OnButexit();
       // return FALSE;  //获取失败，不让函数直接返回，可以继续执行
    }
    else
    {
	...
	m_iCurRuleID = 0;
    m_comboRuleID.SetCurSel(m_iCurRuleID);
   ...
   if (m_struTrackCfg.byEnable)
    {
        g_StringLanType(szLan, "禁止跟随", "Disable track");
        GetDlgItem(IDC_BTN_ENABLE_CALIBRATE)->SetWindowText(szLan);
    }
    else
    {
        g_StringLanType(szLan, "启用跟随", "Enable track");
        GetDlgItem(IDC_BTN_ENABLE_CALIBRATE)->SetWindowText(szLan);
    }
...
/*********************************************************
Function:	OnBnClickedBtnDelOneRule
Desc: 删除某一个规则
Input:
Output:
Return:	void
**********************************************************/
void CDlgVcaPositionRule::OnBnClickedBtnDelOneRule()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
...
//m_struBehaviorAbility.byMaxRuleNum = 8;
    ...
    if (m_struBehaviorAbility.dwAbilityType == 0)
    {
...
BOOL CDlgVcaPositionRule::InsertRuleType(int iBehaviorAbilityType, int iBehaviorAbilityTypeEx)
{
	...
	{
        g_StringLanType(szLan, "授课", "Teaching");
        m_comboRuleType.InsertString(index, szLan);
	...
	{
        g_StringLanType(szLan, "回答问题", "Answer");
        m_comboRuleType.InsertString(index, szLan);
    ...
    {
        g_StringLanType(szLan, "组合规则事件", "Combined event");
        m_comboRuleType.InsertString(index, szLan);
    ...
    {
        g_StringLanType(szLan, "板书", "Writing");
        m_comboRuleType.InsertString(index, szLan);
    ...
    if ((iBehaviorAbilityType&TOILET_TARRY_ABILITY) != 0)
    {
        g_StringLanType(szLan, "如厕超时", "Toilet tarry");
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_TOILET_TARRY);
        index++;
    }
	return TRUE;
...
BOOL CDlgVcaPositionRule::InsertRuleID(int RuleNum)
{
    if (RuleNum > 0)
    {
        m_comboRuleID.ResetContent(); //当获取到的规则数大于0时，清空下来框数据，否则不请空，保留初始数据
    }
    ...
    */
    ...
    if (bRet)
    {
        if (dwRetList == NET_DVR_NOERROR)
        {
            ...
            m_BackgroundSuppression.SetCurSel(m_struRuleCfgV42.struRule[m_iCurRuleID].byBackgroundSuppression);
...
void CDlgVcaPositionRule::OnSelchangeComboRuleId() 
{
    ...
    if (m_iCurRuleID < 0)
    {
        ...
        m_comboRuleID.SetCurSel(m_iCurRuleID);
        /*char szLan[128] = "";
        g_StringLanType(szLan, "请选择一个规则ID", "Please Select a RuleID!");
        MessageBox(szLan);
        return;*/
    }
	m_comEventPriority.SetCurSel(m_struRuleCfgV42.struRule[m_iCurRuleID].byEventPriority);
...
BOOL CDlgVcaPositionRule::ShowRuleTypeWnd(VCA_EVENT_TYPE struEventType, WORD wEventTypeEx)
{
	...
	else
	{
        ...
        else if (struEventType == VCA_TOILET_TARRY)
        {
            GetDlgItem(IDC_COMBO_SENSITIVE)->ShowWindow(SW_HIDE);
            GetDlgItem(IDC_STATIC_SENSITIVITY)->ShowWindow(SW_HIDE);
            GetDlgItem(IDC_STATIC_DURATION)->ShowWindow(SW_HIDE);

            GetDlgItem(IDC_EDIT_TOILETTARRY_DELAY)->ShowWindow(SW_SHOW);
            GetDlgItem(IDC_STATIC_TOILETTARRY_DELAY)->ShowWindow(SW_SHOW);
        }
		else
		{
			return FALSE;
...
void CDlgVcaPositionRule::GetEventParaSet(VCA_EVENT_TYPE struEventType,NET_VCA_EVENT_UNION uEventParam, WORD wEventTypeEx)
{
	...
	case VCA_INTRUSION:
        ...
        m_iAlarmDelay = uEventParam.struIntrusion.wDuration > 0 ? uEventParam.struIntrusion.wDuration : 1; //增加初始值
	...
	case VCA_LOITER:
        ...
        m_iAlarmPersist = uEventParam.struLoiter.wDuration > 0 ? uEventParam.struLoiter.wDuration : 1; //增加初始值
	...
	case VCA_LEFT_TAKE:
        ...
        m_iAlarmPersist = uEventParam.struTakeTeft.wDuration > 0 ? uEventParam.struTakeTeft.wDuration : 1; //增加初始值
		m_staticAlarmPersist.ShowWindow(SW_SHOW);
		m_staticAlarmPersistArea.ShowWindow(SW_SHOW);
		m_editAlarmPersist.ShowWindow(SW_SHOW);
		break;
	case VCA_PARKING:
        ...
        m_iAlarmPersist = uEventParam.struParking.wDuration > 0 ? uEventParam.struParking.wDuration : 1; //增加初始值
	...
	case VCA_STICK_UP:
        ...
        m_iAlarmPersist = uEventParam.struStickUp.wDuration > 4 ? uEventParam.struStickUp.wDuration : 4; //增加初始值
		m_staticAlarmPersist.ShowWindow(SW_SHOW);
		m_staticStickupAlarmPersist.ShowWindow(SW_SHOW);
		m_editAlarmPersist.ShowWindow(SW_SHOW);
		break;
	case VCA_INSTALL_SCANNER:
        ...
        m_iReadCardTimePersist = uEventParam.struScanner.wDuration > 4 ? uEventParam.struScanner.wDuration : 4; //增加初始值
		m_staticReadCardTimePersist.ShowWindow(SW_SHOW);
		m_staticReadCardTimePersistArea.ShowWindow(SW_SHOW);
		m_editReadCardTimePersist.ShowWindow(SW_SHOW);
		break;
	case VCA_LECTURE:
        ...
        m_iAlarmDelay = uEventParam.struLecture.wDuration > 0 ? uEventParam.struLecture.wDuration : 1; //增加初始值
    ...
    case VCA_LEFT:
        ...
        m_iAlarmPersist = uEventParam.struLeft.wDuration > 10 ? uEventParam.struLeft.wDuration : 10; //增加初始值
        bySensitivity = uEventParam.struAnswer.bySensitivity;
        m_staticAlarmPersist.ShowWindow(SW_SHOW);
        m_staticAlarmPersistArea.ShowWindow(SW_SHOW);
        m_editAlarmPersist.ShowWindow(SW_SHOW);
    ...
    case VCA_TAKE:
        ...
        m_iAlarmPersist = uEventParam.struTake.wDuration > 10 ? uEventParam.struTake.wDuration : 10; //增加初始值
    ...
    case VCA_VIOLENT_MOTION:
        ...
        m_iParam1 = uEventParam.struViolentMotion.wDuration > 0 ? uEventParam.struViolentMotion.wDuration : 1; //增加初始值
    ...
    case VCA_GET_UP:
        ...
        m_iParam1 = uEventParam.struGetUp.wDuration > 0 ? uEventParam.struGetUp.wDuration : 1; //增加初始值
...
Input:	lPlayHandle, player handle
		hDc : DC handle
		pUser: User data to distinguish different channels
Output:	
Return:	void
**********************************************************/
void CDlgVcaPositionRule::F_DrawFun(long lPlayHandle, HDC hDc, void* pUser)
{
	COLORREF color;
	...
	if(	m_struOneRule.dwEventType == VCA_ENTER_AREA
        ...
        || m_struOneRule.dwEventType == VCA_TOILET_TARRY
/*        || m_struOneRule.wEventTypeEx == ENUM_VCA_EVENT_COMBINED_RULE*/
		)
	{
		F_DrawFrame(lPlayHandle, hDc, pUser, m_iCurRuleID, color);
	}
	else if (m_struOneRule.dwEventType == VCA_TRAVERSE_PLANE)
	{
		F_DrawLine(lPlayHandle, hDc, pUser, m_iCurRuleID, color);
	}
	
	F_DrawFilter(lPlayHandle, hDc, pUser, m_iCurRuleID, RGB(0, 255, 125));
}
...
Return:	 void
**********************************************************/
void CDlgVcaPositionRule::F_DrawFrame(long lRealHandle, HDC hDc, void* pUser, DWORD dwRuleID, COLORREF color)
{
...
Return:	 void
**********************************************************/
void CDlgVcaPositionRule::F_DrawFilter(long lRealHandle, HDC hDc, void* pUser, DWORD dwRuleID, COLORREF color)
{
...
Return:	void 
**********************************************************/
void CDlgVcaPositionRule::F_DrawLine(long lPlayHandle, HDC hDc, void* pUser, DWORD dwRuleID, COLORREF color)
{
	UNREFERENCED_PARAMETER(pUser);
...
BOOL CDlgVcaPositionRule::SetOneRule(NET_VCA_ONE_RULE_V41 &struVcaOneRule)
{
	...
	//Parameter of Behavior detection
...
void CDlgVcaPositionRule::SetVcaEventPara(NET_VCA_EVENT_UNION &uEventparam, VCA_EVENT_TYPE struEventType, WORD struEventTypeEx)
{
    ...
    case VCA_TOILET_TARRY:
        ...
        uEventparam.struToiletTarry.wDelay = (WORD)m_iToiletTarryDelay;
        break;
	default:
		break;
	}
...
void CDlgVcaPositionRule::SetVcaEventParaV42(NET_VCA_EVENT_UNION &uEventparam, WORD wEventType)
{
    ...
    case ENUM_VCA_EVENT_TOILET_TARRY:
        uEventparam.struToiletTarry.wDelay = m_iToiletTarryDelay;
        memcpy(&uEventparam.struToiletTarry.struRegion, &m_struVcaPolygon[m_iCurRuleID], sizeof(NET_VCA_POLYGON));
        break;
    default:
        break;
...
/*********************************************************
  ...
  Desc:		Setup Behavior detection Rule
  Input:	
  Output:	
  Return:	
**********************************************************/
...
//     	}
    ...
    m_struRuleCfgV42.struRule[m_iCurRuleID].byBackgroundSuppression = m_BackgroundSuppression.GetCurSel();
...
BOOL CDlgVcaPositionRule::InsertComboVcaType()
{
	...
	{
		char szLan[128] = {0};
		g_StringLanType(szLan, "异常行为检测基本版", "Behavior of basic");
	...
	{
		char szLan[128] = {0};
		g_StringLanType(szLan, "异常行为检测高级版", "Behavior of advanced");
		m_comboVcaType.InsertString(index, szLan);
	...
	{
		char szLan[128] = {0};
		g_StringLanType(szLan, "异常行为检测完整版", "Behavior of full");
		m_comboVcaType.InsertString(index, szLan);
...
void CDlgVcaPositionRule::OnBtnEnableCalibrate() 
{
    ...
    if (SetTrackCfg())
    {
        if (m_struTrackCfg.byEnable)
        {
            GetDlgItem(IDC_BTN_ENABLE_CALIBRATE)->SetWindowText("禁止跟随");

        }
        else
        {
            GetDlgItem(IDC_BTN_ENABLE_CALIBRATE)->SetWindowText("启用跟随");
        }
...
BOOL CDlgVcaPositionRule::EventTypeToEventTypeEx(VCA_EVENT_TYPE struEventType, WORD& wEventTypeEx)
{
    ...
    case VCA_INSTALL_SCANNER:
        ...
        break;
    case VCA_TOILET_TARRY:
        wEventTypeEx = ENUM_VCA_EVENT_TOILET_TARRY;
        break;
    default:
         iRet = FALSE;
		break;
...
BOOL CDlgVcaPositionRule::EventTypeExToEventType(VCA_EVENT_TYPE& struEventType, WORD wEventTypeEx)
{
    ...
    case ENUM_VCA_EVENT_INSTALL_SCANNER:
        ...
        break;
    case ENUM_VCA_EVENT_TOILET_TARRY:
        struEventType = VCA_TOILET_TARRY;
        break;
    default:
        iRet = FALSE;
        break;
...
void CDlgVcaPositionRule::OnCbnSelchangeComboPositionChannel()
{
    ...
    {
        bRet = NET_DVR_RigisterDrawFunEx(m_lPlayHandle, DrawPositionAlertShow, NULL);//DC Draw callback
    }
}
