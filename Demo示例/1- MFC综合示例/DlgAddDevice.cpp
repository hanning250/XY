...
extern void CALLBACK LoginResultCallBack_V40(LONG lUserID, DWORD dwResult, LPNET_DVR_DEVICEINFO_V40 lpDeviceInfo, void* pUser);
typedef struct{
	HWND m_hwnd;
	HWND mList_hwnd;
...
返回值:        UTF8编码的字符串
**********************************************************/
static std::string A2UTF8(const char* pCont)
{
    if (NULL == pCont)
    {
        return std::string("");
    ...
    /*lint -e{119} 没有多余参数*/
    wchar_t* pBuffw = new(std::nothrow) wchar_t[(unsigned int)iNum];
    if (pBuffw == NULL)
    {
        return std::string("");
    }
    MultiByteToWideChar(CP_ACP, NULL, pCont, -1, pBuffw, iNum);

    int len = WideCharToMultiByte(CP_UTF8, 0, pBuffw, iNum - 1, NULL, NULL, NULL, NULL);
    /*lint -e{119} 没有多余参数*/
    char* pLpsz = new(std::nothrow) char[(unsigned int)len + 1];
    if (pLpsz == NULL)
    {
        delete[] pBuffw;
        return std::string("");
    ...
    WideCharToMultiByte(CP_UTF8, 0, pBuffw, iNum - 1, pLpsz, len, NULL, NULL);
    pLpsz[len] = '\0';
    delete[] pBuffw;

    std::string strRet(pLpsz);
    delete[] pLpsz;
    return strRet;
#else
    bSucess = HPR_FALSE;
    Core_SetLastError(NET_DVR_FUNCTION_NOT_SUPPORT_OS);
    return std::string("");
#endif
}
...
void CDlgAddDevice::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Control(pDX, IDC_COM_VERIFYMODE, m_cmbVerifyMode);
}
...
DWORD ThreadGetAreaList(LPVOID pParam)		//sub thread to execute network operation
{
	...
	cJSON* dom = cJSON_Parse(retData.c_str());
	assert(cJSON_HasObjectItem(dom, "data"));
	cJSON *arealist = cJSON_GetObjectItem(dom, "data");
	int nAreaSize = cJSON_GetArraySize(arealist);
	for (int i = 0; i < nAreaSize; i++){
		cJSON* item = cJSON_GetArrayItem(arealist, i);
		countryName[i] = cJSON_GetObjectItem(item, "name")->valuestring;
		countryID[i] = cJSON_GetObjectItem(item, "id")->valueint;
	}
	PostMessage(hAreaList, WM_DDNSAREALIST, (WPARAM)0, (LPARAM)nAreaSize);
	cJSON_Delete(dom);
	return (DWORD)0;
}
...
DWORD ThreadGetDeviceList(LPVOID pParam)	// sub thread to execute network operation
{
	...
	cJSON* codeItem = cJSON_GetObjectItem(dom, "code");
	string code = (codeItem && codeItem->valuestring) ? codeItem->valuestring : "";
	if (code != "200"){
		cJSON* msgItem = cJSON_GetObjectItem(dom, "msg");
		string msg = (msgItem && msgItem->valuestring) ? msgItem->valuestring : "";
		...
		cJSON* pageItem = cJSON_GetObjectItem(dom, "page");
		cJSON* totalItem = cJSON_GetObjectItem(pageItem, "total");
		int total = (totalItem != NULL) ? totalItem->valueint : 0;
		if (total == 0){
			g_pMainDlg->AddLog(-1, DDNS_FAIL_T, "Get Device List");
			...
			cJSON* devicelist = cJSON_GetObjectItem(dom, "data");
			int devicelistSize = cJSON_GetArraySize(devicelist);
			for (int i = 0; i < devicelistSize; i++){
				cJSON* devItem = cJSON_GetArrayItem(devicelist, i);

				cJSON* nameItem = cJSON_GetObjectItem(devItem, "deviceName");
				cJSON* serialItem = cJSON_GetObjectItem(devItem, "subSerial");
				cJSON* domainItem = cJSON_GetObjectItem(devItem, "domain");
				cJSON* ipItem = cJSON_GetObjectItem(devItem, "deviceIp");
				cJSON* portItem = cJSON_GetObjectItem(devItem, "mappingHiddnsCmdPort");

				device.name = (nameItem && nameItem->valuestring) ? (CString)nameItem->valuestring : _T("");
				device.serial = (serialItem && serialItem->valuestring) ? (CString)serialItem->valuestring : _T("");
				device.domain = (domainItem && domainItem->valuestring) ? (CString)domainItem->valuestring : _T("");
				device.ipAddr = (ipItem && ipItem->valuestring) ? (CString)ipItem->valuestring : _T("");
				int port = (portItem != NULL) ? portItem->valueint : 0;
				CString devicePort;
...
DWORD ThreadGetToken(LPVOID pParam)
{
	...
	if (retData.find("200") != -1){
		cJSON* dom = cJSON_Parse(retData.c_str());
		cJSON *info = cJSON_GetObjectItem(dom, "data");
		string accessToken, areaDomain;
		accessToken = cJSON_GetObjectItem(info, "accessToken")->valuestring;
		areaDomain = cJSON_GetObjectItem(info, "areaDomain")->valuestring;
...
void CDlgAddDevice::OnBnClickedAddDeviceOk()
{
    ...
    string strDeviceIP = A2UTF8(szDeviceIP);
	...
	if (strncmp(m_csNodeName.GetBuffer(0), "All", 3)==0)//Use "All.X" X means the device number，From 192.168.1.Y To 192.168.1.X+Y
	{
		...
		for (int i = 0; i<nErr; i++)
		{
			...
			sprintf(g_struDeviceInfo[iDeviceIndex].chLoginPwd, "");
            ...
            g_struDeviceInfo[iDeviceIndex].byVerifyMode = m_cmbVerifyMode.GetCurSel();  //0-不认证 1-双向认证 2-单向认证
    ...
    if (m_gbAsynLogin)
    {
        ...
        struLoginInfo.cbLoginResultV40 = LoginResultCallBack_V40;
    ...
    memcpy(struLoginInfo.sDeviceAddress, strDeviceIP.c_str(), NET_DVR_DEV_ADDRESS_MAX_LEN);
    ...
    */
    ...
    struLoginInfo.byVerifyMode = m_cmbVerifyMode.GetCurSel();
    ...
    if ((m_gbAsynLogin && m_giAsynLoginReturn == 2) || lUserID < 0)
	{
        ...
        else if (nErr == NET_DVR_USERNOTEXIST)
        {
            char szLan[256] = { 0 };
            g_StringLanType(szLan, "用户不存在!", "User not exist!");
            AfxMessageBox(szLan);
        }
    ...
    else
    {
        ...
        if (struDeviceInfoV40.byLoginMode == 0)
        {
            ...
            g_StringLanType(szLanTemp, "登录成功，采用协议为私有协议", "Login SUCC, Use Private");
            char szLan[256] = { 0 };
            sprintf(szLan, "%s, byCharEncodeType[%d]", szLanTemp, struDeviceInfoV40.byCharEncodeType);
            //AfxMessageBox(szLan);
        ...
        else if (6 == struDeviceInfoV40.byPasswordLevel)
        {
            char szLan[128] = { 0 };
            g_StringLanType(szLan, "初始密码，无法进行除修改密码外的其他操作", "initial password, can not operate anything except changing initial password");
            AfxMessageBox(szLan);
        }
    ...
    g_struDeviceInfo[m_iDeviceIndex].byVerifyMode = m_cmbVerifyMode.GetCurSel();  //0-不认证 1-双向认证 2-单向认证
    ...
    g_struDeviceInfo[m_iDeviceIndex].bySupport4 = struDeviceInfo.bySupport4;
...
void CDlgAddDevice::OnActivateDevice()
{
    ...
    struActivateCfg.byHttps = m_cmbHttps.GetCurSel();    //0-http 1-https
    struActivateCfg.byLoginMode = m_cmbLoginMode.GetCurSel();  //0-private 1-ISAPI
    if (struActivateCfg.byHttps > 1 || struActivateCfg.byLoginMode > 1)
    {
        g_StringLanType(szLan, "不支持的方式", "This way is not supported");
        AfxMessageBox(szLan);
        return;
    }
