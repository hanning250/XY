...
public class DoorManage {
    ...
    public static void GetAndSetFloorCfg(int lUserID,int floor)
    {
        ...
        boolean b_Get = AcsMain.hCNetSDK.NET_DVR_GetDVRConfig(lUserID, HCNetSDK.NET_DVR_GET_DOOR_CFG, 1, struFloorCfg.getPointer(),
                struFloorCfg.size(), intByReference);
        if (b_Get = false) {
            System.out.println("获取梯控楼层参数，错误码为" + AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("获取梯控楼层参数成功");
            struFloorCfg.read();
            /**
             * 楼层继电器动作时间:普通卡刷卡后,继电器闭合时间。即刷卡授权后可使用电梯按钮的时间。默认5秒。
             * 梯控访客延迟时间:访客呼梯后,访客可使用电梯按钮的时间。默认5分钟。
             */
            System.out.println("楼层名称："+ new String(struFloorCfg.byDoorName).trim());
            System.out.println("楼层继电器动作时间:"+struFloorCfg.byOpenDuration);
            System.out.println("关门延迟时间:"+struFloorCfg.byDisabledOpenDuration);
            System.out.println("梯控访客延迟时间:"+struFloorCfg.byLadderControlDelayTime);
        }

        /**设置梯控楼层参数*/
        struFloorCfg.byDoorName = "floor111".getBytes();
        struFloorCfg.write();
        boolean b_Set = AcsMain.hCNetSDK.NET_DVR_SetDVRConfig(lUserID, HCNetSDK.NET_DVR_SET_DOOR_CFG, 1, struFloorCfg.getPointer(), struFloorCfg.size());
        if (b_Set = false) {
            System.out.println("设置梯控楼层参数成功，错误码为：" + AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("设置梯控楼层参数成功！！！");
            return;
        }
    }
    ...
    public static void doorTemplate(int lUserID, int iDoorTemplateNo,int floorID) {
        ...
        struDoorStatus.dwTemplateNo = iDoorTemplateNo; //计划模板编号，为0表示取消关联、恢复默认状态（普通状态）。非0回复关联（门序号，与相同序号的门状态计划模板关联）
        struDoorStatus.write();
        boolean b_SetDoorStatus = AcsMain.hCNetSDK.NET_DVR_SetDVRConfig(lUserID, HCNetSDK.NET_DVR_SET_DOOR_STATUS_PLAN, floorID, struDoorStatus.getPointer(), struDoorStatus.size());
        if (b_SetDoorStatus == false) {
            System.out.println("设置梯控计划参数，错误码为" + AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("设置梯控计划参数成功");
        }
        //设置梯控计划模板参数
        ...
        if (b_SetDoorTemp == false) {
            System.out.println("设置梯控计划模板失败，错误码为" + AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("设置梯控计划模板成功");
        }
        //获取(设置)门状态周计划参数
        ...
        if (b_GetPlan == false) {
            System.out.println("获取梯控周计划参数失败，错误码为" + AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("获取梯控周计划参数成功");
        }
        struDoorWeekPlan.read();
        ...
        for (int i = 0; i < 7; i++) {
            ...
            //对接梯控主机需要下发此参数
            struDoorWeekPlan.struPlanCfg[i].struPlanCfgDay[0].byDoorStatus = 2; //2- 常开状态（梯控的自由状态），3- 常闭状态（梯控的禁用状态）
        ...
        boolean b_SetPlan = AcsMain.hCNetSDK.NET_DVR_SetDVRConfig(lUserID, HCNetSDK.NET_DVR_SET_WEEK_PLAN_CFG, 2, pstruDoorWeekPlan, struDoorWeekPlan.size());
        if (b_SetPlan == false) {
            System.out.println("设置门状态周计划参数失败，错误码为" + AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
     ...
     *
     * @param lUserID 用户登录句柄
     * @param lGatewayIndex 楼层编号，从1开始，-1表示对所有门（或者梯控的所有楼层）进行操作
