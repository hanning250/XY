...
//     static HCNetSDK ClientDemo.hCNetSDK = HCNetSDK.INSTANCE;
     int m_lPlayBackHandle;//回放句柄
     int m_lDownloadHandle;//下载句柄
     int m_lUserID;//用户ID
     ...
     *************************************************/
    public JDialogPlayBack(java.awt.Frame parent, boolean modal, int lUserID) {
        super(parent, modal);
        initComponents();
        initialDialog();
        m_lUserID = lUserID;
    ...
    {//GEN-HEADEREND:event_jButtonSearchActionPerformed
        ...
        m_strFilecond.lChannel = Integer.parseInt(jTextFieldChannelNumber.getText());//通道号
        ...
        while (true)
        {
            lnext = ClientDemo.hCNetSDK.NET_DVR_FindNextFile_V30(lFindFile, strFile);
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        boolean flag = ClientDemo.hCNetSDK.NET_DVR_FindClose_V30(lFindFile);
                        if (flag == false)
                        {
    ...
    {//GEN-HEADEREND:event_jButtonDownloadActionPerformed
       //如果不在下载,开始下载
        if (m_lDownloadHandle == -1)
        {
            ...
            //暂且将文件名作为保存的名字
            m_lDownloadHandle = ClientDemo.hCNetSDK.NET_DVR_GetFileByName(m_lUserID, sFileName, sFileName.getBytes());
            if (m_lDownloadHandle >= 0)
            {
                ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lDownloadHandle, HCNetSDK.NET_DVR_PLAYSTART, 0, null);
        ...
        {
            	ClientDemo.hCNetSDK.NET_DVR_StopGetFile(m_lDownloadHandle);
		m_lDownloadHandle=-1;
		jButtonDownload.setText("下载");
		jProgressBar.setValue(0);
    ...
    {//GEN-HEADEREND:event_jButtonSaveActionPerformed
      if(m_lPlayBackHandle == -1)
      {
       ...
       String sFileName = myJFileChooser.getSelectedFile() + ".mp4";
       	...
       	if (ClientDemo.hCNetSDK.NET_DVR_PlayBackSaveData(m_lPlayBackHandle, sFileName))
	{
		m_bSaveFile = true;
	}
	else
     ...
     *************************************************/
     ...
     {//GEN-HEADEREND:event_jButtonStopSaveActionPerformed
         if (m_bSaveFile)
         {
             ClientDemo.hCNetSDK.NET_DVR_StopPlayBackSave(m_lPlayBackHandle);
             m_bSaveFile = false;
         ...
         if (m_lPlayBackHandle >= 0)
	{
		if ((iPos >=0) && (iPos <=100))
		{
			...
			{
				if(ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYSETPOS, iPos, null))
				{
     ...
     *************************************************/
     ...
     {//GEN-HEADEREND:event_jButtonPlayActionPerformed
          if(jTableFile.getSelectedRow() == -1)
       {
        ...
        if(m_lPlayBackHandle != -1)
       {
           ClientDemo.hCNetSDK.NET_DVR_StopPlayBack(m_lPlayBackHandle);
           m_lPlayBackHandle=-1;
       }
       ...
       //获取文件名
        ...
        //调用接口开始回放
        m_lPlayBackHandle = ClientDemo.hCNetSDK.NET_DVR_PlayBackByName(m_lUserID, sFileName, hwnd);
        if (m_lPlayBackHandle > -1)
        {
        ...
        //调用playControl才会开始播放
        if (!ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYSTART, 0, null))
        {
     ...
     *************************************************/
     ...
     {//GEN-HEADEREND:event_jButtonNormalActionPerformed
        ...
        if (!ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYNORMAL, 0, null))
        {
     ...
     *************************************************/
     ...
     {//GEN-HEADEREND:event_jButtonPauseActionPerformed
        ...
        if (!ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYPAUSE, 0, null))
        {
     ...
     *************************************************/
     ...
     {//GEN-HEADEREND:event_jButtonSlowActionPerformed
           ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, ClientDemo.hCNetSDK.NET_DVR_PLAYSLOW, 0, null);
     ...
     *************************************************/
     ...
     {//GEN-HEADEREND:event_jButtonFastActionPerformed
          ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, ClientDemo.hCNetSDK.NET_DVR_PLAYFAST, 0, null);
	...
	if (ClientDemo.hCNetSDK.NET_DVR_PlayBackCaptureFile(m_lPlayBackHandle,sPicName))
	{
   ...
   /*************************************************
    ...
    class DownloadTask extends java.util.TimerTask
    {
        ...
        public void run()
        {
        	int  iPos = ClientDemo.hCNetSDK.NET_DVR_GetDownloadPos(m_lDownloadHandle);
   ...
   /*************************************************
    ...
    class PlaybackTask extends java.util.TimerTask
    {
        ...
        public void run()
        {
            	...
            	if (!m_bGetMaxTime)
		{
			ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_GETTOTALTIME, 0, m_nFileTime);
			if (m_nFileTime.getValue() == 0)
			{
				return;
			}

			if (ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_GETTOTALFRAMES, 0, m_nTotalFrames))
			{
		...
		ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYGETTIME, 0, nCurrentTime);
		...
		ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYGETFRAME, 0, nCurrentFrame);
		...
		ClientDemo.hCNetSDK.NET_DVR_PlayBackControl(m_lPlayBackHandle, HCNetSDK.NET_DVR_PLAYGETPOS, 0, nPos);
   ...
   /*************************************************
        	...
        	if (m_lPlayBackHandle >= 0)
	{
		if (m_bSaveFile)
		{
			if (!ClientDemo.hCNetSDK.NET_DVR_StopPlayBackSave(m_lPlayBackHandle))
			{
		...
		if (!ClientDemo.hCNetSDK.NET_DVR_StopPlayBack(m_lPlayBackHandle))
		{
