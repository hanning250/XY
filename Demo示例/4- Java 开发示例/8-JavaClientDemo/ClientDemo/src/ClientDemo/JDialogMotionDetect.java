...
//    static HCNetSDK ClientDemo.hCNetSDK = HCNetSDK.INSTANCE;
    ...
    private int m_lUserID;//用户句柄
    private int m_lPlayHandle;//预览句柄
     ...
     *************************************************/
    public JDialogMotionDetect(javax.swing.JDialog parent, boolean modal, int lUserID, int iChannelNum, HCNetSDK.NET_DVR_PICCFG_V30 struPicCfg, HCNetSDK.NET_DVR_DEVICEINFO_V30 strDeviceInfo)
    {
        super(parent, modal);
        initComponents();
    ...
    private void initialDialog()
    {
        ...
        m_strClientInfo.lChannel =m_iChanShowNum;
        m_strClientInfo.hPlayWnd = hwnd;
        m_lPlayHandle = ClientDemo.hCNetSDK.NET_DVR_RealPlay_V30(m_lUserID,
                m_strClientInfo, null, null, true);
        if (m_lPlayHandle == -1)
        {
            JOptionPane.showMessageDialog(this, "预览失败,错误值:" + ClientDemo.hCNetSDK.NET_DVR_GetLastError());
        }
    ...
    private void initComponents() {
        ...
        jCheckBoxMonitorAlarm.setText("布防器警告");
	...
	{
		ClientDemo.hCNetSDK.NET_DVR_RigisterDrawFun(m_lPlayHandle,null, 0);
            ...
            } catch (InterruptedException ex)
            {
                ...
                ClientDemo.hCNetSDK.NET_DVR_RigisterDrawFun(m_lPlayHandle,MotionDetectGetCallBack, 0);
	}
	else
	{
		ClientDemo.hCNetSDK.NET_DVR_RigisterDrawFun(m_lPlayHandle,null, 0);
       	...
       	if (m_bDrawArea)
	{
			POINT point = new POINT(evt.getX(), evt.getY());
                       ClientDemo.hCNetSDK.NET_DVR_RigisterDrawFun(m_lPlayHandle, MotionDetectSetCallBack, 0);
	...
	{
		ClientDemo.hCNetSDK.NET_DVR_RigisterDrawFun(m_lPlayHandle, null, 0);
		ClientDemo.hCNetSDK.NET_DVR_StopRealPlay(m_lPlayHandle);
		m_lPlayHandle=-1;
	}
        dispose();
    ...
    {
        public void invoke(int lRealHandle, HDC hDc, int dwUser)
        {
        int i = 0;
