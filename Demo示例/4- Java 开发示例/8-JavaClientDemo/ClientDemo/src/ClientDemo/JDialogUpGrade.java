...
//    static HCNetSDK ClientDemo.hCNetSDK = HCNetSDK.INSTANCE;
    ...
    private int m_lUpgradeHandle;//文件升级句柄
    Timer timer;//定时器
     ...
     *************************************************/
    public JDialogUpGrade(java.awt.Frame parent, boolean modal, int lUserID)
    {
        super(parent, modal);
        m_lUserID = lUserID;
        m_lUpgradeHandle = -1;
    ...
    private void jButtonSetNetEnvirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSetNetEnvirActionPerformed
        if (!ClientDemo.hCNetSDK.NET_DVR_SetNetworkEnvironment(jComboBoxNetEnvir.getSelectedIndex()))
        {
    ...
    *************************************************/
    private void jButtonUpgradeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonUpgradeActionPerformed
        ...
        m_lUpgradeHandle = ClientDemo.hCNetSDK.NET_DVR_Upgrade(m_lUserID, jTextFieldFileDir.getText());
    ...
    {//定时器函数 相当于c语言中的onTimer();
        ...
        public void run()
        {
            int UpgradeStatic = ClientDemo.hCNetSDK.NET_DVR_GetUpgradeState(m_lUpgradeHandle);
            int iPos = ClientDemo.hCNetSDK.NET_DVR_GetUpgradeProgress(m_lUpgradeHandle);
            ...
            } else
            {
                ...
                if (ClientDemo.hCNetSDK.NET_DVR_CloseUpgradeHandle(m_lUpgradeHandle) == true)
                {
