...
//    static HCNetSDK ClientDemo.hCNetSDK = HCNetSDK.INSTANCE;
     ...
     *************************************************/
    public JDialogFormatDisk(java.awt.Frame parent, boolean modal, int userId)
    {
        super(parent, modal);
        initComponents();
        ...
        boolean getDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_GetDVRConfig(m_lUserId, HCNetSDK.NET_DVR_GET_HDCFG,
                0, lpPicConfig, m_struHDCfg.size(), ibrBytesReturned);
        m_struHDCfg.read();
    ...
    private void jButtonFormatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonFormatActionPerformed
        IntByReference pCurrentFormatDisk = new IntByReference(-1);
        IntByReference pCurrentDiskPos = new IntByReference(-1);
        IntByReference pFormatStatic = new IntByReference(-1);
        int iDiskNumber = jComboBoxDisk.getSelectedIndex();
        if (iDiskNumber == 0)
        {
            m_lFormatHandle = ClientDemo.hCNetSDK.NET_DVR_FormatDisk(m_lUserId, 0xff);//0xFF表示所有硬盘
            if (m_lFormatHandle < 0)
            {
                int err = ClientDemo.hCNetSDK.NET_DVR_GetLastError();
            ...
            } else
            {
                ...
                do
                {
                    ClientDemo.hCNetSDK.NET_DVR_GetFormatProgress(m_lFormatHandle, pCurrentFormatDisk, pCurrentDiskPos, pFormatStatic);
                    System.out.println("正在格式化硬盘:" + pCurrentFormatDisk.getValue());//正在格式化的硬盘号
                    jProgressBar.setValue(pCurrentDiskPos.getValue());
                    try
                    {//等待500ms
                ...
                } while (pFormatStatic.getValue() == 0);//正在格式化
                System.out.println("static" + pFormatStatic.getValue());
                if (pFormatStatic.getValue() == 2)
                {
                    JOptionPane.showMessageDialog(this, "格式化出错!");
                    return;
                }
                if (pFormatStatic.getValue() == 3)
                {
        ...
        {//格式化单个硬盘
            m_lFormatHandle = ClientDemo.hCNetSDK.NET_DVR_FormatDisk(m_lUserId, m_struHDCfg.struHDInfo[iDiskNumber - 1].dwHDNo);
            if (m_lFormatHandle < 0)
            {
                int err = ClientDemo.hCNetSDK.NET_DVR_GetLastError();
                if (err == HCNetSDK.NET_DVR_DISK_FORMATING)
