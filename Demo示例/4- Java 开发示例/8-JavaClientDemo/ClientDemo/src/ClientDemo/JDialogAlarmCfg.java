...
//    static ClientDemo.hCNetSDK ClientDemo.hCNetSDK = ClientDemo.hCNetSDK.INSTANCE;
    private int m_lUserID;//用户ID
    private HCNetSDK.NET_DVR_DEVICEINFO_V30 m_strDeviceInfo;//设备信息
    private CheckListItem m_exceptionAlarmOut[] = new CheckListItem[ClientDemo.hCNetSDK.MAX_ALARMOUT_V30];//异常配置报警输出通道checkbox对应值
    private CheckListItem m_traggerAlarmOut[] = new CheckListItem[ClientDemo.hCNetSDK.MAX_ALARMOUT_V30];//报警输入配置中报警输出通道checkbox对应值
    private CheckListItem m_traggerRecord[] = new CheckListItem[ClientDemo.hCNetSDK.MAX_CHANNUM_V30];//报警输入配置中触发录像通道checkbox对应值
     ...
     *************************************************/
    public JDialogAlarmCfg(java.awt.Frame parent, boolean modal, int lUserID, HCNetSDK.NET_DVR_DEVICEINFO_V30 strDeviceInfo)
    {
        super(parent, modal);
        ...
        for (int i = 0; i < ClientDemo.hCNetSDK.MAX_ALARMOUT_V30; i++)
        {
            m_exceptionAlarmOut[i] = new CheckListItem(false, null);
            m_traggerAlarmOut[i] = new CheckListItem(false, null);
        }
         for (int i = 0; i < ClientDemo.hCNetSDK.MAX_CHANNUM_V30; i++)
        {
        ...
        for (int i = 0; i < ClientDemo.hCNetSDK.MAX_CRUISE_V30; i++)
        {
            jComboBoxCruise.addItem(i + 1);
        }
        for (int i = 0; i < ClientDemo.hCNetSDK.MAX_TRACK_V30; i++)
        {
            jComboBoxTrack.addItem(i + 1);
        }
        for (int i = 0; i < ClientDemo.hCNetSDK.MAX_PRESET_V30; i++)
        {
    ...
    private void initComponents() {
        ...
        jCheckBoxMoniter.setText("布防器上警告");
        ...
        jCheckBoxMoniterAlarm.setText("布防器上警告");
    ...
    private void jButtonSetupExceptionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSetupExceptionActionPerformed
        ...
        boolean setDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, ClientDemo.hCNetSDK.NET_DVR_SET_EXCEPTIONCFG_V30,
                0, lpConfig, m_struExceptionInfo.size());
    ...
    private void jToggleButtonSetupAlarmOutActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jToggleButtonSetupAlarmOutActionPerformed
        ...
        boolean setDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, ClientDemo.hCNetSDK.NET_DVR_SET_ALARMOUTCFG_V30,
                jComboBoxAlarmOutChannel.getSelectedIndex(), lpConfig, m_struAlarmOutCfg.size());
	...
	for (int i = 0; i< ClientDemo.hCNetSDK.MAX_CHANNUM_V30; i++)
	{
        ...
        boolean setDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, ClientDemo.hCNetSDK.NET_DVR_SET_ALARMINCFG_V30,
                jComboBoxAlarmOutChannel.getSelectedIndex(), lpConfig, m_struAlarmInCfg.size());
    ...
    private void showExceptionCfg()
    {
        ...
        m_struExceptionInfo.write();
        Pointer lpConfig = m_struExceptionInfo.getPointer();
        getDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, ClientDemo.hCNetSDK.NET_DVR_GET_EXCEPTIONCFG_V30,
                0, lpConfig, m_struExceptionInfo.size(), ibrBytesReturned);
        m_struExceptionInfo.read();
    ...
    private void showAlarmInCfg()
    {
        ...
        m_struAlarmInCfg.write();
        Pointer lpConfig = m_struAlarmInCfg.getPointer();
        boolean getDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, ClientDemo.hCNetSDK.NET_DVR_GET_ALARMINCFG_V30,
                jComboBoxAlarmInChannel.getSelectedIndex(), lpConfig, m_struAlarmInCfg.size(), ibrBytesReturned);
        m_struAlarmInCfg.read();
        if (getDVRConfigSuc != true)
        {
    ...
    private void showAlarmOutCfg()
    {
        ...
        m_struAlarmOutCfg.write();
        Pointer lpConfig = m_struAlarmOutCfg.getPointer();
        boolean getDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, ClientDemo.hCNetSDK.NET_DVR_GET_ALARMOUTCFG_V30,
