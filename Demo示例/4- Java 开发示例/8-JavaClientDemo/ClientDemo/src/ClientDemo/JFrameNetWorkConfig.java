...
//    static HCNetSDK ClientDemo.hCNetSDK = HCNetSDK.INSTANCE;
     ...
     *************************************************/
    public JFrameNetWorkConfig(int lUserID)
    {
    ...
    private void jButtonSetupActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSetupActionPerformed
        ...
        boolean setDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, HCNetSDK.NET_DVR_SET_NETCFG_V30,
                0, lpNetConfig, m_strNetConfig.size());
        m_strNetConfig.read();
        if (setDVRConfigSuc == false)
        {
            JOptionPane.showMessageDialog(this, "设置网络参数失败");
            System.out.print("" + ClientDemo.hCNetSDK.NET_DVR_GetLastError());
        } else
        {
     ...
     private void initDialog()
     {
         ...
         m_strNetConfig.write();
         Pointer lpNetConfig = m_strNetConfig.getPointer();
         boolean getDVRConfigSuc = ClientDemo.hCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, HCNetSDK.NET_DVR_GET_NETCFG_V30,
                 0, lpNetConfig, m_strNetConfig.size(), ibrBytesReturned);
         m_strNetConfig.read();
