...
//    static HCNetSDK ClientDemo.hCNetSDK = HCNetSDK.INSTANCE;
     ...
     *************************************************/
    public JDialogVoiceTalk(java.awt.Frame parent, boolean modal, int lUserID, HCNetSDK.NET_DVR_DEVICEINFO_V30 strDeviceInfo) 
    {
        super(parent, modal);
        try
        {
	...
	//这里采用全局的句柄进行操作,因为子对话框销毁后语音对讲还可以进行,用全局的变量保存句柄
        ClientDemo.g_lVoiceHandle = ClientDemo.hCNetSDK.NET_DVR_StartVoiceCom_V30(m_lUserID, m_iSel+1, jComboBoxDataType.getSelectedIndex() == 1, fVoiceDataCallBack,  null);
	if ( ClientDemo.g_lVoiceHandle == -1)
	{
	...
	{
		if (ClientDemo.hCNetSDK.NET_DVR_StopVoiceCom( ClientDemo.g_lVoiceHandle))
		{
			 ClientDemo.g_lVoiceHandle=-1;
		}
		else
		{
    ...
    {//GEN-HEADEREND:event_jButtonRefreshActionPerformed
	    if (!ClientDemo.hCNetSDK.NET_DVR_GetDVRWorkState_V30(m_lUserID, m_strWorkState))
        {
       ...
       //对对讲的音频数据进行回调操作,以下写入文件操作
        public void invoke(int lVoiceComHandle, Pointer pRecvDataBuffer, int dwBufSize, byte byAudioFlag, Pointer pUser)
        {
            ...
            if (byAudioFlag == 0)
            {
                try
                {
                    fLocal.write(pRecvDataBuffer.getCharArray(0,dwBufSize));
                    fLocal.flush();
                } catch (IOException ex)
                {
                    Logger.getLogger(JDialogVoiceTalk.class.getName()).log(Level.SEVERE, null, ex);
            ...
            } else
            {
                if (byAudioFlag == 1)
                {
                    try
                    {
                        fDevice.write(pRecvDataBuffer.getCharArray(0,dwBufSize));
                        fDevice.flush();
                    } catch (IOException ex)
                    {
                        Logger.getLogger(JDialogVoiceTalk.class.getName()).log(Level.SEVERE, null, ex);
