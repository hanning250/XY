...
//网络(NET_DVR_NETCFG_V30结构)
    ...
    public static final int NET_DVR_GET_ENTRANCE_PARAMCFG = 3126;   //获取出入口控制参数
...
//入侵参数
    ...
    */
    ...
    //车辆信息管控参数结构体
    public static class NET_DVR_VEHICLE_CONTROL extends Structure {

        public byte   byGateOperateType;//0-无操作,1-开道闸
        public byte   byRes1 ;
        public short   wAlarmOperateType;//0-无操作,bit0-继电器输出报警 bit1-布防上传报警 bit3-告警主机上传（可复选）(0-表示关，1-表示开)
        public byte[]   byRes2=new byte[8];
    }

    //继电器关联配置结构体
    public static class NET_DVR_RELAY_PARAM extends Structure {
        public byte byAccessDevInfo;//0-不接入设备，1-开道闸、2-关道闸、3-停道闸、4-报警信号、5-常亮灯
        public byte[] byRes = new byte[3];
    }

    //出入口配置
    public static class NET_DVR_ENTRANCE_CFG extends Structure {
        public int dwSize;
        public byte byEnable;// 使能0-表示关闭，1-表示打开
        public byte byBarrierGateCtrlMode;//0-相机自动控制，1-平台外部控制
        public byte[] byRes1 = new byte[2];
        public int dwRelateTriggerMode;//关联触发模式 参考ITC_TRIGGERMODE_TYPE
        /*0x00: 不做匹配*/
        /*0x01: 车牌匹配（带车牌颜色）*/
        /*0x02: 车牌匹配（不带车牌颜色）*/
        /*0x04: 卡号匹配*/
        public int dwMatchContent;  //异常处理,异常处理方式的"或"结果
        public NET_DVR_RELAY_PARAM[] struRelayRelateInfo = new NET_DVR_RELAY_PARAM[MAX_RELAY_NUM/*12*/];//继电器关联配置信息(一个继电器只能控制一个接入设备) 数组0表示继电器1 数组1表示继电器2 依次类推 12
        public byte[] byGateSingleIO = new byte[MAX_IOIN_NUM/*8*/]; //单个IO触发参数，单个IO触发参数，数组0表示IO1，数组1表示IO2，依次类推 ; 具体数组值的含义（0-无，1-道闸开到位，2-道闸关到位，3-消防报警）
        //车辆信息管控
        //数组0表示临时车辆配置的模式
        //数组1-黑名单车辆的模式,
        //数组2-白名单车辆的模式
        public NET_DVR_VEHICLE_CONTROL[] struVehicleCtrl = new NET_DVR_VEHICLE_CONTROL[MAX_VEHICLE_TYPE_NUM/*8*/];//车辆信息管控
        public byte byNotCloseCarFollow;//启用跟车不落闸使能，0~为不启用，1~启用
        public byte byParkingDetectEnabled;//启用驻车检测使能，0~为不启用，1~启用
        public byte byParkingDetectJudgeTime;//驻车检测判断时间，单位秒
        public byte[] byRes2 = new byte[61];
    }

    //出入口条件
    public static class NET_DVR_BARRIERGATE_COND extends Structure {
        public byte byLaneNo;//车道号（0-表示无效值(设备需要做有效值判断),1-车道1）
        public byte[] byRes = new byte[3];

        public static class ByReference extends NET_DVR_BARRIERGATE_COND implements Structure.ByReference {}
    }
